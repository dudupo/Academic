...
\end{lemma}
  ...
  So, we can assume that the vertices in $V^\prime$ compose a single connectivity component. Let be $x|_{v} \in \mathbb{F}_{2}^{\Delta}$ the bits of $x$ on the indices corresponding to $v$'s edges. For any $S \subset E$, define $w_{S}\left( x \right)$ as the weight that $x$ induces over $S$. And notice that any edge of $E$ connected only to a single vertex in $V^\prime$ equals the corresponding bit in the original suggestion made by $c_{v}$. Hence for every $v\in V^\prime$, it holds that $w_{E / E^\prime}\left(x|_{v}\right) = w_{E / E^\prime}\left(c_{v}\right) \Rightarrow \le | x \cap c_{v} | $. 
  ...
  \begin{proof} Assume through contradiction that $w_{E^\prime}\left( c_{v} \right) < \frac{1}{2}\delta_{0}\Delta$, and consider the weight of $|x + c_{v}| = |x| + |c_{v}| - 2|x \cap c_{v}|$. By using the previews insight we get: \begin{equation*}
    \begin{split}
