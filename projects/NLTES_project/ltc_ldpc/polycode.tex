...
\end{proof}
\begin{lemma}[Testability Of Polynomail Code.] Let $G$ be an algortithm that given $d+1$ points return the polynomail that pass throu\ctt{here} Testability
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{split}
      \text{agree} & \le \mathbf{Pr}_{x_{1}, x_{2} ... x_{d} \sim_{U}, x_{d+1}\sim \mathbb{F}_{}}\left[ (Gf)(x) = f(x) \right] \\ & \le \prb{ (Gf)(x) = f(x)} \le \varepsilon \\
       1 - \text{agree} & \ge 1 - \varepsilon 
    \end{split}
  \end{equation*}
\end{proof}
