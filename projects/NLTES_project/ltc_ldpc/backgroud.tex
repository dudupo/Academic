...
\end{lemma}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Where $n(C_{i})$ is the code length of the $i$th code. By repeating on the above $ \log_{\Delta}^{*}(n)$ times, One can lift the initial code $\Delta[1,\rho_{0}]$ to $n[1, \sim 2\rho^{\log_{\Delta}^{*}(n)}]$. Nevertheless any family of finite Groups generated by constant size generator set can define a family of codes by taking their Cayley graphs as a base for Tanner code.   

  The next question to engage after we saw that Tanner codes enable us to achieve rates is to ask what is the distance of the codes. We will need to require a little bit more  of the graph s to achieve a linear distance. But for undersetting that, let's back to the repetition code again.    
  \begin{example}
    The repetition code could be present as Tanner code, over the cycle graph. Any vertex checks that the bits on it's edge are equal, Hence a codeword is an assignment on which all the bits equal (otherwise there is an edge which not connect to any overtax). \cref{fig:cyc} ilustrate a legal assignment.  
  \end{example} 
...
\end{figure}
\end{center}
Now, recall that the distance of linear code is the minimal weight of non-zero codeword. Consider a codeword $c \in C$, and let group the vertex by for sets $V_{i}$ such $V_{i}$ is the set of vertices that see $i \in \{0,1\}^{2}$. By the fact that $c \in C$ we have that $|V_{10}|=|V_{01}| = 0$. But on the other hand any vertex in $V_{00}$ can't be connected to $V_{11}$, Thus we obtain that either all the vertices in $V_{11}$ tr that the graph is not connected. The distance of the code is $\frac{1}{2}\sum{|V_{i}|\cdot |i|} = \frac{1}{2}2 \cdot n = n$.             
