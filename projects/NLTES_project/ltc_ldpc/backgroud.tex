...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Denote $k = \rho n$ and notice that the above holds when $\rho \ge \left(\frac{1}{2} - \delta \right)^{2} $. Or in other words, for any $k = \Theta(n)$ there is a positve $\delta$ such with high probaility the distance of $\Ima$ is at least $\delta n $. 
\end{proof}
%
