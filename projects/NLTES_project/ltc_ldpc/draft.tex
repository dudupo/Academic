...
\begin{claim}
  Let $C_{0}$ be the $[\Delta,d, \Delta-d]$ polynomial code. Then $\left( C_{0}^{\perp} \otimes C_{0}^{\perp} \right)^{\perp}$ is the $[\Delta^{2}, \Delta+d, n^{2} - ? ]  $


  \begin{equation*}
    \begin{split}
      \left( R + C  \right) \delta_{0}\Delta - RC \cdot \gamma \le \Delta^{1 \frac{1}{2}}
    \end{split}
  \end{equation*}<++>
