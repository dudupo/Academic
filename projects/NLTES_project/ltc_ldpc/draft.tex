...
\begin{proof}
  ...
  Hence by Schwartz-Zippel lemma \cite{Schwartz} we have that $\gamma RC \le \left( d + \Delta \right) \sqrt{RC} $. On the other hand, the number of weight of $c$ is at least: 
\begin{equation*}
    \begin{split}
      |c| \ge \left( R + C  \right) \delta_{0}\Delta - 2 RC \cdot \gamma  \ge \left( R + C \right)\delta_{0}\Delta -2 \left( d + \Delta  \right)\sqrt{RC}
    \end{split}
  \end{equation*}
