...
Yet, we still need more to prove a linear distance. By repeating the proof of the Singleton Bound (Ref. \ref{theorem*:Sing}), it follows that the small code $\tilde{C_{0}}$ obtained by ignoring arbitrary $(q - \frac{1}{2})\Delta$ coordinates yields a code with distance:
\begin{equation*}
  \begin{split}
...
\begin{definition}[Testability Tanner Code]\label{testtaner} Let $q > \frac{1}{2}$ and let $J$ be a generator set for group $\Gamma$ such that $|J| = \Delta$, $q | \Delta $, $J$ closed for inverse, and there exist subset of $J$, denote it by, $J^{\prime}$ such that $J^{\prime}$ is a generator set of $\Gamma$ and $|J^{\prime}| = \frac{1}{2}\Delta$. Let $C_{0}$ be a code with parameters $C_{0} = q\Delta \left[1, \rho_{0}, \delta_{0}\right]$. For any vertex associate a subset $\Jvv \subset J/J^{\prime}$ at size: 
...
\begin{proof} We have already proved that the code has a positive rate, so it is left to show a constant relative distance.
...
\begin{remark}[Regarding Quantum Codes.]
...
\end{remark}
\begin{remark}[Note On Random Construction.]
...
%\begin{conjecture}
%  \label{conjecture:setJ} 
%  There is infintly many graphs defined in \ref{testtaner}  
%\end{conjecture}
...
\begin{proof}[Proof of~\cref{theorem:highflux}.] 
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
When the last inequality follows from the fact that the proof of~\cref{claim:maxu} does not rely on graph structure arguments. The same arguments also lead to an analogous inequality for~\cref{claim:flu2}. Choosing $J$ such that \emph{Cayley}$\left( \Gamma, J \right)$ is ramnujan provide that $ 2\lambda/ q\Delta$ scale as $\Theta\left( \Delta^{-1/2} \right)$. That close the case in which there is a linear size layer of nontrivial suggestions. 
\end{proof}

\begin{openproblem}
Is there a decoder/tester for~\ref{testtaner} that can take advantage of the high flux induced by the disagreement on the trivial vertices?
\end{openproblem}
