...
%
	...
	And Let us say that that a tuple $e = \left(  e_1, e_2\right) $   is heavy edge if $e_1,e_2$ are edges in the graphs $\Gsah, \Gsbh$ accordingly and that concatenation is incident to at least $\eta$ plaquettes in $\Gsah$ and $\Gsbh$. Let $ T$ be set of vertices in $V_0$ that are connected to (at least) one normal vertex through a heavy edge. 
    
	Recall that the distance of the tensor code equal to the distances multiplication of commponents code.It is clear that the distance of $\Aabpp$ and $\Acdpp$ is exectly $\delta\Delta $ and therefore the distance of $\Aabcdpp$ equals $\left( \delta\Delta \right)^{2} $.  
      

    Hence the number of vertices in the induced graph by $x$ is bounded by it's weight:
    $ |S| \le \frac{6|x|}{ \delta^{2}\Delta^{2} }$ 
