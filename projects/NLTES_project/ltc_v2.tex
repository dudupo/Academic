...
Let $ T$ be set of vertices in $V_0$ that are connected to (at least) one normal vertex through a heavy edge.  
    ...
    \begin{equation*}
      \begin{split}
	| \ESTs | & \ge \eta |T| \\ 
	\Rightarrow  & |T|\left( \eta - \frac{2\Delta^{2}\cdot 2|x|}{n\left( \delta\Delta \right)^{2}}  \right) \le  2 \lambda\Delta\sqrt{|S||T|} 
      \end{split}
    \end{equation*}
    Hence we have that:
    \begin{equation*}
      \begin{split}
	& \sqrt{|T|}\left( \eta - \frac{2\Delta^{2}\cdot 2|x|}{n\left( \delta\Delta \right)^{2}}  \right) \le   2\lambda\Delta\sqrt{|S|} \\ 
	& |T| \le \left( \frac{ 2\lambda\Delta}{\eta - \frac{4 |x|}{n\ \delta^{2}} } \right)^{2} |S|	
      \end{split}
    \end{equation*}
