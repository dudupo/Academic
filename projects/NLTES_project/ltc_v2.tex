...
Let $ T$ be set of vertices in $V_0$ that are connected to (at least) one normal vertex through a heavy edge.  
 ...
 $ \Rightarrow \tau \ge \frac{\rho}{2\left( 1 - \rho \right)}\ge \rho/3 $. Namely, at least $\rho/3$ of vertices of $T$ are incident to at least $\frac{1}{2}\Delta\rho$ heavy edges. 

 Since each vertex is a source for at most $2\Delta^{2}$ step-edges we get that $|S| - |S_{e}| \le 2\Delta^{2}|T| $. In the other-hand we have shown that 
 \begin{equation*}
   \begin{split}
