...
\paragraph{Tasks.}
	...
	\begin{equation*}
	  \begin{split}
        ...
        \end{cases}
	  \end{split}
	\end{equation*} \color{red} Taking in acount higher momuntens \textbf{only} in the second case is still preserving the inequality direction? For now assume also that $C$ has maximal weight of $\tilde{\delta}$. \color{black} 
	By the assumption that $C$ has maximal weight of $\tilde{\delta}$ we get that the contribuation of the second case is bounded by $\frac{1}{2}\left( \#_{A} + \#_{B} \right)\gamma\left( 1-\tilde{\delta} \right)\Delta^{2} $. 
