...
\paragraph{Tasks.}
	...
	\textbf{Proof.} The proof is trival. 
	\paragraph{Existance Of \BGS Codes}. We will now show that the codes of Levarier and Zemor has the \BGS property. Let $C^{\perp}_{z}$ be the $\left[n, k\ge \left( 2\rho^2-1 \right)n, d\ge \beta^{2}\left( \beta - \lambda/\Delta \right)n \right]$ LTC code defined in \color{blue} cite\color{black}. And recall that the distance of their CSS code is greter than $\tilde{\delta}=\left( \beta / 4\Delta^{3/2+\varepsilon} \right)n $. \\
	By defination the distance of the css code is the weight of the smallest codeword in $C_{x}$ which is not in $C^{\perp}_{z}$. Hence, the codewords of $C^{\perp}_{z}$ are at weight at most $\tilde{\delta}$.   
	\section{The Code.}
