...
\paragraph{Tasks.}
	...
	Remind that the dual tensor of $\Aabpp$ is spanned by the products $ \left\{ e_{i}\otimes v_{j} \right\} $ and $\left\{ u_{i}\otimes e_{j} \right\}$ we will call the vectors which are coresponded to $\mathbb{F}^{\Delta} \otimes \Ab$ 'right products' and similary we will give the name 'left products' to the vectors which spans $\Aa \otims \mathbb{F}^{\Delta}$. 


	the  . Let us write $x$ as formal sum of $\#_{A}$ 
