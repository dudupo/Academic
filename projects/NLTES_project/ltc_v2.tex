...
%
	...
	|E\left( S,T \right)_{\Gssh}| &\le \frac{\Delta^{2}}{n}\Delta^{2}|S||T| + \lambda_{\Gsbh}\Delta\sqrt{|S||T|} \\ 
	  ...
	  \end{split}
	\end{equation*}
	\paragraph{Dense Normal Net Counting} Let $x \in \mathbb{F}^{\left\{ \cdot \right\}}  $ be aword, And consider the graph $\Gsseh\left( x \right)$  obtained by taking the edges induced by setting $x$ over the plaquettes.  
	Let us call the normal vertices the vertices with degree less then $\xi$ in $\Gsseh\left( x \right)$.  
