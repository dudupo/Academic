...
\paragraph{Tasks.}
	...
	Denote by $U = \{u_{0},u_{1},u_{2},\dots u_{\rho^{\prime n}} \}$ the set which spans $\mathcal{C}$, and assume that $U$ is also the base, namly $\log \dim U = \log \dim \mathcal{C} = \rho^{\prime} n $.

	And also denote by $V = \{v_{0},v_{1},v_{2},\dots v_{q}  \} $ the set which spans $C_{z}^{\perp}$ and also $v_{i}$ assosiate with setting some codeward in $\Aab$ over the some vertex. 

	Let $A\in \mathbb{F}^{\rho^{\prime n } \times q}_{2}$ be the matrix such $A : [V] \mapsto [U] $. Now, any face supports at most $2\rho^{2}\Delta^{2}$ (choosing one of the negative vertices of the face and then choosing a codeword of the product $\Aab$). Then by Remark-14 of \color{blue}cite\color{black}, for large enough $\Delta$ we have that each of the genrator contribute at least $\frac{1}{2}\Delta^{2}$ to the weigth. and therefore each row of $A$ has at most $2\tilde{\delta}n/\Delta^{2}$ non zeros. Hence the total number of ones in $A$ is at most $\rho^{\prime}n \cdot 2\tilde{\delta}n/\Delta^{2}$ whcih also bounds the number of times each face appers in the matrix. As we have the fredom to choose $\rho$ let's fix it to be at order $c/n$ and therfore we have that $\gamma = \tilde{\delta}/\Delta^{2}$.   
	%$C_{ Take the base,   
