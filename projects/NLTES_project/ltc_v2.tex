...
\paragraph{Tasks.}
	...
	Denote by $U = \{u_{0},u_{1},u_{2},\dots u_{\rho^{\prime}} \}$ the set which spans $\mathcal{C}$, and assume that $U$ is also the base, namly $\log \dim U = \log \dim \mathcal{C} = \rho^{\prime} n $.
	...
	Let $A\in \mathbb{F}^{\rho \times q}_{2}$ be the matrix such $A : [V] \mapsto [U] $. Now, any face supports at most $2\prime^{2}\Delta^{2}$ (choosing one of the negative vertices of the face and then choosing a codeword of the product $\Aab$). Then by Remark-14 of \color{blue}cite\color{black}, for large enough $\Delta$ we have that each of the genrator contribute at least $\frac{1}{2}\Delta^{2}$ to the weigth. and therefore each row of $A$ has at most $2\tilde{\delta}n/\Delta^{2}$ non zeros.    genrator in $V$  
	%$C_{ Take the base,   
