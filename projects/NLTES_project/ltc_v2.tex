...
%
	...
	Consider now subsets $S^{\star}_{L}$ and $S^{\star}_{R}$ which are the vertices which can be reach from $S$ trough left, right edges. And notice that each step-edge is a path compused by an source vertex from $S$, an intermdiate vertex from $S^{\star}_{L}$ or $S^{\star}_{R}$ and a target vertex from $T$. In addition, by regularity the size of $S^{\star}_{L}$ is at most $\Delta|S|$.

	Hence by applaing the Mixing Lemma we have: 
		\begin{equation*}
	  \begin{split}
	    \ESTs = E\left( S^{\star}_{L}, T  \right)_{\cdot} +  E\left( S^{\star}_{R}, T  \right)_{\cdot} \\
	|\ESTs| &\le \frac{\Delta^{2}}{n}\Delta|S||T| + \lambda_{\Gsbh}\sqrt{\Delta|S||T|} \\ 
      |\ESTs| &\ge \frac{\Delta^{2}}{n}|S||T| - \lambda\sqrt{|S||T|}  
	  \end{split}
	\end{equation*}
...
And Let us say that the tuple $ e = \left( e_{1} e_{2} \right)$ is an heavy edge if $e_{1},e_{2}$ are adjacents squares and they incident to at least $\eta$ plaquettes.
...
Let $ T$ be set of vertices in $V_0$ that are connected to (at least) one normal vertex through a heavy edge.  
    ...
    \begin{equation*}
      \begin{split}
	| \EST | & \ge \eta |T| \\ 
	\Rightarrow  & |T|\left( \eta - \frac{\Delta^{4}\cdot 2|x|}{n\left( \delta\Delta \right)^{2}}  \right) \le   \lambda_{\Gsbh}\Delta\sqrt{|S||T|} 
      \end{split}
    \end{equation*}
    Hence we have that:
    \begin{equation*}
      \begin{split}
	& \sqrt{|T|}\left( \eta - \frac{\Delta^{4}\cdot 2|x|}{n\left( \delta\Delta \right)^{2}}  \right) \le   \lambda_{\Gsbh}\Delta\sqrt{|S|} \\ 
	& |T| \le \left( \frac{ \lambda_{\Gsbh}\Delta}{\eta - \frac{\Delta^{2}\cdot 2|x|}{n\ \delta^{2}} } \right)^{2} |S|	
      \end{split}
    \end{equation*}
