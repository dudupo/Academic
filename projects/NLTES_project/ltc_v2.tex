...
\paragraph{Tasks.}
	...
	Hence it's left to show that we could pick a base which has no more than $m$ vectors sharing the same bit. By Theorem 1 in \color{blue}cite\color{black} the set of codewords which entirly supported on the neighbourhood of negative codewords spans the the codeword of $C^{\perp}_{z}$ and any vecor set which spans $\mathcal{C}$ could represnted as linear transforamrion over the any set which sapns $C^{\perp}_{z}$. Denote by $U = \{u_{0},u_{1},u_{2},\dots u_{\rho} \}$ the set which spans $\mathcal{C}$  and by $V = \{v_{0},v_{1},v_{2},\dots v_{q}  \} $ the set which spans $C_{z}^{\perp}$ and also $v_{i}$ assosiate with setting some codeward in $\Aab$ over the some vertex. 
	%$C_{ Take the base,   
