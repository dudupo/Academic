...
\paragraph{Tasks.}
	...
	Denote by $U = \{u_{0},u_{1},u_{2},\dots u_{\rho} \}$ the set which spans $\mathcal{C}$, and assume that $U$ is also the base, namly $\log \dim U = \log \dim \mathcal{C} = \rho n $.
	...
	Let $A\in \mathbb{F}^{\rho \times q}_{2}$ be the matrix such $A : [V] \mapsto [U] $.
	%$C_{ Take the base,   
