...
\title{Tanner Codes Singleton Bound}
\author{David Ponarovsky}
\maketitle
\abstract{ We propose a new asymptotic upper bound on the trade-off between the rate and the distance of error correction codes obtained by Tanner construction.  }
\begin{multicols*}{2}
...
\begin{figure}[H]
 % \centering
\input{singleton}
\caption{\( \hat{y} \) axis is the square amplitude, after performing symmetrtion in Fourier space, \(\hat{x}\) axis is the frequency in \( kHz \).  }
\end{figure}
