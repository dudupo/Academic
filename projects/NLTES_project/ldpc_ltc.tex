...
\begin{equation*}
  \begin{split}
    ...
    &  \delta_{0}\Delta|U| - \Delta\frac{|U||U_{-1}}{n} - \Delta\frac{|U||U_{+1}}{n} -\lambda\sqrt{|U||U_{-1}|} - \lambda\sqrt{|U||U_{+1}|}
  \end{split}
\end{equation*}

Claim. We can assume that E. 
Proof: Suppose that W then clearly Q, so we could choose U to be Y. Continuing stepping deeper till we have that B. Simiraly, if A, we could take steps upward by replacing E with T. At the end of the process, we will be left with U such  that Q and L.
