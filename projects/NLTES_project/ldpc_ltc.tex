...
\paragraph{Proof:} Define the $T$ precisely as is in the proof of Theorem 1, And assume, without loss of generality $\delta_{-} < \delta_{+}$. Hence it is clear that for any $v \in V^\prime$ it holdes that $w_{E^\prime}\left( c_{v} \right) > \frac{1}{2}\delta_{-}\Delta$, so we could repeat exactly the above steps to get that the $T$ has linear size and the inquality $|x| > \left( \delta_{-} - \frac{|U_{\max}|}{n} - \frac{\lambda}{\Delta} \right) $ still holds, So we can assume that $|U_{\max}|$ is also linear at $n$. Furthermore, if $U_{\max} \subset V_{-}$ then the inequality $\left( \delta_{+} - \frac{2}{3} - \frac{2}{\Delta}\lambda \right) \Delta |U_{\max}|$ also holds and the exactly arguments as before prove the Theoroem.     

Therefore consider the case when $U_{\max} \subset V_{+}$. Denote by the $U_{-1}$ and $U_{+1}$ the previews and the next layers to $U_{\max}$.  

Suppose that for every negative layer $U \subset V_{-}$ ethier $|U_{-1}| \ge \frac{1}{8}\Delta|U|$ or $|U_{+1}| \ge \frac{1}{8}\Delta|U|$. Then its clear that $|U_{\max}| \ge \frac{1}{8}\Delta|U_{\max, \pm 1}|$. And therefore:  
\begin{equation*}
  \begin{split}
    w_{E/E^{\prime}}  \left( x|_{U} \right)  & \ge \Delta\delta_{-}|U| - 8^{2}\frac{|U|^{2}}{\Delta n} - 2 \lambda \cdot \frac{8}{ \sqrt{\Delta} } |U| \\
    & \ge \left( \delta_{-} - \frac{1}{2}\left( \frac{8}{\Delta} \right)^{2} - 16\lambda\frac{1}{\Delta^{\frac{3}{2}}} \right) \Delta |U|
  \end{split}
\end{equation*}
So, assume the oppesite, namely that there is a negative layer that both of it's neighbors layers satisfies: $|U_{\pm 1}| \le \frac{1}{8}\Delta|U|$ 
\begin{equation*}
  \begin{split}
    w_{E/E^{\prime}} & \left( x|_{U} \right)  \ge \delta_{+}\Delta |U|\\
    &  -w\left( E( U_{-1} ,U)  \right) -  w \left( E( U , U_{+1}) \right) \ge  \\
    &  \delta_{+}\Delta |U| -  \frac{\Delta}{n}|U|\left( |U_{-1}| + |U_{+1}| \right) -  2 \sqrt{\frac{1}{8}} \sqrt{\Delta}\lambda |U|  \ge \\ 
   &  \left( \delta_{+} -\frac{1}{2} - 2 \cdot \sqrt{\frac{1}{8}}\frac{\lambda}{\sqrt{\Delta}}   \right)\Delta|U|
  \end{split}
\end{equation*}
