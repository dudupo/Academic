...
\paragraph{Definition.} Consider a code $C$  a string $x$, and denote by $\xi\left( x \right)$ the fraction of the checks in which $x$ fails. $C$ will be called a local-te, stability $f\left( n \right)$ If there exsits $\kappa > 0$ such that 
\begin{equation*}
  \begin{split}
    d\left( x, C \right) / n \le \kappa \cdot  \xi\left( x \right) f\left( n \right)
  \end{split}
\end{equation*}
