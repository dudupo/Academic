...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 %  \end{equation*}
  ...
  \paragraph{Lemma.} Let $J$ be defined as above such that both \emph{Cayly}$\left( \Gamma, J \right)$, \emph{Cayly}$\left( \Gamma, J^{\prime} \right)$ are expanders with algebric expansion greater then $\lambda$ and $C_0$ with the same paramters above. Then for $\rho_{0} > 1 - \frac{1}{2q}$ the code $\mathcal{T}\left(J, q ,C_{0}\right)$ is a good code. 
  \paragraph{Proof.} We have all ready prove that the code has positive rate. Consider a codeword $x$.    
