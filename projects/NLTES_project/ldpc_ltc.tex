...
\paragraph{Lemma}. Let $U$ be a layer at height $l$. The number of vertices in $U$ is at least F.

\paragraph{Proof:} We will think of $T$ as the tree obtained by purning a tree, with degree greater than $\frac{1}{2}\delta_{0}\Delta$, at height $l$, namely any vertex at height $i \le l $ with two parents can be counted as subtree with height $l-i$ that has pruned. Then the problem is equivalence to bounding from above the number of pruned vertices at height $l$. Define $X_{i}$ to be the number of pruned subtrees whose root is at height $i$. Denote $ \omega = \frac{1}{2}\delta_{0}\Delta$, Then consider the next maximization problem: 
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  \begin{split}
    |U| \ge \left( \frac{1}{2}\delta_0\Delta \right)^{l}- \frac{\left( \frac{\Delta^{2}}{\omega} \right)\Delta^{2l-g} - \omega^{l-g/2} }{\frac{\Delta^{2}}{\omega}-1}
  \end{split}
\end{equation*}$\square$
