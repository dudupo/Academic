...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 %  \end{equation*}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Define the code $\mathcal{T}\left(J, q ,C_{0}\right)$ to be the subspace such that any vertex's local view over the edges defined by $\Jvv \cup J^{\prime}$ is a codeword of $C_{0}$. In addition, let's associate for any vertex a code $\Cvv$ obtaind by ignoring the bits supported on the $\Jvv$ coordinates. Noitce that code defined by requiring that the local view of any vertex $v$ of \emph{Cayly}$\left( J^{\prime} \right)$ is a codeword of $\Cvv$ is a TannerCode. Denote it by $ \tilde{\mathcal{T}}\left(J, q ,C_{0}\right)$.   

  \paragraph{Lemma.} Let $G$ be an $\left( \Delta,\lambda \right)$ expander graph, and $q < 1$ such that the code $\mathcal{T}$ 
