...
\paragraph{Definition.}Let $L = \{L_{i}\}^{2|E|}_{0}$  be a series of $2|E|$. Such that each element $L_{i} \in C_{0}$. We will call $L$ a \textit{Potential list} and refer to the $e_{v}$'th element of $L$ as a suggestion made by the vertex $v \in V$ for the edge $e \in E$. Notice that $\sum_{i}{L_{i}} \in C_{\oplus}$.
\paragraph{Definition} Let $L$ be a Potential list, define the Force of $L$ to be the following sum $  F\left( L \right) = \sum_{e = \{v,u\} \in E }{ \left(L_{e_v} + L_{e_u}\right) } \in \mathbb{F}_{2}^{|E|} $
\paragraph{}
