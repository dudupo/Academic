...
\begin{equation*}
  \begin{split}
    w_{E/E^{\prime}} & \left( x|_{U_{-1}\cup U_{+1}} \right)  \ge \delta_{+}\Delta\left(|U_{-1}|+ \Delta|U_{+1}|\right)  \\
    &  -w\left( E( U_{-2} ,U_{-1})  \right) -  w \left( E( U_{-1} , U_{\max}) \right) \\
    &  -w\left( E( U_{\max}, U_{+1})  \right)  - w \left( E( U_{+1}, U_{+2} ) \right)  \ge \\
    &  \delta_{+}\Delta\left(|U_{-1}|+ \Delta|U_{+1}|\right) - \\
    &   \frac{\Delta}{n}|U_{\max}|\left(|U_{-2}| + |U_{-1}| + |U_{+1}|+|U_{+2}| \right) - \\
    &  4 \lambda |U_{\max}|   
  \end{split}
\end{equation*}
\paragraph{Claim.}  
