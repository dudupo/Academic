...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 %  \end{equation*}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Hence, if we could enginer an expander familiy such that both the graphs obtiand by removing $\frac{1}{2}$ of the edegs connected for each vertex result also expnders, and regarding $\tilde{C_{0}}$ each edge is checked by both verticrs on it support. Namly a good Tanners Code could be defined on the restricted graphs then we have that any string that satesfied the orignal checks has also a linear weight. For achiving this property we are going to restrict ourselfs to particular familily of Cayly Graphs.  
  \paragraph{Definion. Testible-Tanner-Code.} Let $q > \frac{1}{2}$ and let $J$ be a geneator set for group $\Gamma$ such that $|J| = \Delta$, $q | \Delta $, $J$ closed for inverse, and there exist subset of $J$, denote it by, $J^{\prime}$ such that $J^{\prime}$ is a generator set of $\Gamma$ and $|J^{\prime}| = \frac{1}{2}\Delta$. Let $C_{0}$ be a code with parameters $C_{0} = q\Delta \left[1, \rho_{0}, \delta_{0}\right]$. For any vertex assoicate a subset $\Jvv \subset J/J^{\prime}$ at size: 
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Define the code $\mathcal{T}\left(J, q ,C_{0}\right)$ to be the subspace such that any vertex's local view over the edges defined by $\Jvv \cup J^{\prime}$ is a codeword of $C_{0}$. In addition, let's associate for any vertex a code $\Cvv$ obtaind by ignoring the bits supported on the $\Jvv$ coordinates. Noitce that code defined by requiring that the local view of any vertex $v$ of \emph{Cayly}$\left(\Gamma, J^{\prime} \right)$ is a codeword of $\Cvv$ is a TannerCode. Denote it by $ \tilde{\mathcal{T}}\left(J, q ,C_{0}\right)$.   

  \paragraph{Lemma.} Let $J$ be defined as above such that both \emph{Cayly}$\left( \Gamma, J \right)$, \emph{Cayly}$\left( \Gamma, J^{\prime} \right)$ are expanders with algebric expansion greater then $\lambda$ and $C_0$ with the parameters $\rho_{0} > 1 - \frac{1}{2q}$ and $ \delta_{0} - \left( q - \frac{1}{2} \right) > \lambda$. Then the code $\mathcal{T}\left(J, q ,C_{0}\right)$ is a good code. 
  \paragraph{Proof.} We have all ready prove that the code has positive rate. Consider a codeword $x$ and denote by $x^{\prime}$ the restriction of $x$ to \emph{Cayly }$\left( \Gamma, J^{\prime}  \right)$ which is a codeword of $\tilde{C} = \tilde{\mathcal{T}}\left(J, q ,C_{0}\right)$. But $\tilde{C}$ is a Tanner Code such that any vertrx see at least $ \tilde{\delta_{0}} \Delta := \left(\delta_{0} - \left( q - \frac{1}{2}   \right) \right)\Delta $ non trivial bits, Therefore as the expansion of the graph \emph{Cayly}$\left( \Gamma, J^{\prime} \right)$ is lower than $\tilde{\delta}\Delta$ than in any subset $S$ of $V$ at size at most $|S|/|V| < \tilde{\delta_{0}} - \lambda / \Delta $  there is at least one vertex that see less than  $\tilde{\delta}\Delta$ non trivial bits, In contrudiction for the fact that $x^{\prime} \in \tilde{C}$. $\square$ 

  \paragraph{Good Testaible Code.} \ctt{Need to look for such $G=$\emph{Cayly}$\left(\Gamma, J^{\prime}  \right)$. what about $G \cup gG$ for some $g\in \Gamma$?  }. 
