...
$ L \leftarrow \text{Array} \{ \} $\\
 ...
 \eIf{ $ |z| < \tau \frac{f\left( n \right)}{n} $}{
  \While{ $|z| > 0$ }{
...
\paragraph{Theorem.} Consider a Tanner Code $C$ and the disagreement code $C_{\oplus}$ defined by it. Suppose that for every codeword $ z \in C_{\oplus}$ in $C_{\oplus}$ such that $|z| < \tau n / f\left(n\right)$, there exists another codeword $y \in C_{\oplus} $ such that $|y| < |z|$ then:
