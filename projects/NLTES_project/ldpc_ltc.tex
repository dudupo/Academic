...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 %  \end{equation*}
  ...
  \begin{equation*}
    \begin{split}
     &  f^{\prime \top}f^{\prime}  = \sum_{v \in V}f^{\prime}\left( v \right)f^{\prime}\left( v \right) \\  = &  \sum_{v} f^{\top}\left( gv \right)f\left( gv \right) = f^{\top}f 
    \end{split}
  \end{equation*}

  \begin{equation*}
    \begin{split}
       = & \max_{f \perp \mathbf{1}} { \sum_{v}\sum_{u\sim_{G} v}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} 
    \end{split}
  \end{equation*}
