...
Consider a code $C$  a string $x$, and denote by $\xi\left( x \right)$ the fraction of the checks in which $x$ fails. $C$ will be called a local-te, stability $f\left( n \right)$ If there exsits $\kappa > 0$ such that $d\left( x, C \right) / n \le \xi\left( x \right) f\left( n \right)$
