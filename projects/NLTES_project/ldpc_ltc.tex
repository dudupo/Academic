...
%\end{adjustbox} 
  ...
  \paragraph{Proof.} Recall that the seconed largest eigen value 
...
%To overcome the vanishing rate issue we are going to consider graphs family in which the maximal layer of BFS scanning couldn't excced a linear  
  ...
  \ctt{ Consdier $T$ at height less than the graph diameter.  } 
  \subsection{Tanner Code Testability When With strongly-Honest Vertices. }
  The result obtained by the proof of Theorem 1 is that one cannot hide errors in a sublinear size tree. Namely, any codeword of the disagreement code with a weight less than $ \Omega \left( n^{1-\varepsilon} \right) $ could be reduced, regardless the parameters of the small code. 
  ...
  \paragraph{Proof:} Proof, Define $T$ again to be the DAG constructed as in the proof of Theorem 1 and denote by $U_{\max}$ the layer at maximal size. The analiyses of the flux over projeced by $x$ over the vertices which sugessted trivial code words remain the same. But now, as $U_{\max} \subset \text{supp}(x)$ follows that $|U_{\max}| \le 2|x| \le O(n^{1-\varepsilon})$. Therefore, $ \lim_{n \rightarrow \infty} \frac{|U_{\max}|}{n} \rightarrow 0 $ and we have that for sufficently large $n$ it holds that: 
  ...
  \subsection { Overcoming The Vanishing Rate \ctt{ Draft} } 
  Consider the follow code, instead of assoicating each edge with pair of checks, lets define the vertices to be the checks of small codes over $q \in [0,1]$ of the edges. That it, know each vetex define only $q\Delta$ restrictions. Hence, the rate of the code is at least:   
  \begin{equation*}
    \begin{split}
    ...
    \end{split}
  \end{equation*} for exmple if $q = 2/3$ then for having constant rate, it is enough to ensure that $ \rho_{0} \ge 1 - \frac{3}{4} = \frac{1}{4}$. In the otherhand the lowerbound of the flux the trival vertices remains up to multiplication by the fraction factor $q$: 
  \begin{equation*}
    \begin{split}
