...
\paragraph{Lemma. semi-Linearity Of The Disagreement.} Consdier the code $C = \Tann$. Let $ x \in \mathbb{F}_{2}^{E}$ then for any $ y \in C$ it holds that: 
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  \begin{split}
    \left( x + y \right)^{\opplus} =& \sum_{v\in V}{c_{v}} = \sum_{v \in V}{y|_{v}} + \sum_{v\in V}{ \arg_{ \tilde{c} \in C_{0}} \min{ d( x|_{v} , \tilde{c} ) } } \\ 
