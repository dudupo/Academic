...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 %  \end{equation*}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Yet, We still have require more for proving a linear distance. Noitce that by reapting on the \emph{Singleton Bound} proof it followes that the small code $\tilde{C_{0}}$ obtained by ignoring the first $ \left( q - \frac{1}{2} \right) \Delta $ coordinates yield a code with distance: 
  \begin{equation*}
    \begin{split}
  ...
  \paragraph{$qp-G$} Let $S$ be a geneator set for group $\Gamma$ such that $|S| = \Delta$, $p,q | \Delta $ and $S$ closed for inverse.  
