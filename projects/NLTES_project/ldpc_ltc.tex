...
%To overcome the vanishing rate issue, we are going to consider the graphs family in which the maximal layer of BFS scanning couldn't exceed a linear  
 ...
 So assume that we could engineer an expander family such that the graphs obtained by removing $\frac{1}{2}$ of the edges connected for each vertex result also expanders, and in addition, regarding $\tilde{C_{0}}$ each edge is checked by both vertices on its support. Namely, a good Tanners Code could be defined on the restricted graphs; Then, any string that satisfies the original checks also has a linear weight. To achieve this property, we will restrict ourselves to a particular family of Cayley Graphs.  
  ...
  \paragraph{Claim. Existancess of such \em{Cayley's}.} Let $S$ be a generator set such that \em{Cayley}$\left( \Gamma , S \right)$ has a second largest eigen value greater then $\lambda$, And consider an arbitray group element $g \in \Gamma$. Denote by $S_{g}$ the set $gSg^{-1}$. Then the senond eigen value of the graph obtaind by $\left( \Gamma , S \right) \cup \left( \Gamma , S \right)$ is at most $2\lambda$. 
  \paragraph{Proof.} Let $A$ be the adjacency matrix of $\Delta$ regular graph, then the seconed eignenvalue is:


  \begin{equation*}
    \begin{split}
      \max_{f \perp \mathbf{1}} { \frac{f^{\top}A_{gSg^{-1}} f  }{ f^{\top}f}} = \max_{f \perp \mathbf{1}} { \sum_{u\sim v}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} \\
      \max_{f \perp \mathbf{1}} { \sum_{v , gSg^{-1}v}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} = \max_{f \perp \mathbf{1}} { \sum_{v , gSg^{-1}v}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} =\\  
      = \max_{f \perp \mathbf{1}} { \sum_{gv}\sum_{gSg^{-1}gv}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} \\  =\max_{f \perp \mathbf{1}} { \sum_{gv}\sum_{gSg^{-1}gv}\frac{f\left( gu \right) f \left( gv \right)  }{ f^{\top}f}} =\\
      = \max_{f \perp \mathbf{1}} { \sum_{gv}\sum_{gSg^{-1}gv}\frac{f\left( u \right) f \left( v \right)  }{ f^{\top}f}} 
    \end{split}
  \end{equation*}
