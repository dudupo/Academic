...
Suppose that for every negative layer $U \subset V_{-}$ ethier $|U_{-1}| \ge 8\Delta|U|$ or $|U_{+1}| \ge 8\Delta|U|$. Then its clear that $|U_{\max}| \ge 8\Delta|U_{\max, \pm 1}|$. And therefore:  
\begin{equation*}
  \begin{split}
  ...
  \end{split}
\end{equation*}
So, assume the oppesite, namely that there is a negative layer that both of it's neighbors layers satisfies: $|U_{\pm 1}| \le 8\Delta|U|$ 
\begin{equation*}
  \begin{split}
    w_{E/E^{\prime}} & \left( x|_{U} \right)  \ge \delta_{+}\Delta |U|\\
    &  -w\left( E( U_{-1} ,U)  \right) -  w \left( E( U , U_{+1}) \right) \ge  \\
    &  \delta_{+}\Delta |U| -  \frac{\Delta}{n}|U|\left( |U_{-1}| + |U_{+1}| \right) -  2 \sqrt{8} \sqrt{\Delta}\lambda |U|  \ge \\ 
   &  \left( \delta_{+} -\frac{2}{3} - 2 \cdot \sqrt{8}\frac{\lambda}{\sqrt{\Delta}}   \right)\Delta|U|
  \end{split}
\end{equation*}
