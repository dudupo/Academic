...
\begin{multicols*}{2}
  ...
  \end{equation*}
   Then define the codes:
	...
	the small codes $C_{A_{j}}$, $\delta \Delta$ satisfy the equation $ \left(\Delta r\right)^{4}\left(1 - \delta\right) < \frac{1}{2}\delta^{3} $ and also the code $ C_{A_{1}}  $ 
	...
	\end{equation*}
      ...
      \end{equation*}
      \paragraph{Proof.} First notice that $y_{1a}y_{2b}z_{2c} $ is a valid codeword of $\Aabc$. That because that the projection obtained bt 
  ...
  \end{equation*}
	...
	\paragraph{Dense Normal Net Countiong} Let us call the normal vertices the vertices with degree less then $\xi$ in $\Guq = \Gamma_{\square, 1}^{x} \cup \Gamma_{\square, 2}^{x}$. 
	...
	& |T| \le \left(\frac{\lambda^{\star}}{\eta - \frac{6|x|}{n \delta}}\right)^{2}|S|
      \end{split}
    \end{equation*}
    Denote by $S_{e}$ the set of vertices in $\Guq$ with degree greater then $\xi$. Then by reapting on the above calculation, while subsuting $\Gamma_{i}$ by $\Gamma_{i, \square}$, We obtain that there is $\lambda^{\star}_{2}$ such that:
     \begin{equation*}
      \begin{split}
 ...
 $ \Rightarrow \tau \ge \frac{\rho}{2\left( 3 - \rho \right)}\ge \rho/3 $. Namely, at least $\rho/3$ of vertices of $T$ are indcident to at least $\frac{1}{2}\Delta\rho$ heavy edges. 

 Since $\Gu$ is $3\Delta$ regular we get that $|S| - |S_{e}| \le 3\Delta |T| $. In the otherhand we have shown that 
 \begin{equation*}
   \begin{split}
     |S_{e}| & \le \left(\frac{\lambda^{\star}_{2}}{\xi - \left(2\Delta^{2} - \Delta  \right)\frac{|x|}{n \delta\Delta}}\right)^{2}|S| \\ &
 ...
 \begin{equation*}
  ...
  \end{split}
 \end{equation*}
 Hence at most an $\frac{1}{6}\rho $ propartion of vertices of $T$ are adjacent to more than $\frac{6}{\rho} \left( 9\Delta^{2} + \lambda^{\star}  \right) $ vertices of $S_{e}$, And at least $\frac{5}{6}\rho$ propartion of $T$ are adjacent to less then $\frac{6}{\rho} \left( 9\Delta^{2} + \lambda^{\star}  \right) $ . 
 And therefore we have that at least $\frac{1}{6}\rho$ vertices are:
 \begin{enumerate}
   \item Indceidnt to at least $\frac{1}{2}\Delta\rho$ heavy edges. 
   \item Adjacent to at most $ \frac{6}{\rho} \left( 9\Delta^{2} + \lambda^{\star}  \right)$ vertices of $S_{e}$.  
 \end{enumerate}
 \paragraph{Proof Of Theorem 1} Let us call to the set of verices satisfy the constarints above \textbf{good vertices}. Pick any good vertex $g \in T$.
 Remmeber that each heavy edge between anormal vertex of $S$ and a vertex of $T$ corresponds to either a row or a column shared by the two local views.
 

 By $w$-robustness, for any small enougth $\xi \le w $, the local view of any normal vertex is supported on at most $\frac{\xi}{\delta\Delta}$ rows and columns. 
 Hence, the row (or column) shared between the normal vertex and $v$ is at distance at most $\frac{\xi}{\delta\Delta}$ from a nonzero codewoed of $\Aa$ (or $\Ab$, $\Ac$).


 Let us denote by $x_{v^{\prime}}$ the the local view obtained by taking only the rows and columns that shared bewteen $v$ and notrmal vertics. The $\gamma$-resistance to puncturing propperty implies that if we could find $ \eta, \xi  $ such that for any $ |x| \le d $ we have:
 \begin{equation*}
   \begin{split}
     &  \frac{6}{\rho} \left( 9\Delta^{2} + \lambda^{\star}  \right) \le \gamma \ \ \ \ \ \left( \Theta\left(  \Delta^{2\frac{1}{2}} \right) \right)
   \end{split}
 \end{equation*}
 Then the local view of $v$ is at distance at most:
 \begin{equation*}
   \begin{split}
     d\left(x_{v}, \Aabc\right) &\le d\left(x_{v^{\prime}, \cdot }\right) + |\text{ ignored bits }|\\
     & \le  d\left(x_{v^{\prime}, \cdot }\right) +  \color{red}\frac{3}{2} \color{black} \Delta \cdot \frac{6}{\rho} \left( 9\Delta^{2} + \lambda^{\star}  \right) 
   \end{split}
 \end{equation*}
\end{multicols*}
  % \printbibliography 
