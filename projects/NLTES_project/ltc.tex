...
\begin{multicols*}{2}
  ...
  \end{equation*}
    ...
    \paragraph{Dense Normal Net Countiong} Let us call the normal vertices the vertices with degree less then $\xi$ in $\Gu = \Gamma_{1}^{x} \cup \Gamma_{2}^{x}$. 
    And Let us say that that an edge of $\Gu$ is heavy if it is incident to at least $\eta$ squares in $\Gsa$ and $\Gsb$. Let $ T$ be set of vertices in $V_0$ that are connected to (at least) one normal vertex. 

    %$ |S| \le \frac{2|x|}{ \delta\Delta }$ 
    \begin{equation*}
      \begin{split}
	|E\left( S,T \right)| & \ge \eta \\ 
