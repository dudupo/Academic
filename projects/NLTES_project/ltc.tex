  ...
  \end{equation*}
   Then define the codes:
	...
	\begin{split}
	  ...
	  \left( \sum_{ij} { \alpha_{ij} y_{1}^{i}y_{2}^{j  \top}}  \right)\left(   \sum_{ij} { \beta_{ij} z_{1}^{i}z_{2}^{j  \top}} \right)\\ 
	  &= \sum_{ijlk} { \alpha_{ij}\beta_{lk} y_{1}^{i}y_{2}^{j \top}} z_{1}^{l}z_{2}^{k \top} 
	\end{split}
      \end{equation*}
