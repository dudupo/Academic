...
\begin{multicols*}{2}
  ...
  \end{equation*}
	...
	& |T| \le \left(\frac{\lambda^{\star}}{\eta - \frac{6|x|}{n \delta}}\right)^{2}|S|
      \end{split}
    \end{equation*}
    Denote by $S_e$ the set of vertices in $\Guq$ with degree greater then $\zi$. Then by reapting on the above calculation, while subsuting $\Gamma_{i}$ by $\Gamma_{i, \square}$, We obtain that there is $\lambda^{\star}_{2}$ such that:
     \begin{equation*}
      \begin{split}
	& |S_{e}| \le \left(\frac{\lambda^{\star}}{\zi - \left(2\Delta^{2} - \Delta  \right)<++>\frac{|x|}{n \delta}}\right)^{2}|S|
      \end{split}
    \end{equation*}
