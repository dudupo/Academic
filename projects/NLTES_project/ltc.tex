...
\begin{multicols*}{2}
  ...
  \end{equation*}
   Then define the codes:
	...
	the small codes $C_{A_{j}}$, $\delta \Delta$ satisfay the eqution $ r^{2}\left(1 - \delta\right) < \frac{1}{2}\delta^{3} $ and also the code $ C_{A_{1}}  $ 
	...
	\end{split}
      ...
      \begin{equation*}
	d\left(w, \Aabc \right) \le \left( r\Delta \right)^{4}\left( 1 - \delta \right)\Delta^{3}
      \end{equation*}

      Consider the local
      \begin{equation*}
	\begin{split}
	  x_{abc} &= y_{ab}z_{ac} = \left( \YY + \xi_{y} \right)_{ab}   \left( \ZZ + \xi_{z} \right)_{ac} \\ 
	  ...
	  & \Rightarrow d\left( \left( y_{1}y_{2}^{\top}\right)_{ab}\left(z_{1}z_{2}^{\top} \right)_{ac} , \Aabc \right) \le \left( \Delta r \right)^{4} \left( 1-\delta \right)\Delta^{3} \\ & 
