  ...
  \end{equation*}
   Then define the codes:
	...
	\end{equation*}
      ...
      \paragraph{Claim.} Assume that $ \YY $ and $ \ZZ $ are in the  bases defined above. Let $\tau \in \mathbb{F}_{2}^{A\times B\times C} $ such that $ \tau_{abc} = \left( \YY \right)_{ab}\left( \ZZ \right)_{ac} $ then $d\left( \tau, \Aabc \right) \le \left( 1-\delta \right)\Delta^{3} $.  
      Consider the local
      \begin{equation*}
	\begin{split}
