...
\newcommand{ \Hw }{ \delta\Delta -\Delta^{\frac{1}{2}-\varepsilon}/\delta  }
  ...
  \end{equation*}
   Then define the codes:
	...
	\end{equation*}
	%\newcommand{}<++>
	\paragraph{Definition} Define the mapping (not linear)  $\phi : \mathcal{T}\left(\Gsa \cup \Gsb , \mathbb{F}_2  \right) \rightarrow  \mathcal{T}\left( \Gamma_{squre \square}, \mathbb{F}_{2} \right)  $ as the summtion over the fowlloing local maps $\phi_{g}$. which for given vertex $g \in  \Gamma_{\square \square}$    
