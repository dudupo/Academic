...
Then in next section we review all the techniques that we have used to improve a  local gates assignment. Namely, this section deals with the question of how presenting each of the local term as subgate. 
  ...
  \paragraph{Simulation in nutshell.} Let's starting with presanting notation, sticking to the standart defintions, qauntum state $\ket{\psi}$ is a vector in an hilbert state, and Hamiltonian is a hermithan operator act on that space. As physical foundemntals are not the main focous of the paper, we will just mention without giving a justification that a quantum sate at time $t$ in the presence of Hamilitonian $H$ is given by the relation $\ket{\psi\left( t\right)} = e^{-iHt}\ket{\psi\left( 0 \right)}$. Noitce that by the fact that $H$ is hermitan operator $e^{-iHt}$ is a unitary operator.   

  For example, suppoe that $\ket{\psi} = \ket{00}$ and $H = Z\otimes Z $. Thus the time evolution operator is $e^{itZ\otimes Z}$. As $\ket{00}$ is an eigen state for $H$, cause $Z\otimes Z \ket{00} = \ket{00}$ we have that $\ket{psi\left( t \right)} = e^{-it}\ket{00}$. But if $\ket{\psi} = \ket{00} + \ket{10}$ (we ignore normolazition factors). then we would have that $\ket{\psi(t)} = e^{-it}\ket{00} + e^{it}\ket{10}$.    


  \paragraph{Naive approach.}      Before diving into the technical details let's review our competitor first. Consider the straightforward assignment Lie-Trotter \cite{Lie-Trotter} just handle each of Hamiltonian terms one by one. Assume that the hamming weight of the support of \(H_i\) equal to \(d_i\). so we pay two steps to rotate each wire into the parity base and uncompute it in the end. then we could apply the \(CX\) from each qubit in the support to a chosen one which will sum the wires parity and finally will propagate the sum trough the \(RZ\left(\theta \Delta t \right) \) gate (rotation by the coefficient of the term and the step size). So in total we pay \((2 + 2d_i\) for each term and therefore the whole circuit will require: 
