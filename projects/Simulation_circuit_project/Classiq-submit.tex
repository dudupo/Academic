...
%\paragraph{Simulation in nutshell.} 
  ...
  \paragraph{Naive approach.} As physical foundemntals are not the main focous of the paper, we will just mention without giving a justification that a quantum sate at time $t$ in the presence of Hamilitonian $H$ is given by the relation $\ket{\psi\left( t\right)} = e^{iHt}\ket{\psi\left( 0 \right)}$.     Before diving into the technical details let's review our competitor first. Consider the straightforward assignment Lie-Trotter \cite{Lie-Trotter} just handle each of Hamiltonian terms one by one. Assume that the hamming weight of the support of \(H_i\) equal to \(d_i\). so we pay two steps to rotate each wire into the parity base and uncompute it in the end. then we could apply the \(CX\) from each qubit in the support to a chosen one which will sum the wires parity and finally will propagate the sum trough the \(RZ\left(\theta \Delta t \right) \) gate (rotation by the coefficient of the term and the step size). So in total we pay \((2 + 2d_i\) for each term and therefore the whole circuit will require: 
