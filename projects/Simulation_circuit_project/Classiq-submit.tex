...
Using the Lie-Trotter formula \cite{Lie-Trotter} $e^{A+B} = \lim_{n \rightarrow \infty} (e^{A/n}e^{B/n})^n$ for arbitrary matrices $A,B$, we obtain that one can apply the gates $e^{\Delta t H_{i}}$ for $1/(\Delta t)$ iterations and expect to converge to the desired product. This insight defines a circuit which we will refer to as the naive approach.
