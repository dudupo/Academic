...
\paragraph{Proof.} By induction over \(w(v,u)\). For the base case consider \(u,v\) such \(u\) differ from \(v\) only by single coordinate. Denote it by \(j\). Observes that if $\textbf{Ham}(u) <\textbf{Ham}(v)$ then $\textbf{SWAP}^{v,u}$ is just the indentity. So we can assume that $ \textbf{Ham}(u) = \textbf{Ham}(v)+1 $. 

First case \(v_j = 0, u_j =1\), denote by \(j^{\prime}\) the wire which \(SWAP^{v}\) sends \(j\) into. then it's sufficient to apply the gate \(\textbf{SWAP}(\text{Ham}(v) +1, j^\prime) \). Second case \(v_j = 1, u_j =0\), Note that \(\left(\textbf{SWAP}^{v,u}\right)^{\dagger}=\textbf{SWAP}^{u,v}\) and in our case the gates are involve only Clifford gates and therefore the inverse gate has the same length has it's origin. In other hand the inverse gate matches to the first case above.  
