...
%  \ \textbf{Ham}(v,u^\prime)\cdot\text{Depth}\left(\textbf{SWAP}\right)  + 
	...
	\(T  \leftarrow\) MST(\(G\)). \\
	\(P \leftarrow \) ordering of the vertices \( \braket{v_{1} , .. v_{l}}\) \\ 
	\ according to the \textbf{DFS} scanning of \(T\). \\
	\ \\ 
	\For{ \(v \in P\) }{
	...
	\end{equation*}
      \end{lemma}
      \begin{proof} Define \(Y : \Omega \rightarrow \mathbb{R} \) to be the random  variable which is the hist of the \(X\) by \(\mu\) i.e \(Y = X -\mu\). From the linearity of expectation we get that \(\mathbb{E}[X] = \mu + \mathbb{E}[Y]\). let's note by \(Y^{s}\) and \(Y^{as}\) the symmetric and asymmetric parts of \(Y\). the \(\square\). 
