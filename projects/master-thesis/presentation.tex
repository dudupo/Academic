...
\title[From calassical to quantum good LDPC codes.] % (optional, only for long titles)
...
\uncover<8->{
  \draw (8.5,3) node { $\rightarrow$ };
  \draw (7,3.8) node { NLTS };
  \draw (7,3.3) node { Hamiltonians };
  \draw (7,2.8) node { from good};
  \draw (7,2.3) node { qLDPC codes \cite{anshu2022nlts}};
  \draw (7,6) node { 22-23 };
}
\uncover<9->{
