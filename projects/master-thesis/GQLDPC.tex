...
\begin{theorem}
  \label{theorem:maint}
  For $\varepsilon \in \left( 0,\frac{1}{2} \right)$, $\delta_{0}> 0$, large enough $\Delta$, and small codes $C_{A},C_{B}$ with distance at least $\delta_{0}\Delta$ if the dual tensor code of $C_{A},C_{B}$ is $\Delta^{1\frac{1}{2}}+\varepsilon$-robust then there exists an infinite family of square complexes for which the Tanner code $ \mathcal{T}\left(G^{+}, \left(C_{A}^\perp\otimes C_{B}^{\perp}\right)^{\perp} \right)$  defined by the complexes and the dual tensor code such that for any codeword $c$ with weigh less than $ \frac{\delta_{0}}{4\Delta^{3/2 + \varepsilon}} \cdot n \Delta^{2} $ there exist a negative vertex in $v \in V^{-}$ and code word in $ y \in  C_{A}\otimes C_{B}$ supported only on the squers adjoint to $v$ such that $|c + y| < |c|$.
\end{theorem}

Observe that $y \in C_{Z}^{\perp} \subset C_{X}$, so $c + y \in C_{X}$ and $|c + y| < |c| < \frac{\delta_{0}}{4\Delta^{3/2 + \varepsilon}} \cdot n \Delta^{2}$. Therefore, by~\cref{theorem:maint}, there is another $y_{1} \in C_{Z}^{\perp}$ such that $|c + y + y_{1}| < |c + y| < |c|$. Repeating this process enough times yields a series of $y, y_{1}, y_{2}, \dots, y_{l}$, all of them in $C_{Z}^{\perp}$, such that $|c + y + \dots + y_{l}| = 0 \Rightarrow c = y + y_{1} + y_{2} + \dots + y_{l} \Rightarrow c \in C_{Z}^{\perp}$.
