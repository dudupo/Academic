...
\begin{definition}[Quantum Tanner Code.] Let $\Gamma$ be a group on $n$ verices. And let $A,B$ be a two generator set of $\Gamma$. Define the left right Cayley complex to be the graph $G = \left( \Gamma, E \right) $ such that $\{u,v\} \in E$ only if there are $a\in A$ and $b \in B$ such that $u = avb$. 
\end{definition}
