...
\begin{theorem} 
Fix $\rho_{A},\rho_{B} \in (0,1)$, and let $\kappa$ be:
\begin{equation*}
  \begin{split}
  ...
  \end{split}
\end{equation*}
Where $H^{-1}_{2}$ denote the inverse of the binary entropy function. Let $C_{A},C_{B}$ be an $\Delta$ length and $\rho_{A}\Delta$,$\rho_{A}\Delta$ codiminsion codes sampled a uniformly random. Then with high probability as $\Delta\rightarrow\infty$ for any codeword of their dual tensor code $c \in \duC$ there are  can decomposed into a sum of $c = t + s$ such that $t \in C_A \otimes \mathbb{F}^{B}$ and $s \in \mathbb{F}^A \otimes C_B$, where $s$ and $t$ are each supported on at most $\frac{c}{\kappa \Delta}$ rows and columns.
\end{theorem}
