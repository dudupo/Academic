...
\begin{definition}[Quantum Tanner Code.] Let $\Gamma$ be a group on $n$ verices. And let $A,B$ be a two generator set of $\Gamma$. Define the left right Cayley complex to be the graph $G = \left( \Gamma, E \right) $ such that $\{u,v\} \in E$ only if there are $a\in A$ and $b \in B$ such that $u = avb$. We can assume that $G$ is a bipartite graph (otherise just take $\Gamma^{\prime} = \Gamma \times \mathbb{Z}_{2}$ and define the product to be $a\left( u,\pm \right) = \left( au, \mp \right)$). 

  Now divide the graph into postivie and negative vertices according their coloring $V^{-}$ and $V^{+}$. And define the positive graph to be $G^{+} = \left( V^{+}, E \right)$ and by $G^{-} = \left( V^{-}, E \right)$.
\end{definition}
