...
\begin{equation*}
  \begin{split}
    H_{X} &= \left[ H \otimes I_{r} \ \  I_{n} \otimes H^{\top} \right] \\ 
    H_{Z} &= \left[ I_{r} \otimes H^{\top} \ \  H \otimes I_{n} \right] \\ 
  \end{split}
  \label{equ:css}
\end{equation*}
The matrices are orthogonal to each other as $H_{X}H_{Z}^{\top} = H\otimes H^{\top} + H\otimes H^{\top} = 0$ and therefore the pair define a valid CSS code. 
\end{definition}
