...
\begin{proof}
  ...
  \begin{claim}
    Denote by $H^{\otimes n}$ the Hadamard gate over $n$ qubits. Then for any code $C$ it holds that:
    
    \begin{equation*}
      \begin{split}
        H^{n}\ket{C^{\perp}} = \ket{C} 
      \end{split}
    \end{equation*}
  \end{claim}
  \begin{proof}
    \begin{equation*}
      H^{n}\ket{C^{\perp}} = \GZZZ{ H^{n}\ket{z} } =  \GZZZ{ \sum_{y\in \mathbb{F}_{2}^{n}}{ (-1)^{\braket{z,y} \ket{y} } } }
     \end{equation*}
  \end{proof}
  Let $P = X^{f}Z^{e}$ be an error such that $e, f < d/2$ act on the state $\ket{\mathbf{x}}$. Denote by $H_{X}, H_{Z}$ the parity check matrices of $C_{X},C_{Z}$. 
    ...
    \begin{split}
      ...
      & \mapsto^{H^{\otimes n}} \GZZZ{  Z^{e}H^{\otimes n}\ket{ x + z} } \\
      & = \GZZZ{  Z^{e}H^{\otimes n} X^{x} \ket{ z} } \\
      & = \GZZZ{  Z^{e} Z^{x} H^{\otimes n} \ket{ z} } \\
      & = \GZZZW{ \ket{ z} }{Z^{e} Z^{x} H^{\otimes n} } \\
      & =  \GXXXW{  \ket{ z} }{ Z^{e} Z^{x} } \\
    \end{split}
  \end{equation*}
\end{proof}
