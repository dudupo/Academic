...
Another way to look on the  toric code is as a prodcut of two cycles. If the checks in the repetition code are of the form $H_i =  x_i + x_{i+1}$  then in the toros code the checks are $x_{i,j}+ x_{i, j+1} + x_{i+1, j} + x_{i+1,j+1}$.
