...
\begin{claim}
  Let $k_{1},k_{2},k_{1}^{\top},k_{2}^{\top}$ be the demission of the codes with the full rank  parity check matrices $H_{1},H_{2},H_{1}^{\top},H_{2}^{\top}$. Then the dimension of the Hyperproduct code is $k_{1}k_{2} + k_{1}^{\top}k_{2}^{\top}$. 
\end{claim}

\begin{proof}
  We will find the dimensions of each of the classics codes defined by $H_{X}$ and $H_{Z}$. Notice that length of the code, assuming the fullness of the ranks, is $n \cdot  (n-k_{1}) + n \cdot k_{2}$ be the rank of the.  
\end{proof}
