...
\begin{proof}
  ...
  \begin{equation*}
    \begin{split}
      \ket{\mathbf{x}} &  \mapsto^{P} =  \frac{1}{\sqrt{|C_{Z}^{\perp}|}} \sum_{z \in C_{Z}^{\perp}}{ X^{f}Z^{e}\ket{ x + z }} = \frac{1}{\sqrt{|C_{Z}^{\perp}|}} \sum_{z \in C_{Z}^{\perp}}{ (-1)^{\braket{ e, x+z }}\ket{ x + z + f}}\\
      & \mapsto^{H_{X}} \GZZZ{ (-1)^{\braket{ e, x+z }}\ket{ x + z + f} \ket{ H_{X} \left(x + z + f\right) }  } \\ 
      & = \ \ \ \ \ \GZZZ{ (-1)^{\braket{ e, x+z }}\ket{ x + z + f} \ket{ H_{X}f }  }\\
      & \mapsto  \ \ \ \ \ \GZZZ{ (-1)^{\braket{ e, x+z }}\ket{ x + z} \ket{ H_{X}f }  }\\
      & \mapsto^{H^{\otimes n}} \GZZZ{  (-1)^{\braket{ e, x+z }}H^{\otimes n}\ket{ x + z} } \\
      & \mapsto
    \end{split}
  \end{equation*}
\end{proof}
