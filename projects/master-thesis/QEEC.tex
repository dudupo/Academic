...
\begin{proof}
  ...
  \begin{proof}
    Assume without loose of genrality that $v_{i-1} = \left( x, y -1 \right)$ and $v_{i+1} =\left( x,y+1 \right)$. Now denote by $f$ the projection $f : Z_{l} \times Z_{l} \rightarrow Z_{l}$ on the second cordinate, $f(a,b)= b$. On $P$ we have that $|f(v_{j+1}) - f(v_{j})| \le 1$. By extanding $f$ to be continus function on the real, and associate the $v_{j+1},v_{j+2},v_{j+3}..v_{k-1},v_{k},v_{0}..v_{j-2}v_{j-1}$ with continus segment. We obtain trough intermediate value theorem that there is a $\tau$ such that $f(\tau) = x$ as the values of $f$ on the vertices are must to be integer we obtain the exsitness of desired $u$.   
  \end{proof}
