...
\begin{proof}
  Let $x,y \in \FF$. for a random $\xi$ with probability $\varepsilon$ ecah of the following equations satisfied: 
  \begin{equation*}
    \begin{split}
      f(x) & = \frac{1}{d}\sum_{t}{ f\left( x + w^{t}\xi \right)  } \\
      f\left( x + w^{t}\xi \right)   & = \frac{1}{d} \sum_{t^{\prime}}{ f\left( x + w^{t}\xi + w^{t^{\prime}}y \right)  } \\
      f(x + w^{t^\prime}y) & = \frac{1}{d}\sum_{t}{ f\left( x + w^{t^\prime} + w^{t}\xi \right)  } 
    \end{split}
  \end{equation*}<++>
\end{proof}
