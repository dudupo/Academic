...
\caption{ Usage of  Ideal $(\beta,\gamma)$-Memory to obtain fault tolerance computation. }
  ...
  \begin{enumerate}
    \item We start by intializing many encoded magic states via the state preparation scheme and encoded zeros, since the block is at length $\log^{\alpha}(n)$ the depth reuired to that stage is $\log\log(n)$. 
    \item In each even step we applay $\DD$ to restrain the error rate low.  
    \item 
  \end{enumerate}
