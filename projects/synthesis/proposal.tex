...
As mentioned before, that gadget allows us to assume the initialization of encoded codewords at block length $|B|$ such that $|B| \cdot \text{polylog}(|B|) = O(\log n)$, and to focus on finding suitable codes that enable computation at constant depth overhead.
