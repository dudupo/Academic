...
\end{definition}
 ...
 \end{definition}
For example, consider a code $C$ with a $\Delta$-regular Tanner graph. Let $\Po$ be all the noisy states derived from codewords in $C$ such that the syndrome graph induced by them can be decomposed into disjoint $\Delta/2$-connected components, each of size at most $\beta \sqrt{n}$, and the $\Delta/2$-distance between any two of them, namely the number of edges needed to add to merge them into one single $\Delta/2$-connected component, is at least $\theta\sqrt{n}$.

Now let $\Pt$ be all the deviations from $C$, such that the syndrome graph induced by them can be decomposed into disjoint $\Delta/2$-connected components, each of size at most $\gamma \sqrt{n}$, and the $\Delta/2$-distance between them is at least $\theta\sqrt{n}$.
