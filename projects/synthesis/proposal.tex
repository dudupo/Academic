...
Classical Lower bounds \cite{AnnaGal}, \cite{185425}. \cite{10.1007/11672142_44} shows that a tradeof in model in which the given input is already encoded. Then the width overhead of the circuit is $\Theta(\frac{\log n}{\log \log n})$ while the depth overhead is $\Theta(\log \log n)$. \ctt{ Re-write this.} \cite{krishna2025tradeoffsvolumefaulttolerantcircuits}, A work that studied width-depth tradeoff, and shows that codes with non-trivial rate and distance, can not offer the control-X gate for any pair of logical bits.  
