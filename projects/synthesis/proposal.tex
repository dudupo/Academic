...
Consider the setting where the qubits are subjected to $p$-stochastic local noise and consider the mixed state $\rho$. Let $I$ be the subset of qubits that were not affected by the noise and denote by $\bar{I}$ its complement. We say that C is a $(\beta, \gamma)$-Memory if there is a constant depth procedure $\DD$, such that upon conditioning on the event $|I| \ge (1-\beta)n$, then with probability $1 - o(poly(n))$ there is $J$ such that:
   \begin{equation*}
     \begin{split}
       \Tr_{J}\DD \left(\rho\right) =  \Tr_{J}\left(\sigma\right) 
     \end{split}
