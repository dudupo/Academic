...
\begin{definition}[State Preparation.] Let $\mathcal{C}$ be a set of circuits, We say that $S$ is an $\left[ n, (w,d), (p \rightarrow q) \right]$-State-Perpetration scheme for $\mathcal{C}$, if for any $C \in \mathcal{C}$, $S(C) \rightarrow \tilde{C}$ such that:
  \begin{enumerate}
    \item $\text{ Depth } (\tilde{C} ) \le d\text{ Depth } (C )$. 
    \item $\text{ Width } (\tilde{C} ) \le d\text{ Width } (C )$. 
    \item The state prepared by $\tilde{C}$, namely $\tilde{C}\ket{0}$ is the state prepared by $C$ when subjected to $p$-local stochastic noise. 
  \end{enumerate}
