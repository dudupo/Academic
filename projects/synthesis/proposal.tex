...
Gadgets.
  ...
  \begin{figure}[h]
    \centering
\begin{quantikz}[row sep=0.3cm, column sep=0.7cm]
  \lstick{$q_1$} & \slice{step}  \gate[wires=9][1.7cm]{U} & \slice{step}  \gate[wires=9][1.7cm]{ F } & \slice{step}  \gate{\mathcal{N}} & \slice{step}  \qw & \\
  \lstick{$q_2$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_3$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_4$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_5$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_6$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_7$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_8$} &                  &    & \gate{\mathcal{N}} & \qw & \\
  \lstick{$q_9$} &                  &    & \gate{\mathcal{N}} & \qw & 
\end{quantikz}
\caption{ Preparing a quantum states via the concatenation fault tolerance.    }
    \label{fig:mem}
  \end{figure}
