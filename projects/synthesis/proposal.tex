...
\begin{definition}[State Perpetration.] Let $\mathcal{C}$ be a set of circuits, We say that $S$ is an $\left[ n, (w,d), (p \rightarrow q) \right]$-State-Perpetration scheme for $\mathcal{C}$, if for any $C \in \mathcal{C}$, S  states $\ket{\psi}$ that can be   
