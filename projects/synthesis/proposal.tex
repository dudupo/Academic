...
\begin{enumerate}
  \item \textbf{Disjointness of classical repetition code.} $ = 1$ 
    ...
    The $X_{1}$ generator is $X^{\otimes n}$, the $Z_{1}$ generator representations are $ \{ Z^{i} \otimes I : i \in [n] \} $ and for $Y_{1}$ are  $ \{ Y^{i} \otimes X^{\otimes n/i }  : i \in [n]  \}  $. Thus, $\Delta_{c}(X_{1}) = \frac{1}{c}$ for any $c$ and $\Delta = \frac{1}{c}$.
