...
Based on the fault-tolerance construction in \cite{aharonov1999faulttolerant}, one can obtain a $\left[ \text{polylog}(n) , (p \rightarrow q) \right]$-SP, where $n$ is the number of the source's circuit qubits, which will next be used as a block length of a desired code. Here we present an informal description of it and refer to \cite{grospellier:tel-03364419} for more details.
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*} So we prepared the state subjected to local noise (depolarizing noise) at rate $q=c_{2}p$. The depth of the circuit is at most two times the depth of $\Phi^{k}(E)$ so the $\alpha$ equals the overhead of the fault tolerance scheme namly $\alpha = \text{polylog}(n)$.
