...
Gadgets.
  ...
  \begin{figure}[h]
    \centering
    \begin{quantikz}[row sep=0.3cm, column sep=0.7cm, slice style=blue,slice label style={inner sep=1pt,anchor=south west,rotate=40}]
  \lstick{$q_1$} & \qw & \qw & \qw &   \slice{ $\rho$ at $\alpha$ error rate. } & \gate[wires=9][1.7cm]{U}  \slice{ $c\alpha + p$ error rate.  }  & \gate[wires=9][1.7cm]{ F }  & \gate{\mathcal{N}} \slice{ $\gamma\left( c \alpha + p  \right) + p < \alpha $ error rate. } & \qw & \\
