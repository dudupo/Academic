...
\begin{openproblem}
If the answer to \Cref{openproblem:constantd} is yes, what is the minimum distance for which there exists a code with that distance, and the code provides a universal gate set realization at constant depth?
\end{openproblem}
...
Last but not least, we would like to connect fault-tolerance results to complexity theory in a broad manner. Historically, new inventions of error correction codes stimulate advances in PCP theory and vice versa. For instance, the left-right Cayley complex \cite{Dinur}, \cite{leverrier2022quantum}, \cite{Pavel}, was used recently in \cite{bafna2024quasilinearsizepcpssmall} to design a routing net that is robust to collapses, which in turn was used to construct a proof-checking scheme in a similar fashion to the Polishchuk-Spielman manner \cite{10.1145/195058.195132}. Another recent result showed that having an $O(1)$-depth overhead fault tolerance protocol implies that quantum PCP with logarithmic soundness could be achieved \cite{Anshu_2024}. Thus, we ask the following:
...
Something about the importance of fault tolerance computation to PCP construction. New classical \cite{bafna2024quasilinearsizepcpssmall}, and quantum construction . \cite{10.1145/195058.195132} Seems like a construction that based on the fact that the previews circuit was fault tolerance. 
