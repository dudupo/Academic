...
Zoming out from the operations over code block lens, raising the question in the context of fault tolerance circuit. In the classical settings,  lower bounds over the overall circuit size were established, a serieas of works  \cite{AnnaGal}, \cite{185425}. \cite{10.1007/11672142_44} shown that fault tolerance circuits that compute boolean functions that are $s$-senstaive, meaning that a change in one of $s$ bits in the input leads to different output, has to exhibit at least $\Omega(s \log s )$ size. In particular, a fault tolreance circuit that compute the $n$-bit $\mathbf{Xor}$ function has to have $\Omega(n \log n )$ size. 
...
\begin{openproblem}
  Consider a circuit over $n$ classical (quantum) bits, What are the domains of depth $d$ and width $w$ for which the circuit can be fault-tolarized at the cost of $w$ and $d$ width and depth overheads.
\end{openproblem}
