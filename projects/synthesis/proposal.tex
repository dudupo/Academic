...
Gadgets.
 ...
 \begin{definition}
We say that a (quantum) error correction code $C$ is an Ideal $(\beta,\gamma)$-Memory code if there is a constant depth procedure $\DD$ such that for any $I$ of size $|I| \ge (1 - \beta) n$ and a mixed state over the $C$'s codewords $\rho \in C$, we have that there is subset of qubits $J$ at depth at least $(1-\gamma)n$:
   \begin{equation*}
     \begin{split}
       \DD \Tr_{I}\left(\rho\right) = \DD \Tr_{J}\left(\rho\right) 
     \end{split}
   \end{equation*}
   Furthermore, we say that C is a $(\beta, \gamma)$-Memory in respecting to distribution $f$ over the qubits, If for sampled subset $I \sim f$ we have that:   
   \begin{equation*}
     \begin{split}
     \DD \Tr_{I}\left(\rho\right) = \DD \Tr_{J}\left(\rho\right) 
     \end{split}
