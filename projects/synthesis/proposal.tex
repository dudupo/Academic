...
\title{ Research Proposal - Complexity Costs of Fault-Tolerance Circuits. }
...
Recently, the feasibility question has been raised again, this time regarding quantum circuits. While intensive work has been done and has succeeded in proving that polynomial quantum computation can be made fault-tolerant \cite{aharonov1999faulttolerant}, \cite{gottesman2014faulttolerant}, even with only a constant multiplicative overhead at the original circuit width \cite{grospellier:tel-03364419}, polylogarithmic depth \cite{tamiya2024polylogtimeconstantspaceoverheadfaulttolerantquantum}, and logarithmic depth \cite{Nguyen2024QuantumFT}, the minimal required depth overhead is still not well understood. This raises our first open problem: Is there a fault-tolerance scheme that maps circuits to noise-tolerant versions with a depth that is at most a constant times more? If not, what is the lower bound for depth overhead?

We stress that in all the mentioned constructions, the original constant-depth gates are mapped to logical presentations whose underlying implementation has asymptotically growing depth. This is in contrast to Pippenger \cite{Pippenger}, where the logical gates are $O(1)$-depth circuits.
Such an expansion prevents any construction that merely shifts presentations without interfering with the logical computation path of the original circuits from reaching a constant depth overhead. Previous works have made way to bypass this obstacle; notably, \cite{nguyen2024goodbinaryquantumcodes} and \cite{li2025poincaredualitymultiplicativestructures} present good error correction codes with transversal $\mathbf{CCZ}$ gates, and the series of works by Bombin \cite{PhysRevLett.97.180501}, \cite{PhysRevA.81.032301}, \cite{Bombín_2015} yield the 2D-color and 3D-color codes, which can be embedded on low-dimensional lattices (2D and 3D) and offer transversal gate implementation, where the first offers the whole Clifford group and the second the $\mathbf{T}$ gate. However, no code with a non-trivial distance and $O(1)$-depth implementation for a complete universal gate set is known what raises our following question: Is there an error correction code with non-trivial distance for which a full universal gate set over the logical qubits can be realized at constant depth? 


One of the main approaches to tackling the difficulty of performing computation is to use gate teleportation, namely encoding the gate operation in a preprocessing resource state. Then, the application of the gate is made by entangling a target state with the resource, measuring the resource, and then applying a simpler gate conditioned on the outcome of the measurement. We refer to \cite{gottesman2014faulttolerant} and \cite{grospellier:tel-03364419} for further reading. That method moves the difficulty from the quantum side to the classical one, where in many models it is common not to count classical computation in the running time. Yet, we don't distinguish between classical and quantum depth, which leads us to suggest the intermediate question about whether there exists a code for which gate teleportation can be made at constant depth.
...
Namely, if having a fault-tolerant construction that exhibits a constant multiplicative depth overhead is indeed possible, what is the minimum distance for which there exists a code with that distance, and the code provides a universal gate set realization at constant depth?


Similarly, one can also ask whether the code dimension plays a role in the feasibility of realizing logical gates. Recent work \cite{krishna2025tradeoffsvolumefaulttolerantcircuits} provides a first result regarding the width-depth tradeoff. Specifically, they show that for classical codes with distance $d$, for any logical qubit $i$, there exists a logical qubit $j \neq i$ such that the code offers the $\mathbf{CX_{i,j}}$ logical gate at most at $l$-depth. Then, it must hold that $k = O\left(\frac{n}{d^{2^{-l}}}\right)$. They also mention that their proof does not imply an analogous tradeoff relation for quantum codes. This brings us to ask if the result can be generalized to the implementation of general gates and to quantum error correction codes. 
Whether there is a relation between the depth of a logical gate of an error correction code and the dimension and depth of that code. Is that relation identical for both classical and quantum codes? If not, how does the trade-off behave in each case?


Zooming out from operations over code blocks raises the trade-off question in a wider context of fault-tolerant circuits, namely how big a fault-tolerant version of a circuit should be compared to the original circuit. In terms of size, we quantify the total number of physical gates composing the circuit. That measure somehow defines a rough trade-off between the width and the depth multiplicative overhead. In classical settings, lower bounds on the overall circuit size were established. A series of works \cite{AnnaGal}, \cite{185425} showed that fault-tolerant circuits that compute Boolean functions that are $s$-sensitive, meaning that a change in one of $s$ bits in the input leads to a different output, must exhibit at least $\Omega(s \log s)$ size, or in other words, a logarithmic expansion in $s$. In particular, a fault-tolerant circuit that computes the $n$-bit $\mathbf{Xor}$ function must have $\Omega(n \log n)$ size. Hence, we would like to generalize those results too, and ask if there is, and if so then what is, the lower bound on the size expansion of quantum fault-tolerant circuits?

Furthermore, \cite{10.1007/11672142_44} presents a construction that achieves different width and depth overheads that respect the classical trade-off induced by the required size overhead. Hence, it showed that the depth-width curve obtained by the size lower bound is tight. They showed this in a model in which the given input is already encoded. In that model, they provide a fault-tolerant circuit with a width overhead of the circuit being $\Theta(\frac{\log n}{\log \log n})$, while the depth overhead is $\Theta(\log \log n)$. So, assuming that the lower bound on the expansion in size is $\Lambda$, it's tempting to ask if for any $w$ and $d$ such that $w \cdot d = \Lambda$, a fault-tolerant circuit with $w$ and $d$ width and depth overhead can be realized.

%\begin{oproblem}
%Consider a circuit over $n$ classical (quantum) bits. What are the domains of depth $d$ and width $w$ for which the circuit can be fault-toleraized at the cost of $w$ and $d$ width and depth overheads? How does the answer change if we assume encoded input?
%\end{oproblem}

Until now, all the questions we asked addressed the fault tolerance of general circuits, but focusing on shallow circuits also attracts special interest, particularly for near-term applications. Since today's quantum machines struggle to maintain quantum states for extended periods, researchers were motivated to define NISQ, which stands for Noisy Intermediate-Scale Quantum, referring to the current era of quantum computing characterized by quantum processors with limited resources. Nevertheless, there are certain known problem candidates for demonstrating quantum advantage, such as factoring \cite{Shor_1997} and Instantaneous Quantum Polynomial-time (\textbf{IQP}) \cite{Bremner_2017}, \cite{Paletta_2024}, which have been proven to be solvable by sampling from logarithmic-depth quantum circuits.

We emphasize that in \cite{Paletta_2024} they showed a realization that uses constant quantum depth circuits with a intermediation of a polynomial classical circuit in the middle. From a technical perspective, that means that to implement the protocol, one has to protect the quantum state for the time that the classical routine lasts.
Hence, it is interesting to ask what both quantum and classical low-resource fault-tolerant realizations can be designed for those particular problems. Summarizing the above, we ask, Does sampling from $\mathbf{IQP}$ have a fault-tolerant realization using $\log n$ depth circuits, namely an $O(1)$-depth overhead? Or at least, is it possible to postpone all the classical computation until after the qubits are measured? Are there any other candidates for demonstrating quantum advantage that can be realized using shallow circuits?

Another application that uses logarithmic depth is encoding. It was shown in \cite{moore1998parallel} that codewords of stabilizer codes can be prepared at logarithmic depth with the use of quadratic ancillas, and not only that, but the preparation circuit is composed of only Hadamard and controlled-Pauli gates. In other words, the family of encoder circuits is much less rich than general circuits, and therefore, it is interesting to ask about the synthesis of encoded codewords. By synthesis, we mean generating the encoded codeword itself and not a logical representation of it (not a concatenation with another code).

Thus, this brings us to ask each of the above questions about encoding circuits. Specifically, do encoders have a fault-tolerant realization with an $O(1)$-depth overhead?

In addition to NISQ, another common characterization for limited quantum computation is computation without reset gates—namely, the gates that initialize qubits to the zero state $\ket{0}$. It was proved that in this model, when restricted to polynomial space, any computation that lasts longer than logarithmic depth results in noise \cite{aharonov1996limitationsnoisyreversiblecomputation}. Having a constant depth-overhead fault tolerance scheme would imply the feasibility of log depth computation in that model. So in this work, we are also aiming to answer the following: Assuming there are $\Theta(\log(n))$-depth circuits that can be fault-tolerized at the cost of $O(1)$-depth overhead, could those constructions be generalized to hold in the non-reset gates regime?

Last but not least, we would like to connect fault-tolerance results to complexity theory in a broad manner. Historically, new inventions of error correction codes have stimulated advances in PCP theory and vice versa. For instance, the left-right Cayley complex \cite{Dinur}, \cite{leverrier2022quantum}, \cite{Pavel} was used recently in \cite{bafna2024quasilinearsizepcpssmall} to design a routing net that is robust to collapses, which in turn was used to construct a proof-checking scheme in a manner similar to the Polishchuk-Spielman approach \cite{10.1145/195058.195132}. Another recent result showed that having an $O(1)$-depth overhead fault tolerance protocol implies that quantum PCP with logarithmic soundness could be achieved \cite{Anshu_2024}. Thus, we suggest the following vague question: Whether fault-tolerant construction can help to design a verification scheme? Does an analog to the robustness routing protocol exist for verifying quantum computations? What else can be borrowed from the fault-tolerant world for the use of quantum PCP?
...
\subsection{State Preparation.}
  ...
  \begin{figure}[h]
    \centering
\begin{quantikz}[row sep=0.3cm, column sep=0.7cm]
  \lstick{$q_1$} & \gate[wires=9][1.7cm]{\Phi^{k}(E)} & \gate{\mathcal{N}} &   \gate[wires=3]{\Phi^{k-1}(E_{\text{con}}^{-1})} & \gate{\mathcal{N}} & \gate[wires=2]{\Phi^{k-2}(E_{\text{con}}^{-1})}   & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_2$} &                      & \gate{\mathcal{N}} &                & \gate{\mathcal{N}} &                      & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_3$} &                      & \gate{\mathcal{N}} &                & \gate{\mathcal{N}} &                      & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_4$} &                      & \gate{\mathcal{N}} &     \gate[wires=3]{\Phi^{k-1}(E_{\text{con}}^{-1})} & \gate{\mathcal{N}} & \gate[wires=2]{\Phi^{k-2}(E_{\text{con}}^{-1})}                                 & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_5$} &                      & \gate{\mathcal{N}} &                & \gate{\mathcal{N}} &                      & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_6$} &                      & \gate{\mathcal{N}} &                & \gate{\mathcal{N}} &                      & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_7$} &                      & \gate{\mathcal{N}} &      \gate[wires=3]{\Phi^{k-1}(E_{\text{con}}^{-1})} & \gate{\mathcal{N}} & \gate[wires=2]{\Phi^{k-2}(E_{\text{con}}^{-1})}                                & \gate{\mathcal{N}} & \qw &\\
  \lstick{$q_8$} &                      & \gate{\mathcal{N}} &                & \gate{\mathcal{N}} &                      & \gate{\mathcal{N}} & \qw &\\
...
\begin{definition}[State Preparation.] Let $\mathcal{C}$ be a set of circuits. We say that $S : \mathcal{C} \rightarrow p\textbf{-noisy circuits} $ is an $\left[ \alpha , (p \rightarrow q) \right]$-State-Preparation scheme for $\mathcal{C}$ if for any $C \in \mathcal{C}$, we have that $S(C) = \tilde{C}$ such that:
  \begin{enumerate}
    \item $\text{ Depth } (\tilde{C} ) \le \alpha \text{ Depth } (C )$. 
    \item $\text{ Width } (\tilde{C} ) \le \alpha \text{ Width } (C )$. 
  ...
  \end{enumerate}  
  \end{definition}
Based on the fault-tolerance construction in \cite{aharonov1999faulttolerant}, one can obtain a $\left[ \text{polylog}(n) , (p \rightarrow q) \right]$-SP, where $n$ is the number of the source's circuit qubits, which will next be used as a block length of a desired code. Here we present an informal description of it and refer to \cite{grospellier:tel-03364419} for more details.

  \Cref{fig:prep} illustrates the preparation scheme. Denote by $E$ the encoder of the desired code to prepare, and by $E_{\text{con}}^{-1}$ the decoder of the single-level concatenation code. First, we apply a fault-tolerant version of the encoder $E$, using the $k$-level concatenated code; that stage is represented as $\Phi^{k}(E)$. Then we decode the concatenated code from top to bottom; namely, in the $j$th stage, we apply $\Phi^{j-1}(E_{\text{con}}^{-1})$.
  
  Since the probability of a logic error at the $k$-level concatenation code is bounded by $(cp)^{2^{k-1}}$, we have by the union bound that the probability that the $i$th bit, in the target state, will absorb an error at the end is bounded by:
  \begin{equation*}
    \begin{split}
      \left( cp \right)^{2^{k-1}} + \left( cp \right)^{2^{k-2}} + .. \left( cp \right)^{2^{k-3}} + .. +  cp \le c_{2}p 
    \end{split}
  \end{equation*} So we prepared the state subjected to local noise (depolarizing noise) at rate $q=c_{2}p$. The depth of the circuit is at most two times the depth of $\Phi^{k}(E)$ so $\alpha$ equals the overhead of the fault tolerance scheme namly $\alpha = \text{polylog}(n)$.


As mentioned before, that gadget allows us to assume the initialization of encoded codewords at block length $|B|$ such that $|B| \cdot \text{polylog}(|B|) = O(\log n)$ while remaining in $\QNCon{}$, and to focus on finding suitable codes that enable computation with constant depth overhead.


 \subsection{Memory.} 
 \newcommand*{\DD}{\mathbf{D} }
Informally, a memory code is a code that stores a logical state for a long time while keeping the noise below a certain amount. We define it formally by saying that the code is equipped with a constant depth decoder that reduces an error affecting at most a $\beta$ portion of the qubits into an error affecting at most a $\gamma$ portion of the qubits.
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Similarly, any two-qubit gate $U$ can be encoded using a proper magic state, and the gate teleportation will be composed by entangling using CNOT, then measurement, and eventually, upon the measurement result $P$, applying a correction $UPU^{\dagger}$. We refer to \cite{gottesman2014faulttolerant} and \cite{grospellier:tel-03364419} for further reading. 


\begin{oproblem}
Does there exist a Memory code for which one can implement gate teleportation to a universal gate set in a local manner?
\end{oproblem}
  ...
  \end{enumerate}
So overall, the depth overhead we get is $\log \log n$, where the bottleneck is the time required to decide on the final gate to apply given the measurement results in the gate-teleportation gadget. That raises the question of whether there exist memory codes for which one can perform the correction search in $O(1)$ depth. \cite{Gu_2024}
...
We first present a property of stabilizer codes called disjointness \cite{Jochym_O_Connor_2018} \cite{bostanci2021findingdisjointnessstabilizercodes}, which quantifies how the structure of the code is far from being just a union of disjoint subsystems. In \cite{Jochym_O_Connor_2018}, they proved a relation between the disjointness value and the complexities of the logical operators that can be computed either transversally or by shallow depth circuits.
...
\begin{definition}
Let $P$ be a logical Pauli in a stabilizer code $C$. Denote by $\Delta{c}(P)$ the $c$-disjointness of $P$, which is defined to be the size of the maximal subset of $P$-representations such that each qubit touches at most $c$ of them, divided by $c$. 


The $c$-disjointness of $C$ is defined to be the minimum of $c$-disjointness, where the minimization is taken over all the logical Paulis of $C$. \cite{Jochym_O_Connor_2018} proves the following: let $d_{\uparrow}$ and $d_{\downarrow}$ be the maximal and minimal weights of a codeword in $C$. Then all the logical operators in $C$ are in the   
\begin{equation*}
  \begin{split}
    1 + \lceil \log_{\Delta}\frac{d_{\uparrow} }{d_{\downarrow} } \rceil \text{   'th Clifford hierarchy}
  \end{split}
\end{equation*}
Hence, we interest code with a finite disjointness.  
...
\begin{enumerate}
  \item \textbf{Disjointness of classical repetition code.} $ = 1$ 
    ...
    The $X_{1}$ generator is $X^{\otimes n}$, the $Z_{1}$ generator representations are $ \{ Z^{i} \otimes I^{\otimes n / i} : i \in [n] \} $ and for $Y_{1}$ are  $ \{ Y^{i} \otimes X^{\otimes n/i }  : i \in [n]  \}  $. Thus, $\Delta_{c}(X_{1}) = \frac{1}{c}$ for any $c$ and $\Delta_{c} = \frac{1}{c} \Rightarrow \Delta = 1$.
  \item \textbf{Disjointness of 2D Toric code.} $\ge \frac{\sqrt{n}}{2}$. 

First, notice that any two representations of $Y_{1}$ intersect in at least two qubits (four orbits, two horizontally and two vertically). Thus, $\Delta_{1}\left( Y_{1} \right) = 1$. In addition, since one can take disjoint columns as $X_{1}$ representations and disjoint rows as $Z_{1}$ representations, we have that $\Delta_{1}(X_1) = \Delta_{1}(Z_1)$ is at least $\sqrt{n}$. Similarly, we get the same for $Y_{2}, X_{2}, Z_{2}$, and therefore, $\Delta_{1} = \min \Delta_{1} = 1$.
    
Yet, for $\Delta_{2}$, we have from the same arguments as presented previously that $\Delta_{2}(X_{1}) = \Delta_{2}(Z_{1}) \ge \frac{1}{2}\sqrt{n}$. And also $\Delta_{2}(Y_{1}) \ge \frac{1}{2}\sqrt{n}$.

\end{enumerate}

  \begin{oproblem}
    Does there exist a memory code that has asymptotically approaches $\rightarrow 1$ disjointness? 
  \end{oproblem}
...
While the disjointness property is a property of a code and therefore can be thought of as a computational restriction, one can also ask if there are informational obstacles. In \cite{newman2017limitationstransversalcomputationquantum}, the authors succeeded in linking a transversal Toffoli to extremely low overhead homomorphic computation, what in turn contradicts a known information theoretic bounds. 
\begin{definition}
  A quantum code is an $r$-fold quantum code, if it's codewords can be written as product over $r$ subsystems: 
  \begin{equation*}
    \begin{split}
      \ket{i}_{L} =  \otimes^{r}_{j}\ket{\psi_{i,j}}  
    \end{split}
  \end{equation*} When we think of the states in each subsystem as codewords of subcodes, note that any code is at least a $1$-fold code. If each of the subcodes has distance $1$, then we call the code a maximally redundant code. An example of a maximally redundant code is the Shor code.
\end{definition}

\Cref{fig:ensch} outlines the encoding in the suggested homomorphic scheme. The input is encoded via the $r$-fold code (in the figure $r=1$), and then we think of the qubits as arranged in a matrix, each row is matched to a logical qubit, and the $i$th row consists of all of the $i$th physical qubits of each logical qubit. Then, any $i$th row of qubits, except the last one, is transplanted in the place of a random column of an $n\times n$-qubit mixed state. Those qubits are sent to the prover. (For $r > 1$, we spare a qubit from each subsystem.)

Observe that any transversal gate in the original code can be implemented by transversal application over each of the \(n \times n\) separation bulk. So having transversal Toffoli enables the prover to simulate any classical computation while keeping the mixed structure.  


It was shown in \cite{newman2017limitationstransversalcomputationquantum} that any binary code which is not a maximally redundant code cannot offer transversal logical Toffoli; otherwise, it implies the existence of a relatively low-resource consuming homomorphic protocol which violates Nayak's bound \cite{nayak1999optimallowerboundsquantum}.


\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{homomo.png}
  \caption{Skecth of the encryption scheme, taken form \cite{newman2017limitationstransversalcomputationquantum} }
  \label{fig:ensch}
\end{figure}

\begin{oproblem}
Does computing Toffoli at shallow depth, similar to the transversal case, imply homomorphic computation at a cost contradicting Nayak’s bound? And in a more general context, when Information Theory imposes restrictions on the depth overhead of fault tolerance?
\end{oproblem}

  \section {Summery and Applications.} \label{sec:app}
 % We summarize the proposal by extensively pointing out the potential applications of the study.

  Up to this point, we have tried to gets hints about whether it is likely to have a \CDO. Now we're prompted to ask: Suppose that we have an answer about the hardness of computing in the presence of noise compared to computing in an ideal environment, what can be learned beyond the context of fault tolerance? We divide our discussion into the YES scenario, in which we have \CDO, and the NO scenario, in which we don't.
  ...
  \begin{oproblem}
    Whether and when, separation between noise classes implies separation between their ideal origins, In particular, on what conditions: 
    \begin{equation*}
      \begin{split}
        \QNC_{1} \neq \text{Noisy-}\QNC_{1} \Rightarrow \QNC_{1} \neq \NC_{1}
      \end{split}
    \end{equation*}
  \end{oproblem}

  \subsection{The YES Scenario.} 
Having a \CDO{} is the first stage in realizing a quantum advantage when restricted to limited (quantum) computational power. Especially, it's required to enable computation in logarithmic depth in the setting where reset gates are not allowed. This is due to the fact that there is a trade-off between the memory of the system and the entropy accumulated, which grows exponentially with the depth \cite{aharonov1999faulttolerant}. We finish the proposal by presenting a question of engineering. We would like to have an end-to-end realization of computation that might demonstrate quantum advantage in the limited setting without reset gates.

  \begin{oproblem}{Quantum Advantage Without Reset Gates.}
    Does there exist problems believed to be hard classically but easy quantumly, which can be computed fault-tolerantly, without the use of reset gates? In particular, Is it possible to factor big integers or sampling from IQP  without reset gates? What is the computational power of an ideally polynomial machine with an access to noisy-$\QNC_{1}$ machines without reset gates? 
  \end{oproblem}
