...
\begin{definition}[State Preparation.] Let $\mathcal{C}$ be a set of circuits. We say that $S : \mathcal{C} \rightarrow p\text{-noisy circuits} $ is an $\left[ \alpha , (p \rightarrow q) \right]$-State-Preparation scheme for $\mathcal{C}$ if for any $C \in \mathcal{C}$, we have that $S(C) = \tilde{C}$ such that:
  \begin{enumerate}
