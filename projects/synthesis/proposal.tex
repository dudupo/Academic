...
Gadgets.
 ...
 \subsection{Memory.} 
 \begin{definition}
We say that a (quantum) error correction code is a $(\beta,\gamma)$-memory code if there is a constant depth procedure $D$ such that for any $I$ of size $|I| \ge (1 - \beta) n$ and $\rho \in C$, we have that:
   for some subset of qubits $J$ at depth at least $(1-\gamma)n$. 
 \end{definition}

   \begin{equation*}
     \begin{split}]
     \text{D} \Tr_{I}\rho = \text{D} \Tr_{J}\rho 
     \end{split}
   \end{equation*}
