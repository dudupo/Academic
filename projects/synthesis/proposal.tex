...
Yet the minimal required depth overhead is still not well understood. We stress out that in the all mentioned constructions, the original constant-depth gates are mapped to logical presentations which their underline implementation have asymptotically grow\footnote{Note, that here, classical computation is also counted in the overall depth cost} depth. That in construct to Pippenger \cite{Pippenger} where the logical presentation has $O(1)$ depth circuit. 

Even those, there 
\cite{nguyen2024goodbinaryquantumcodes}, \cite{li2025poincaredualitymultiplicativestructures} a paragraph that  talks about that there are codes which implement logical codes at constant depth. Yet its no known about a code that can compute a complete universal gate set. \cite{PhysRevLett.97.180501}, \cite{PhysRevA.81.032301}, \cite{Bombín_2015}. 
}
