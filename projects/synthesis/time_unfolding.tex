...
\begin{definition}[Faults Propagation to Time $t$]
  For a quantum circuit $C=\{u_{i}\}$, with depth $d$, and gate $u_{j}$ in $C$, a propagation of $u_{j}$ to time $t$ is the quantum circuit    obtained by the follow process: 
  \begin{enumerate}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
\item Then, for $k \le t - l_{j,1}$, construct $C_{k}$ from $C_{k-1}$ as follows: Denote $\tau = k + l_{j,1}$, and look for an operator $X \colon L(\mathcal{H}_{2}^{w}) \rightarrow L(\mathcal{H}_{2}^{w})$, such that $C_{k-1}|_{\tau}C_{k-1}|_{\tau+1} = C_{k-1}|_{\tau+1}X$ \inb{(Up to ancilea tracing)}. Then define
  \begin{equation*}
    \begin{split}
