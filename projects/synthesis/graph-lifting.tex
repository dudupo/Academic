...
\begin{proof}
  ...
  \item \ctt{Assume $H_{r}, H_{l}$ are the complete graphs, and adding self-loops in step 1.} If $X\neq Y$, then with probability $1 - P(G)$, $X$ and $Y$ commute in $G$ and therefore there are even number of red edges at the form $\{o , o^{\prime}\}$ to connect paths from $x,y \rightarrow w$.     
    \begin{equation*}
      \begin{split}
        & P(G){ m \choose 2} + Q(G){ m \choose 2} = \frac{1}{2}m \Delta^{2} \\ 
        & Q(G) = { m \choose 2 }^{-1}\cdot \frac{1}{2}m\Delta^{2} - P(G)
      \end{split}
    \end{equation*}
    ...
    \begin{equation*}
      \begin{split}
       ...
       \rightarrow_{m \rightarrow \infty } &  \left(  1 - \frac{1}{\Delta} \right)P(G)
      \end{split}
    \end{equation*}
