...
\begin{proof}
  ...
  \item \ctt{Assume $H_{r}, H_{l}$ are the complete graphs, and adding self-loops in step 1.} If $X\neq Y$, then with probability $1 - P(G)$, $X$ and $Y$ commute in $G$ and therefore there are even number of red edges at the form $\{o , o^{\prime}\}$ to connect paths from $x,y \rightarrow w$.     
    ...
    \begin{equation*}
      \begin{split}
        P(G^{\prime}) \le \left( 1 - \frac{1}{m} \right) \left( P(G) + Q(G) \cdot \frac{1}{\Delta} \right) = \left( 1 - \frac{1}{m} \right) \left( { m \choose 2 }^{-1}\cdot \frac{1}{2}mD^{2} + \left( 1 -  \frac{1}{\Delta}\right)P(G) \right)
      \end{split}
    \end{equation*}
  \end{itemize}
\end{proof}
