  ...
  So, we connect arrow between $u \in \partial z^{(t)}$ to $v \in \partial z^{(t+1)}$ if there is an edge between $u$ to $v$ in the base graph $G$. Then for any triple $v \in \partial z^{(t+1)}$ and $u,w \in \partial z^{(t)}$ such $\{u,v\}$ and $\{w,v\}$ are in $A$ we connect $\{u,w\}$ by fork. In such way we get a space time graph.
  \begin{definition}
