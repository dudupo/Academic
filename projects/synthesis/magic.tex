...
The notation used in this paper follows the standard conventions for coding theory, with the alphabet $\mathbb{F}_2$ representing the binary field and $n$ representing the length of the code.  We will also use $k$ to represent the dimension of the code and $\rho$ to represent the rate $k/n$ of the code. The minimum distance of the code will be denoted as $d$ and the relative distance, i.e $d/n$ by $\rho$. In this paper, $n$ and $k$ will be referred sometimes by the number of physical and logical bits. Codes will be denoted by capital $C$ followed by either subscript or superscript. When multiply codes are been referred we will use the above parameters as function, for example $\rho(C_{1})$ represents the rate of the code $C_{1}$. Square brackets are used to presents compactly all those parameters and we use them as follow $C=[n,k,d]$. 

The parity check matrix of the code will be denoted as $H$, with the rows of $H$ representing the parity check equations. The generator matrix of the code will be denoted as $G$, with the rows of $G$ representing base of codewords. The syndrome of a received word $r$ will be denoted as $s$, which is the result of multiplying $r$ by the transpose of $H$. We use $C^\perp$ to denote the dual code of $C$, namely the code defined such any codeword of it $z\in C^\perp$ is orthogonal to any $x\in C$, means $z\cdot x = 0$, where the products defined to be $x\cdot z = \sum_{i}{x_{i}z_{i}}$. $C^{\top}$ stands for the code obtained by taking the parity check matrix of $C$ and transporting it.  
