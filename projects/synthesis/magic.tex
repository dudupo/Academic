...
\begin{proof}
  ...
  Now for defying $\text{ span}\mathcal{X}$, we are going to consider the parity checks matrix obtained by adding restrictions to $C_{X}$ restrictions as follows: Divide the first $n^{\frac{1}{4}}$ bits into $\Delta$-size buckets $B_{1} = \{x_{1}, x_{2}, ..,x_{\Delta}\}$, $B_{2} = \{x_{\Delta + 1}, x_{\Delta + 2}, ..,x_{2\Delta}\}$, .., $B_{i} = \{x_{(i-1)\Delta + 1}, x_{(i-1)\Delta + 2}, ..,x_{i\Delta}\}$, .. $B_{n^{\frac{1}{4}}/\Delta}$. Then let $\text{ span}\mathcal{X}$ be all the codewords of $\QQ$ satisfying $\tilde{C}_{0}$ restrictions for each bucket.
\end{proof}
