...
\begin{claim}
  Consider the ring $\mathbb{F}_{q}[x]$ ($q$ is prime). Let $\Delta = 4^{c}$, and $c >\ge 3$, Then we have that:
\begin{equation*}
  \begin{split}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Step. Denote by $B_{j}(\Delta)$ the bucket $\Delta\cdot j +1,  \Delta\cdot j +2, .. \Delta \cdot (j+1)-1$. Observes that $\sum_{x\in B_{j+1}(\Delta)}{x^{i}} =_{\Delta}\sum_{x\in B_{j+1}(\Delta)}{(x-\Delta)^{i}} =_{\Delta} \sum_{x\in B_{j}(\Delta)}{x^{i}}$.  
      \begin{equation*}
        \begin{split}
          \sum_{x \in \mathbb{F}_{\Delta}}{x^{i}} &= \sum_{j \in [4]}\sum_{x \in \mathbb{F}_{j\cdot \frac{\Delta}{4}}}{x^{i}} \\
          &= 
        \end{split}
      \end{equation*}
  \end{enumerate}
