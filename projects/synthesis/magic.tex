...
In this work we consider quantum circuits under the Clifford-free noise model, in this model it is assumed that any of the Clifford gates, let's say $S,H,CZ$, can be applied perfectly. In addition, the circuits has access to noisy magic states at error rate $p$, formulated as the mixed state $(1-p)\ket{T} + pZ\ket{T}$, Where $p \in (0,1)$ is the probability that a given state is actually an faulted one and $\ket{T}= \frac{1}{\sqrt{2}}(\ket{0} + e^{i\frac{\pi}{4}}\ket{1})$ is a Magic States, finally the model allowed to perform intermediate measurements and apply Clifford gates controlled on the measurements classical outcomes. It was shown that this model is quantum universal. Magic State Distillation Protocol, is a quantum circuit in the Clifford-free noise model, which consume $n$ noisy magic state at error rate $p$ and outputs $k$ independently magic states at error rate $\frac{1}{\varepsilon}$.   
...
subset of a base for the code $\QQ$. Such $\left(\text{ span } \mathcal{X}/x_0 \ \right)|_{w}$ is \trig code in respect to $C_{X}|_{w}$.  
