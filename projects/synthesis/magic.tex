...
\begin{claim}
  ...
  \begin{enumerate}
    ...
    \item Otherwise, the circuit computes the state $Z^{e}\prod_{g \in \Lambda} T_{g} \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }$, where the probability that $e_{i} = 1$ is less than $\zeta_{\Delta} \cdot  p$. Additionally, for any $i$, there are at most $\xi_{\Delta}$ indices $j$ such that $e_{i}$ and $e_{j}$ are dependent.
  \end{enumerate}
\end{claim}
