...
\begin{claim} 
  \label{claim:commute}
  Let $C$ be a $[n,k,d]$ binary linear code, and let $\Lambda$ be subcode $\Lambda \subset C$ at dimension $k'$. Then there exists a code $C' = [\leq 2n, \geq k-k'/2, d]$ and a subcode of it $\Lambda' $ in it at dimension $\ge k^{\prime}/2$, such every triple $x \in \Lambda'$ and $y,z \in C^{\prime}$  we have 
  \begin{enumerate}
    \item $x\cdot y = 0$
