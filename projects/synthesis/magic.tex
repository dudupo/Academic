...
\end{claim}
\begin{proof}
  Let $\tau \in \sMbas + C_{Z}^\perp$, applying $T^{(w)}$ on $\ket{\tau}$ add a phase of $i\frac{\pi}{4}\left|\tau\right|_{w}$. Notice that $\tau$ can decompose to the sum of  $x_{0} + y + z$ when $y \in \text{ span } \mathcal{X}$ and $z\in C_{Z}^{\perp}$.  
\end{proof}
