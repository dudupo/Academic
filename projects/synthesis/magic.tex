...
For any $v \in V$, let $\ket{z_{v}}$ be the superposition of codewords in $C_{0}$ supported by the local view of $v$. Similarly, for any subset of vertices $W \subset V$, let $\ket{z_{W}}$ be the uniform superposition over the subspace spanned by the generators supported by the vertices in $W$. In other words:
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
Using the notation, applying all the encoders $E_{v}$ at once over the positive vertices results in:
  \begin{equation*}
    \begin{split}
  ...
  \begin{align*}
          ...
          \text{CNOT} & \ \sum_{v \in V^{+}}\sum_{u\in V^{-}} & \ket{z_{v}}  \ket{z_{u}} & \mapsto  \\ 
          I \otimes \left( \cup_{v\in V^{-}} D \right)  & \ \sum_{v \in V^{+}}\sum_{u\in V^{-}} & \ket{z_{v} + z_{u}} {\ket{z_{u}}} & \mapsto  \\
            &  \ \sum_{v \in V^{+}}\sum_{u\in V^{-}} & \ket{z_{v} + z_{u}}\ket{0} & 
    \end{align*}
\end{proof}
