...
\end{claim}
\begin{proof} 
  %First, consider the trigonal/staris/square matrix given by gauss applying elimination on $\Lambda$. Now, consider the following process, go up hil in the squares matrix. Let $j$ be the first nonzero cordinate in the most down row in the matrix. In the $i$th iteration, we ask how many vectors $u_{m}$ such that $m < k^{\prime}-i$ are statisfies $u_{m} u_{k^{\prime}-i} = 0$. Now, if encoding the $j-i$ cordinate by $C_{0}^{1}$ such $1_{C_{0}^{1}}$ give $|B_{i} | > |\bar{B_{i}}|$ then choose to encode with $C_{0}^{1}$ otherwise encode that coordiante with $C_{0}^{0}$. Denote the vectors yilded form that process by $\Gamma$. 
First, consider the upper triangular matrix obtained by applying Gaussian elimination on $\Lambda$. Now, consider the following process: go up hill in the square matrix. Let $j$ be the first nonzero coordinate in the bottom row of the matrix. In the $i$th iteration, we ask how many vectors $u_{m}$ satisfy $u_{m} u_{k^{\prime}-i} = 0$ for $m < k^{\prime}-i$. If encoding the $j-i$ coordinate by $C_{0}^{1}$ such that $1_{C_{0}^{1}}$ gives $|B_{i}| > |\bar{B_{i}}|$, then choose to encode with $C_{0}^{1}$; otherwise, encode that coordinate with $C_{0}^{0}$. Denote the vectors yielded from that process by $\Gamma$.
 
Denote by $S\subset \Gamma$ the group of vectors such that there exists at least one vector in $\Gamma$ for which their multiplication is not zero. Observe that the total number of pairs with zero multiplication is greater than:
  \begin{equation*}
    \begin{split}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Set $\Lambda^{\prime} \leftarrow \Gamma / S$. 
