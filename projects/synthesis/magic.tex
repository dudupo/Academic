...
Let $\ket{f}$ be a codeword in $C_{\hat{X}}$, and let $\hat{X}_{g}$ be the indicator that equals $1$ if $f$ has support on $\hat{X}_{g}$, and $0$ otherwise. Observes that applying $T^{\otimes}$ on $\ket{f}$ yilds the state: 
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  \begin{split}
    ...
    & -  2 \cdot \pi/4 \sum_{g ,h \in C_{z}^{\perp}} \hat{X}_{g}\hat{X}_{h}|g\cdot h| \\
   \ifdefined\MORECALC & +  \cancel{4 \cdot i\pi/4 \sum_{g \in \Lambda ,h} \hat{X}_{g}\hat{X}_{h}\hat{X}_{l}2} \\ \fi
    & +  4 \cdot i\pi/4 \sum_{g,h \in C_{Z}^{\perp}} \hat{X}_{g}\hat{X}_{h}\hat{X}_{l}|g\cdot h \cdot l|  \Big) \ket{f}
  \end{split}
\end{equation*}
...
\begin{equation*}
  \begin{split}
    \prod_{g \in \Lambda} \left( I +  T_{g}X_{g} \right) \ket{C_{Z}^{\perp}} &=  M_{1}^{\dagger} T^{n} \ \ \prod_{g \in \Lambda}  E \ \ L[M_{2}^{\dagger}] \ \ \prod_{g \in C_{Z}^{\perp}\cup \Lambda} \left( I +  X_{L[g]} \right) \ket{0}  
  \end{split}
\end{equation*}

\begin{claim}
  The state $\ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }\ket{0}$ can be compute, such that the light cone depth of any non-clifford gate is bounded by constant.    
\end{claim}
\begin{proof}
\begin{align*}
  \left( H_{X} \otimes I \right) CX_{n\rightarrow n} \left(E \otimes E \right) \ \ I \otimes L[M_{2}^{\dagger}] \ \ & \prod_{J\in \{\Lambda,C_{Z}^{\perp} \}} \prod_{g \in J} \left( I +  X_{L[g]} \right) && \ket{0} \ket{0} \\  
   = \left( H_{X} \otimes I \right) CX_{n\rightarrow n} & \sum_{\substack{ z \in C_{Z}^\perp \\  x\in \text{ span } \Lambda}}  e^{\varphi(z)} && \ket{x}\ket{z} \\ 
