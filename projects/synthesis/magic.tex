...
For $\Delta$ prime, $\Delta < q$, We have that 
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
So it's left to show that if take the bipartite graph to be an expander graph then we have a good code.  


Let $G$ be bipartite $G=(L,R,E)$ is a $(n,m,\gamma,\alpha)$ expander, means that for any $S \subset V(G)$, such that $|S| < \gamma n $ the size of the group of the $S$ neighbors, is at least $\Gamma(|S|) > \alpha |S|$. Consider the graph, $G^{\prime} = ( \Delta \times L, R, E^{\prime} )$ defined as  
\begin{equation*}
  \begin{split}
