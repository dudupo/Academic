...
There are at most $2\rho\Delta^{2}$ such generators, and we are looking for $\zeta \in \mathbb{F}_{2}^{\star}$ such $\zeta \cdot g = 1$ but for any other generator. 
    \end{proof}
  \end{subfigure}
...
\foreach \i in {1,...,5}
        \draw (\i) -- (0);
\foreach \i[evaluate=\i as \m using int(\+1)]in {1,...,5}
        \draw (\i\i) -- (\m\m);
\foreach \i in {1,...,5}
    \foreach \j in {1,...,4}
        \draw (\i) -- (5*\i+\j);
