...
\begin{claim}
  ...
  \begin{enumerate}
    \item In the no-noise setting, The circuit compute the state $\mathcal{C} \ket{0}^{\Theta(n)} \otimes \ket{A}^{\Theta(n)} \rightarrow  \prod_{g \in \Lambda} T_{g} \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }$
    \item Otherwise, the circuit computes the state 
      \begin{equation*}
        \begin{split}
      Z^{e} \ \ \ \prod_{g \in \Lambda} T_{g} \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }
        \end{split}
      \end{equation*}
...
\begin{enumerate}
  ...
  \item In any rounds $\mathcal{D}$ perform a at most $O(n)$ work (depth $\times$ width).  
