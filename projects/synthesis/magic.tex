...
\begin{definition}
  ...
  \begin{equation*}
    \begin{split}
      \sum_{i \in [n]}\prod_{g_{j}\in G}g_{j}^{i} =_{k} 0 
    \end{split}
  \end{equation*}
\end{definition}

\begin{claim} 
If there exists a single $m$-genorthogonal code for a finite length $\Delta$, then there is a family of $m$-genorthogonal codes.
\end{claim}
\begin{proof}
