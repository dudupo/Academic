...
\begin{claim} 
  \label{claim:commute}
  Let $C$ be a $[n,k,d]$ binary linear code, and let $\Lambda$ be subcode $\Lambda \subset C$ at dimension $k'$. Then there exists a code $C' = [\leq 2n, \geq k-k'/2, d]$ and a subcode of it $\Lambda' $ in it at dimension $\ge k^{\prime}/2$, such:
  \begin{enumerate}
    \item $x \in \Lambda'$ and $y \in C^{\prime} \ \ x\cdot y = 0$
    \item $x \in \Lambda'$ and $y,z \in C^{\prime} \ \ x \cdot y \cdot z =0 $
  \end{enumerate}
\end{claim}
\begin{proof} 
