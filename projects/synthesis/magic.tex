...
\begin{remark}
  We emphasise that instead of setting more restrictions on the vertices in $\Gamma(w)$ one could just divides the non non-trival bits of $w$ into $\Delta$-size buckets, and then considering the codewords which their restrictions to a bucket is a codeword of $\tilde{C}_{0}$. Then the above proof can be easily adapted to result the following for general CSS codes: 
\begin{equation*}
    \begin{split}
