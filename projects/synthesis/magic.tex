...
\begin{claim} 
  \label{claim:commute}
  Let $C$ be a $[n,k,d]$ binary linear code, and let $\Lambda$ be subcode $\Lambda \subset C$ at dimension $k' > \alpha k$ for some $\alpha\in(0,1)$ and distance $d^{\prime}$. Then there exists a code $C' = [\leq 2n, \geq (1 - \alpha + \frac{\alpha^{3}}{24})k, d]$ and a subcode of it $\Lambda' $ in it at dimension $\ge \frac{\alpha^{3}}{24}k$ and distance $d^{\prime}$, such:
  \begin{enumerate}
      \label{property:prop}
...
Let $\ket{f}$ be a codeword in $C_{X}$, and let $\hat{X}_{g}$ be the indicator that equals $1$ if $f$ has support on $\hat{X}_{g}$, and $0$ otherwise. Observes that applying $T^{\otimes}$ on $\ket{f}$ yilds the state: 
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  \begin{split}
    T^{\otimes n}\ket{f} & = \\
    & = \exp \Big( i\pi/4 \sum_{g \in \Lambda } \hat{X}_{g}  \\
    & -  2 \cdot \pi/4 \sum_{g \in \Lambda ,h} 2 \hat{X}_{g}\hat{X}_{h}  \\
...
\begin{equation*}
  \begin{split}
    T^{n}\ket{f} = & \prod_{g \in \Lambda, h} \{ CZ_{g,h} | I \} \ \ \prod_{g \in \Lambda} T_{g} \ \  X_{f_{1}} \\
...
\begin{equation*}
  \begin{split}
    M_{1} &= \prod_{g \in \Lambda, h} \{ CZ_{g,h} | I \} \\
    M_{2} &= \prod_{g ,h \in C_{z}^{\perp}} \{ CS_{g,h} | CZ_{g,h} | I \} \ \ \prod_{g ,h, l \in C_{z}^{\perp}} \{ CCZ_{g,h,l} | I \}
  \end{split}
\end{equation*}
...
\begin{equation*}
  \begin{split}
    \prod_{g \in \Lambda} T_{g} \ket{f} &=  M_{1}^{\dagger} T^{n}  M_{2}^{\dagger} \ket{f} \\
    \prod_{g \in \Lambda} T_{g}\ket{f} &=  M_{1}^{\dagger} T^{n} \ \ E \ \ L[M_{2}^{\dagger}] \ \ \ket{L[f]}  
  \end{split}
\end{equation*}

\begin{claim}
  \label{claim:lowlightcone}
  The state $\left(M_{2}^{\dagger}\otimes I\right)\ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }\ket{0}$ can be computed, such that the light cone depth of any non-clifford gate is bounded by constant.    
\end{claim}
\begin{proof}
\begin{align*}
  \left( I \otimes H_{X} \right) CX_{n\rightarrow n} \left(E \otimes E \right) \ \ I \otimes L[M_{2}^{\dagger}] \ \ & \prod_{\substack{ J\in \{\Lambda,\\ \text{ gen } C_{Z}^{\perp} \}}} \prod_{g \in J} \left( I +  X_{L[g]} \right) && \ket{0} \ket{0} \\  
   = \left( I \otimes H_{X}   \right) CX_{n\rightarrow n} & \sum_{\substack{ z \in C_{Z}^\perp \\  x\in \text{ span } \Lambda}}  e^{\varphi(z)} && \ket{x}\ket{z} \\ 
    = & \sum_{\substack{ z \in C_{Z}^\perp \\  x\in \text{ span } \Lambda}}   e^{\varphi(z)} && \ket{x+z}\ket{0} \\ 
    = & \sum_{\substack{ z \in C_{Z}^\perp \\  x\in \text{ span } \Lambda}} \left(M_{2}^{\dagger}\otimes I\right) && \ket{x+z}\ket{0} \\ 
    = & \left(M_{2}^{\dagger}\otimes I\right) && \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }\ket{0}
\end{align*}
\end{proof}
...
\begin{claim}
  ...
  \begin{enumerate}
    \item In the no-noise setting, The circuit compute the state 
      \begin{equation*}
        \begin{split}
      \mathcal{C} \ket{0}^{\Theta(n)} \otimes \ket{A}^{\Theta(n)} \rightarrow  \prod_{g \in \Lambda} T_{g} \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }
        \end{split}
      \end{equation*}
    \item Otherwise, the circuit computes the state 
      \begin{equation*}
        \begin{split}
      \mathcal{C} \ket{0}^{\Theta(n)} \otimes \ket{A}^{\Theta(n)} \rightarrow Z^{e} \ \ \ \prod_{g \in \Lambda} T_{g} \ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }
        \end{split}
      \end{equation*}
...
\begin{claim}
  ...
  \begin{equation*}
    \begin{split}
      \prb{ |e| > (1+\alpha) \expp{|e|} } < \frac{\xi_{\Delta}\zeta_{\Delta}p(1-\zeta_{\Delta}p)}{\alpha^{2}n} 
    \end{split}
  \end{equation*}
\end{claim}
\begin{proof}
By the Chebyshev bound, notice that the number for which $\expp{e_{i}e_{j}}-\expp{e_{i}}\expp{e_{j}} \neq 0$ is less than $\xi_{\Delta}n$.
\end{proof}
