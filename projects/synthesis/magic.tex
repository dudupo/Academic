...
\begin{proof}
  ...
  For any $v \in V$ denote by $\ket{z_{v}}$ the superposition of $C_{0}$ codewords supported by $v$ local view, and for any subset of vertices $W \subset V$ denote by $\ket{z_{W}} = \ket{\sum_{v \in W} z_{v}}$. Using the notation we have that applying at once all the encoders $E_{v}$ over the positive vertices result into:  
  \begin{equation*}
    \begin{split}
