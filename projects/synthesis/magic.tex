...
\begin{claim}
  There is a family of quantum circuits $\mathcal{C}$ consists of Clifford gates and at most $o(\sqrt{n})$ number of $T$ gates, and produce $\Theta(n)$ $T$ states. 
\end{claim}
\begin{proof}
  Chain recursively the protocol in \Cref{Claim:flift} for $\log_{(1 + \frac{1}{3})}{2}$ times.
\end{proof}
