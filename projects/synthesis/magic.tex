...
\begin{claim}
  Let $G$ be a $\Delta$-regular bipartite graph. And denote by $C$ the dual-tanner code $\mathcal{T}(G,C_{0})^\perp$. Then there is circuit that compute the state $\ket{C}$ and has constant propagation factor. 
  %Suppose that $G_{}
\end{claim}
  ...
  \begin{equation*}
    \begin{split}
      \sum_{v \in V^{+}}\ket{z_{v}} \otimes \sum_{u\in V^{-}}{\ket{z_{u}}} & \mapsto  \sum_{\substack{v \in V^{+} \\ u \in V^{-}}}\ket{z_{v} + z_{u}} {\ket{z_{u}}}\\
        & \mapsto_{H_X}\sum_{\substack{v \in V^{+} \\ u \in V^{-}}}\ket{z_{v} + z_{u}} {0}
    \end{split}
  \end{equation*}
\end{proof}
