...
\begin{claim}
  Consider the Left-Right ($\Delta$,$n$)-Complex $\Gamma$ and let  $C_{0} = C_{A}\otimes C_{B}^{\perp} \oplus C_{A}^{\perp} \otimes C_{B}$. Define $\Lambda$ to be the code $\mathcal{T}(V_{+}, C_{0} ) / C_{Z}^\perp$. Then: 
  \begin{enumerate}
    \item $ \Lambda \in C_{Z} \cap C_{X}$ 
    \item $\Lambda$ has a positive rate. 
  \end{enumerate}
\end{claim}

\begin{proof}
  It's easy to see that $C_{0} \subset C_{A}\otimes \mathbb{F}^{B} \oplus \mathbb{F}^{A}\otimes C_{B}$. And therefore $\Lambda \subset C_{X}$. 
\end{proof}
