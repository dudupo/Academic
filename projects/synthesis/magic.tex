...
Let $G^{\prime}$ be a graph constructed as described above. Consider the code over the $\mathbb{F}_{q}$ alphabet, defined as all the assignments of symbols from $\mathbb{F}_{q}$ to the $\Delta \times L$ vertices. Such any vertex on the right side of $G$ sees a polynomial of degree at most $d$ on its local view.
