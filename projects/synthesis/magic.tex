...
\begin{claim}
  Let $G$ be a $\Delta$-regular bipartite graph. And denote by $C$ the dual-tanner code $\mathcal{T}(G,C_{0})^\perp$. Then there is circuit that compute the state $\ket{C}$ and has constant propagation factor, $\log \Delta$ depth and $\Delta^{2}n$ ancillary.   
  %Suppose that $G_{}
\end{claim}
