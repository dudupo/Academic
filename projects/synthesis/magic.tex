...
\end{claim}
\begin{proof}
  Let $\tau \in \sMbas + C_{Z}^\perp$, applying $T^{(w)}$ on $\ket{\tau}$ add a phase of $i\frac{\pi}{4}\left|\tau\right|_{w}$. Notice that $\tau$ can decompose to the sum of  $x_{0} + \sum_{y_{i} \in \mathcal{X}}{X_{y_{i}}y_{i}} + \sum_{z_{i} \in \text{ base }C_{Z}^{\perp}}{X_{z_{i}}z_{i}}$ when $y \in \text{ span } \mathcal{X}$ and $z\in C_{Z}^{\perp}$, so 
