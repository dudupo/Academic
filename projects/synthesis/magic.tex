...
\begin{claim} 
  Suppose that a set of vectors $\Lambda \subset C$ satisfies the relation for any $x,y,z \in \Lambda$ $xy =0$ and $xyz =0$, then there is a code $C^{\prime}$ with code length roughly equals to $C$ with $\Lambda^{\prime} \subset C^{\prime}$ such that for any differ $x,y,z \in \Lambda^\prime$ $xy =0$, $xyz =0$ but also $xx =_{4} 1$. 
