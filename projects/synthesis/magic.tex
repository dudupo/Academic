...
\begin{claim}
  The state $\ket{ C_{Z}^{\perp} +  \text{ span } \Lambda   }\ket{0}$ can be computed, such that the light cone depth of any non-clifford gate is bounded by constant.    
\end{claim}
\begin{proof}
\begin{align*}
...
\begin{claim}
  Denote by $p \in [0,1]$ the error rate of input magic states, denote by $\ket{A}$ an ancila initilaized to a magic state. There are constant numbers $\zeta_{\Delta},\xi_{\Delta}$, and a circuit $\mathcal{C}$ such that: 
  \begin{enumerate}
    \item $\mathcal{C} \ket{0}^{\Theta(n)} \otimes \ket{A}^{\Theta(n)}$ 
  \end{enumerate}<++>
\end{claim}
