...
The idea form here, set $C_{Z}$ to be the above code, and $C_{X}^{\perp} = \emptyset  $, clearly $C_{X}^{\perp} \subset C_{Z}$. Now, construct $\Lambda$ by the method in section 6.The additional phase that we get by applying the gate $T^{n}$ corresponds to controlled-S, controlled-controlled-Z between the generators of $C_{Z}^{\perp}$. So one ca fix them by applying at most $\Theta( (\gamma n^{\frac{1}{3}})^{3})$ perfect $T$ gates. 
