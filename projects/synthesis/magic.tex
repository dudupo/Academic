...
\begin{claim} 
  \label{claim:commute}
  Let $C$ be a $[n,k,d]$ binary linear code, and let $\Lambda$ be subcode $\Lambda \subset C$ at dimension $k' > \alpha k$ for some $\alpha\in(0,1)$ and distance $d^{\prime}$. Then there exists a code $C' = [\leq 2n, \geq k-k'/2, d]$ and a subcode of it $\Lambda' $ in it at dimension $\ge k^{\prime}/2$ and distance $d^{\prime}$, such:
  \begin{enumerate}
      \label{property:prop}
