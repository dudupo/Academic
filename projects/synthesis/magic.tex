...
Denote by $E$ the circuit that encodes the logical $ith$ bit to $y_{i}$, by $T^{(w\cap x_{0})}$ the application of
...
\begin{claim}
  ...
  \begin{equation*}
    \begin{split}
      T^{(w\cap x_{0})}\ket{\Mbas + C_{Z}^{\perp}} \propto E \ \mathcal{C} \ \left(TH\right)^{\rho\left(\sMbas\right)n} \ket{0} 
    \end{split}
  \end{equation*}
\end{claim}
\begin{proof}
  Let $\tau \in \sMbas + C_{Z}^\perp$, applying $T^{(w \cap x_{0})}$ on $\ket{\tau}$ add a phase of $i\frac{\pi}{4}\left|\tau\right|_{w \cap x_{0}}$. Notice that $\tau$ can decompose to the sum of  $x_{0} + y + z$ when $y \in \text{ span } \mathcal{X}$ and $z\in C_{Z}^{\perp}$, so 

  \begin{equation*}
    \begin{split}
  \left|\tau\right|_{w\cap x_{0}} &= \left| x_{0} + y _ z  \right|_{w\cap x_{0}} \\
  &= |x_{0}|_{w\cap x_{0}} + |y|_{w\cap x_{0}} +|z|_{w\cap x_{0}} - 2| x \cdot y |_{w\cap x_{0}} - 2| x \cdot z |_{w\cap x_{0}} -2| z \cdot y |_{w\cap x_{0}} + 4|x_{0}\cdot y \cdot z|_{w\cap x_{0}}  \\
  &= |x_{0}|_{w\cap x_{0}} + |y|_{w\cap x_{0}} +|z|_{w\cap x_{0}} - 2| x \cdot y |_{w\cap x_{0}} - 2| x \cdot z |_{w\cap x_{0}} -2| z \cdot y |_{w\cap x_{0}} + 4|y \cdot z|_{w\cap x_{0}}  
    \end{split}
  \end{equation*}
  Since we picked $\tilde{C}_{0} \in C_{0}^{\perp}$ then $y \cdot z|_{w\cap x_{0}} = 0$ $\Rightarrow |y \cdot z|_{w\cap x_{0}}|$ is even.
\end{proof}
