...
For $\Delta = 4^{c}$, $\Delta < q$, We have that 
...
\begin{equation*}
  ...
  Let $B$ be the bucket which matches $\{2,3,..\Delta-1\}$. On that bucket, the multiplication of the checks corresponds to $\sum_{x\in \mathbb{F}_{\Delta}}x^{i} - 1^i$, which is $\in \{ -1, \Delta/2 - 1 \}$. On the otherhand, the codeword $\xi$ that corresponds to the linear function $f(x)= x$ on every bucket gives $\xi \cdot \xi =_{\Delta} -1$. 
