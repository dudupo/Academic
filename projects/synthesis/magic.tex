...
\begin{claim}
  Consider the Left-Right ($\Delta$,$n$)-Complex $\Gamma$ and let  $C_{0} = C_{A}\otimes C_{B}^{\perp} \oplus C_{A}^{\perp} \otimes C_{B}$. Then the Tanner code $\mathcal{T}(V_{+}, C_{0} )$ is:  
  \begin{enumerate}
