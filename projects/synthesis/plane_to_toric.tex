...
\begin{claim}
  Assume that until step $\tau$, there was no a deviation cluster of size greater than $d$.  Let $\hat{K}$ be a spanned slice induced by a deviation of a cluster state at step $\tau$ and let $\omega$ be an explanation for $\hat{K}$ such that the number of edges in $w$ is greater than $\beta n$. Then there is a subgraph (subtree) $\omega^{\prime}$ of $\omega$ such that $\omega^{\prime}$ has less than $\frac{\beta}{2}n$ edges and at least $\frac{\beta}{4\gamma} n$ leaves.
\end{claim}
