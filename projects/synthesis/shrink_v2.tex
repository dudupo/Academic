...
\begin{claim}
  Let $z$ be an assignment. There is $\tilde{z} \sim_{\mathcal{C}_{X}} z$ such that for any vertex $v \in \tilde{z}$, which is a local maximum of $L_{d+1}$ in $\partial \tilde{z}$, it holds that $v$ is also a local maximum of $L_{d+1}$ in $\tilde{z}$.
\end{claim}
\begin{proof}
  
Let $v$ be a vertex in $z/ \parital z$ such that $v$ is a local maximum of $L_{d+1}$ in $z$. By definition, since $v \notin \parital z$, we have $(\partial z|_{v})|_{v} = 0$, and since $v$ is a local maximum of $L_{d+1}$ in $z$, it holds that $z|_{v+} = z|_{v}$. Combining the two, we get that $z|_{v}$ is a codeword of the small code at $v$, namely $z|_{v} \in \mathcal{C}_{v}$.
...
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\parital z = \parital \tilde{z}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. We say that the \textit{termination-condition} holds if: For any vertex $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\parital \tilde{z}$ it holds that 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) \le \min_{v \in \parital z} L_{d+1}(v)
  \end{split}
\end{equation*}
Observes that if the \textit{termination-condition} holds then any local maximum of $L_{d+1}$ in $\parital z$ is also a local maximum of $L_{d+1}$ in $\tilde{z}$ and we finish. Consider the other case, namely there is at least one vertex $u \in \tilde{z}/\parital z$ such that $u$ is a local maximum for $L_{d+1}$ and 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) > \min_{v \in \parital z} L_{d+1}(v)
  \end{split}
\end{equation*}
...
%\begin{algorithm}[H] 
%  $\gamma \leftarrow \min_{v \in \parital z} L_{d+1}(v)$\\ 
