...
\begin{claim}
Suppose that $v$ is a local maximum of $L_{d+1}$. Then the syndrome is supported only on the positive edges of $v$.
\end{claim}

\begin{claim}
  Suppose that $v$ is a local max of $L_{d+1}$, then there is $S_{1},..,S_{k}$ such that $\partial \cup_{i \in [k]} \theta_{v,S_{i}} = \partial_{v}$. 
\end{claim}
