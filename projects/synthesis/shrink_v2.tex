...
For a quantum circuit $C$ and a set of faults $\mathcal{E}$, denote:
  \begin{equation*}
    \begin{split}
      \rho & \colonequals C[\mathcal{E}] \ketbra{0}{0}\bra{0} C[\mathcal{E}]^{\dagger} \\
      \rho_{\infty} & \colonequals C^{\infty}[\mathcal{E}^{\infty}]\ket{0}\bra{0}\left(C^{\infty}[\mathcal{E}^{\infty}]\right)^{\dagger}
    \end{split}
  \end{equation*}
We say that a quantum circuit $C$ is \textit{$\infty$-compatible} if, for any set of faults $\mathcal{E} = \{ f_{i} \}$, it holds that:
  \begin{equation*}
    \begin{split}
      \Tr{}\left( A \rho \right) =  \Tr{}_{/[w]}\left( A \rho_{\infty} \right) 
    \end{split}
  \end{equation*}
\end{definition}
