...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Thus if  $g \notin \cup S^{\prime}_{i}$, Then: 
\begin{equation*}
  \begin{split}
    \rotfv{gv}{S^{\prime}_{i}/g}  \in \partial \phi_{v}|_{gv} 
  \end{split}
\end{equation*}
So for $\rotfv{gv}{S^{\prime}_{i}/g} = 0$, it must appear at least twice in the sum, yet the left terms in the sum cannot contain it (they differ by the root vertex), and if there is a right term $\rotfv{gv}{S^{\prime}_{j}/g} = \rotfv{gv}{S^{\prime}_{i}/g}$, then it implies that $S_{j} = S_{i}$.
\end{proof}
\newpage
