...
\begin{equation*}
  \begin{split}
     ...
     \text{apply } H^{\otimes n } \mapsto & (-1)^{(e+\hat{e}) (f+w) } (-1)^{e \cdot w} X^{f}X^{w}Z^{\hat{e}}\ket { C_{z} } \\
     = & (-1)^{(e+\hat{e}) (f+w) } (-1)^{e \cdot w} (-1)^{\hat{e} \cdot w} X^{f}Z^{\hat{e}}X^{w}\ket { C_{z} } \\
     = & (-1)^{(e+\hat{e}) f } X^{f}Z^{\hat{e}}X^{w}\ket { w + C_{z}^{\perp}  } \\
  \end{split}
\end{equation*}
