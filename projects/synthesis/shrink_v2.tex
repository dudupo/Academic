...
In addition, for any $j \neq i$, it follows that $v_{j} \notin w_{i}$. Otherwise, there exists $S^{\prime} \subset S$ such that $v_{j} \in \rotf{v_{i}}{S^{\prime}}$, and therefore $L_{d+1}(v_{j}) > L_{d+1}(v_{i})$, in contradiction to $v_{i}$ and $v_{j}$ being both global maximum points for $L_{d+1}$ in $\partial^{\pm} z$.
  Thus we have $\tilde{z}|_{v_{i}} = (z + w_{i})|_{v_{i}} = 0$ namely $v_{i} \notin \tilde{z}$, Hence we get: 
\begin{equation*}
  \begin{split}
