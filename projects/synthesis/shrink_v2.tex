...
\end{claim}
\begin{proof}
  We will show that codes obtained from $\text{Stab}_{v}$ and $\text{Stab}_{v}^{*}$ by forcing the faces rooted at $v$ to $z|_{v}$ have non-zero dimensions. Then it follows that any code word of $\mathcal{C}_{v}$ could be extend to codewords in $\text{Stab}_{v}$ and $\text{Stab}_{v}^{*}$. The number of bits in both $\text{Stab}_{v}$ and $\text{Stab}_{v}^{*}$ is:
\begin{equation*}
  \begin{split}
