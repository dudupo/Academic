...
\begin{claim}
  \label{claim:contain}
  Let $I \subset [d]$. Suppose that $v$ is a \inb{ (strong) }local maximum of $L_{I}$. Then, for any $I^{\prime} \subset I$, it holds that $v$ is also a \inb{ (strong) }local maximum of $L_{I^{\prime}}$. 
\inb{ Rewrite it. }
\end{claim}
...
Assume, toward contradiction, that $v$ has a neighbor $u$ such that $L_{I^{\prime}}(u) > L_{I^{\prime}}(v)$. By neighborhood, $v$ and $u$ differ in exactly one coordinate. Thus, for any $i \not\in I^{\prime}$, we have $v_{i} = u_{i}$. So,
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  In contradiction to $v$ be a \inb{ (strong) }local maximum of $L_{I}$. 
\end{proof}

\begin{claim}
  A vertex $v$ can not be simultaneously both a local maximum of $L_{x}$ and a \inb{ strong } local maximum of $L_{\{x\}}$.
\end{claim}
\begin{proof}
