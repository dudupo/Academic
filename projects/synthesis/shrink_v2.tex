...
\end{claim}
\begin{proof}
  First, since the Sweep rule is invariant to an addition by stabilizers, for any $\tilde{z}$ such that $\tilde{z} \sim_{C_{X}} z$, the correction applied by the Sweep rule on $\tilde{z}$ is the same as the application on $z$. Let $\tilde{\xi} = \xi + z + \tilde{z}$, namely, $\tilde{\xi}$ is obtained from $\xi$, by shifting it by the same stabilizer shift between $z$ and $\tilde{z}$. Thus
  \begin{equation*}
    \begin{split}
