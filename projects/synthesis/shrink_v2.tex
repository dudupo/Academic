...
Let $v$ be a vertex in $z/ \partial z$ such that $v$ is a local maximum of $L_{d+1}$ in $z$. By definition, since $v \notin \partial z$, we have $(\partial z|_{v})|_{v} = 0$, and since $v$ is a local maximum of $L_{d+1}$ in $z$, it holds that $z|_{v+} = z|_{v}$. Combining the two, we get that $z|_{v}$ is a codeword of the small code at $v$, namely $z|_{v} \in \mathcal{C}_{v}$.
