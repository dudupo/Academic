...
Consider a subset of $d$-facets $z \subset \mathcal{F}_{d}$ and a vertex $v \in \mathcal{G}_{n}^{d}$. We say that $z$ is a \textit{rooted assignment} at vertex $v$ if there are subsets $S_{1}, \ldots, S_{k} \subset S^{\times k}$ such that $z$ can be decomposed as:
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  Put differently, $z$ is the result of taking a collection of $d$-facets that are rooted at $v$.
\end{definition}

\begin{definition}
  Let $\sigma^{(t)}$ be a domain wall at time $t$, and let $\Gamma^{t} = \{ v \colon \exists \{v,u\} \in \sigma^{t} \}$.

\end{definition}
\inb{below, define what is it $\Gamma$}
\begin{definition}[Sweep Rule]
  The \textit{sweep rule decoding procedure} at vertex $v \in \mathcal{G}_{n}^{d}$ defining as follow: 
  \begin{enumerate}
    ...
    \item Then, check if there is a rooted assignment $\tilde{z}$ at $v$ such that $\tilde{z}_{v} = z_{v}$. If there is, then flip it.
  \end{enumerate}
...
Suppose that $d^{\prime} > 1$ and a vertex $v \in \mathcal{G}^{d}_{\infty}$ is also adjacent to the domain wall, namely $v \in \Gamma$. It holds that $v$ cannot be simultaneously both a (strict) local max of $L_{d+1}$ and a local min of $L_{d+1}$.
\end{claim}
...
\begin{claim}
  \label{claim:contain}
  Let $I \subset [d]$. Suppose that $v$ is a \inb{ (strict) }local maximum of $L_{I}$. Then, for any $I^{\prime} \subset I$, it holds that $v$ is also a \inb{ (strict) }local maximum of $L_{I^{\prime}}$. 
\inb{ Rewrite it. }
\end{claim}
...
Assume, toward contradiction, that $v$ has a neighbor $u$ such that $L_{I^{\prime}}(u) > L_{I^{\prime}}(v)$. By neighborhood, $v$ and $u$ differ in exactly one coordinate. Thus, for any $i \not\in I^{\prime}$, we have $v_{i} = u_{i}$. So,
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  In contradiction to $v$ be a \inb{ (strict) }local maximum of $L_{I}$. 
\end{proof}

\begin{claim}
  A vertex $v$ can not be simultaneously both a local maximum of $L_{x}$ and a \inb{ strict } local maximum of $L_{\{x\}}$.
\end{claim}
\begin{proof}
  Assume towards contradiction that there are a vertex $v$ and a coordinate $x$ such $v$ is both a strict local maximum to $L_{\{x\}}$ and a local maximum to $L_{x}$. Thus for any $u\sim v$ we have both:
  \begin{equation*}
    \begin{split}
      u_{x} &> v_{x} \\
      u_{x} & \le v_{x}
...
\begin{claim}
  There is no a coordinates subset $I \subset [d]$, a coordinate $x \in I$, and a vertex $v$ such $v$ is both strict local maximum to $L_{\{x\}}$ and a local maximum to $L_{x}$. 
\end{claim}
\begin{proof}
...
\begin{claim}
  Denote by $\mathcal{S}$ the generators $\mathcal{S} = \cup_{i\in[d]}\{ \mathbf{1}_{i} \}$. Let $v$ be a vertex in $\mathcal{G}^{d}_{\infty}$. For $I \subset [d]$ at size $|I| \ge d^{\prime}$, Suppose that $v$ is a (strict) local maximum of $L_{I}$. Then there is a subset of $d$-facets $w$, supported only on generators in $I$ such that:
  \begin{equation*}
    \begin{split}
      z|_{v}  & = \left( \partial w \right)|_{v} \\
      & \sum_{S^{\prime} \in J} \theta_{v,S^{\prime}}
    \end{split}
  \end{equation*}
\end{claim}
\begin{proof}
  As exectly as in the proof of  \Cref{claim:extermal}, there is $\theta_{v,S^{\prime}}$. \inb{ Substitute it from the local view, We should show that the weight decrease.} 
\end{proof}
