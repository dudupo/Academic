...
\begin{definition}
  Let $n$ be an integer, and let $C = \{u_{i} \}$ be a quantum circuit such that each of its registers $\matchal{R} = R_{1},..,R_{k}$ is at length at most $n$. We define the \textit{$\infty$-extension of $C$}, denoted by $C^{\infty}$, as follows:
  \begin{enumerate}
    \item Each register $R_{i}$ of $C$ is mapped to a register $R^{\prime}_{i}$ with infinite length. If $R_{i}$ is an encoded register by code $\mathcal{C}$, then a check $c_{x} = (x_{1},..,x_{\Delta})$ of the code is mapped to the collection of checks: 
      \begin{equation*}
        \begin{split}
