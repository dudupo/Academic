...
Suppose not, namely $u \notin \Gamma$. Then it means that any $d^{\prime}-1$ face containing $u$ is satisfied. Observe that $\sum \theta_{u,J_{i}} \neq 0$; otherwise, it would contradict the non-independence. So, by \Cref{claim:syn_edge} there exists a $d^{\prime}-1$ face rooted at $u$, contained in $\sum \theta_{u,J_{i}}$, denote it by $\theta_{u,J^{\prime}}$.
