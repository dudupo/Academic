...
For a vertex $v$, we define the \textit{small code at vertex} $v$ to be the binary assignments over the bits ($d^{\prime}$ faces) rooted at $v$ that satisfy the $\mathcal{C}_{X}$'s restrictions rooted at $v$ ($d^{\prime}-1$ faces). We denote by $\mathcal{C}_{v}$ the small code at $v$. By definition, if $v \notin \Gamma_{z}$ and $v$ is a local maximum of $L_{d+1}$ in $z$, then $z|_{v+} = z|_{v} \in \mathcal{C}_{v}$.

For $i,j \in [d]$, such that $i \neq j$, denote the bit rooted at $v$ and spanned by generators $g_{i}, g_{j} \in S$, namely $\theta_{v,\{ g_{i},g_{j} \}}$, by $g_{ij} = g_{ji}$. On one hand, for any stabilizer $w$ rooted at $v$, we have $(\partial w)|_{v} = 0$; particularly, $(\partial w)|_{v+} = 0$, and therefore $w|_{v} \in \mathcal{C}_{v}$. We claim that, on the other hand, $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$.
  The restrictions: 
  \begin{equation*}
    \begin{split}
      r_{i} \colon \sum_{j\neq i} g_{ij}
