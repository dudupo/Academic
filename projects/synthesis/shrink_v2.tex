...
\begin{definition}[Rooted Assignment at $v$]
  Consider a subset of $d$-faces $z \subset \mathcal{F}_{d}$ and a vertex $v \in \mathcal{G}_{n}^{d}$. We say that $z$ is a \textit{rooted assignment} at vertex $v$ if there are subsets $S^{\prime}_{1}, \ldots, S^{\prime}_{k} \subset S^{\times k}$ such that $z$ can be decomposed as:
  \begin{equation*}
    \begin{split}
      z = \sum_{i}^{k}\theta_{v,S^{\prime}_{i}}
    \end{split}
  \end{equation*}
  Put differently, $z$ is the result of taking a collection of $d^{\prime}$-faces that are rooted at $v$.
\end{definition}

\begin{definition}[Front View at $v$]
Consider a subset of $d$-faces $z \subset \mathcal{F}_{d}$ and a vertex $v$. We define the \textit{front view at} $v$ to be all the unsatisfied checks rooted at $v$. We denote it by $z_{v}$.
...
\begin{definition}[Decomposition Form]
   Let $z$ be a connected assignment on the $d^{\prime}$-faces at finite size, and let $\theta_{v_{1},S^{\prime}_{1}}, \theta_{v_{2},S^{\prime}_{2}},.. \theta_{v_{k},S^{\prime}_{k}}$ be the collection of $d$-faces forms $z$, namely:  
  \begin{equation*}
    \begin{split}
