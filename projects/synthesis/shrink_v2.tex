...
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\Gamma_{z} = \Gamma_{\tilde{z}}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. If there are no more vertices $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\Gamma_{\tilde{z}}$, then we finish. Else, we peak such $u$, and repeat on the above where $u$ plays the role of $v$. 

We argue that the process terminates, assume that $u \in \tilde{z}/z$, meaning that $u \in w|_{v}$ which implies that there is a non empty subset $I \subset S$ such that $u = (\prod_{g \in I}g)v$, and therefore $L_{d+1}(u) \ge 1 + L_{d+1}(v)$.
