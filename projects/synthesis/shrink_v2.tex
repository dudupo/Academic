...
For $i,j \in [d]$, such that $i \neq j$, denote the bit rooted at $v$ and spanned by generators $g_{i}, g_{j} \in S$, namely $\theta_{v,\{ g_{i},g_{j} \}}$, by $g_{ij} = g_{ji}$. On one hand, for any stabilizer $w$ rooted at $v$, we have $(\partial w)|_{v} = 0$; particularly, $(\partial w)|_{v+} = 0$, and therefore $w|_{v} \in \mathcal{C}_{v}$. On the other hand, we claim that, $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$. Then, since $z|_{v} \in \mathcal{C}|_{v}$, it follows, that there is must to be a stabilizer $w \in \text{Stab}_{v}$ such $w$ and $z$ agree on the positives faces of $v$ namely $ w|_{v} = z|_{v}$. We prove that $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$ below, for now assume it corrects.

 
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\Gamma_{z} = \Gamma_{\tilde{z}}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. If for any vertex $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\Gamma_{\tilde{z}}$ it holds that 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) \le \min_{v \in \Gamma_{z}} L_{d+1}(v)
  \end{split}
\end{equation*}
then any local maximum of $L_{d+1}$ in $\Gamma_{z}$ is also a local maximum of $L_{d+1}$ in $\tilde{z}$ and we finish. Consider the other case, namely there is at least one vertex $u \in \tilde{z}/\Gamma_{z}$ such that $u$ is a local maximum for $L_{d+1}$ and 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) > \min_{v \in \Gamma_{z}} L_{d+1}(v)
  \end{split}
\end{equation*}
. We peak such $u$, and looking for stabilizer $w$ such $u \notin \tilde{z} + w$ by repeating over the above procedure where $u$ plays the role of $v$. We keep repeating on the procedure until 


We argue that the process terminates, assume that $u \in \tilde{z}/z$, meaning that $u \in w|_{v} / v$ which implies that there is a non empty subset $I \subset S$ such that $u = (\prod_{g \in I}g)v$, and therefore $L_{d+1}(u) \le -1 + L_{d+1}(v)$. 

It's left to prove that $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$. The restrictions: 
  \begin{equation*}
    \begin{split}
      r_{i} \colon \sum_{j\neq i} g_{ij}
    \end{split}
