...
\inb{Add a proof for $\partial \rotfvs \cdot \partial^{*} \rotfv{u}{S^{\prime\prime}} = 0 $. }
...
\begin{definition}[Sweep Rule]
  Let $z$ be an assignment over the $d^\prime$-faces, The \textit{sweep rule decoding procedure} at vertex $v \in \mathcal{G}_{n}^{d}$, for the $X$ codes, defining as follow: 
  \begin{enumerate}
    \item Look for a rooted assignment $\tilde{z}$ at $v$ such that $(\partial\tilde{z})|_{v+} = \partial(z)|_{v+}$. Namely, the checks rooted at $v$ get the same syndromes for $z$ and $\tilde{z}$. If there is one, then flip $\tilde{z}$.
  \end{enumerate}
\end{definition}
