...
\begin{definition}[$d$-Dimensional Plaquettes Toric Complex] 
  ...
  \begin{equation*}
    \begin{split}
      \theta_{v,S^{\prime}} \colonequals \bigcup_{I \subset S^{\prime}} \left\{  \left(\prod_{g \in S^{\prime}} g \right) v  \right\}
    \end{split}
  \end{equation*}
...
\begin{definition}[Plaquettes Code] \cite{Dennis_2002}
  Let $d$ and $n$ be integers and let $G = (V,E,F)$ be the $d$-dimensional plaquettes toric complex obtained from $\mathcal{G}_{n}^{d}$. We define the \textit{plaquettes code}, denoted by $\mathcal{C}$, to be the quantum code, constructed as follow: We assosicate the (q)bits with plaquettes of $G$, the $X$-checks with edges, and the $Z$-checks with the cubes.  our code, bits are set on the plaquette, and the edges correspond to checks that restrict the parity of the bits on their support to be even.
\end{definition}
