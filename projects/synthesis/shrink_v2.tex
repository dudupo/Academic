...
\begin{claim}
  Let $z$ be a finite assignment. Suppose that $\arg \max_{v \in z} L_{d+1} \notin \partial z$, Then there is $\tilde{z} \sim_{\mathcal{C}_{X}} z$ such that $ \max_{v \in \tilde{z}} L_{d+1} < \max_{v \in z} L_{d+1} - 1$.
\end{claim}
\begin{proof}
  
  Let $v$ be a vertex in $z$ which gets the maximum for $L_{d+1}$, namely $v = \arg \max_{v\in z}L_{d+1}(v)$, and assume that that $v \notin \partial z$. By definition, since $v \notin \partial z$, we have $(\partial z|_{v})|_{v} = 0$, and since $v$ is a maximum point of $L_{d+1}$ in $z$, then it also a local maximum, and therefore $z|_{v+} = z|_{v}$. Combining the two, we get that $z|_{v}$ is a codeword of the small code at $v$, namely $z|_{v} \in \mathcal{C}_{v}$.

On one hand, for any stabilizer $w$ rooted at $v$, we have $(\partial w)|_{v} = 0$; particularly, $(\partial w)|_{v+} = 0$, and therefore $w|_{v} \in \mathcal{C}_{v}$. On the other hand, we claim that, $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$. Then, since $z|_{v} \in \mathcal{C}|_{v}$, it follows, that there is must to be a stabilizer $w \in \text{Stab}_{v}$ such $w$ and $z$ agree on the positives faces of $v$ namely $ w|_{v} = z|_{v}$. We prove that $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$ below, for now assume it is correct.

 
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\partial z = \partial \tilde{z}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. We say that the \textit{termination-condition} holds if: For any vertex $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\partial \tilde{z}$ it holds that 
