...
\end{claim}
\begin{proof}
  We will show that codes obtained from $\text{Stab}_{v}^{-}$ and $\text{Stab}_{v}^{+}$ by forcing the faces rooted at $v$ to $z|_{v}$ have non-zero dimensions. Then it follows that any code word of $\mathcal{C}_{v}^{\pm}$ could be extend to codewords in $\text{Stab}_{v}^{-}$ and $\text{Stab}_{v}^{+}$. The number of bits in both $\text{Stab}_{v}^{\pm}$ is:
\begin{equation*}
  \begin{split}
    \overbrace{ { d \choose d^{\prime} } }^{ \text{ bits of the form }  \rotfv{v}{S^{\prime}/g} }   +\overbrace{d { d - 1 \choose d^{\prime} } }^{\text{ bits of the form } \rotfv{gv}{S^{\prime}/g}}    
  \end{split}
\end{equation*}
The numbers of checks for $\text{Stab}_{\pm}$ are:
\begin{equation*}
  \begin{split}
    \overbrace{{ d \choose d^{\prime} \pm 1 }}^{  \text{ checks of the form } }  +\overbrace{d { d-1  \choose d^{\prime} \pm 1 }}^{ \text{checks of the form   } }    
 \end{split}
\end{equation*}
