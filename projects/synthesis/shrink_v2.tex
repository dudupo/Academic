...
For $v,u \in \mathcal{G}_{n}^{d}$ and $g_{1} \neq g_{2} \in S$, consider the plaquette $\theta = \{v, g_{1}v, g_{2}g_{1}v, g_{2}v\}$. For $x, y \in \mathcal{G}_{n}^{d}$, we say that an edge $\{x, y\}$ is \textit{positive relative to a plaquette} $\theta$ if $x = v$ and $y$ is either $g_{1}v$ or $g_{2}v$. When the plaquette is clear from the context, we will abbreviate and say that $\{x, y\}$ is \textit{positive}.
