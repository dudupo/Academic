...
For a vertex $v$, we define the \textit{small code at vertex} $v$ to be the binary assignments over the bits ($d^{\prime}$ faces) rooted at $v$ that satisfy the $\mathcal{C}_{X}$'s restrictions rooted at $v$ ($d^{\prime}-1$ faces). We denote by $\mathcal{C}_{v}$ the small code at $v$. By definition, since $v \notin \Gamma_{z}$ and $v$ is a local maximum of $L_{d+1}$ in $z$, it holds that $z|_{v+} = z|_{v} \in \mathcal{C}_{v}$.
...
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\Gamma_{z} = \Gamma_{\tilde{z}}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. Let $v^{*}$ be a vertex which is a local maximum of $L_{d+1}$ in $\Gamma_{z}$. 

If for any vertex $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\Gamma_{\tilde{z}}$ it holds that 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) \le \min_{v \in \Gamma_{z}} L_{d+1}(v)
  \end{split}
\end{equation*}
then we finish. Else, we peak such $u$, and repeat over the above where $u$ plays the role of $v$. 
