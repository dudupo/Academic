...
\begin{definition}[Sweep Rule]
  ...
  \begin{enumerate}
    ...
    \itemFor the $\mathcal{C}_{Z}$ code. Look for a rooted assignment $\tilde{z}$ at $v$ such that $(\partial^{*}\tilde{z})|_{v+} = \partial^{*}(z)|_{v+}$. Namely, the $Z$-checks rooted at $v$ get the same syndromes for $z$ and $\tilde{z}$. If there is one, then flip $\tilde{z}$.
