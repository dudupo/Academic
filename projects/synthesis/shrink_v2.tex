...
\inb{
Write a remark indicating that when $d^\prime = 1$, meaning qubits are on the edges (2D toric), the claim is not true.
}
\end{remark}
...
\begin{claim}
  \label{claim:contain}
  Let $I \subset [d]$. Suppose that $v$ is a  (strong) local maximum of $L_{I}$. Then, for any $I^{\prime} \subset I$, it holds that $v$ is also a  (strong) local maximum of $L_{I^{\prime}}$.  \
\inb{ Rewrite it. }
\end{claim}

\begin{proof}
Assume, toward contradiction, that $v$ has a neighbor $u$ such that $L_{I^{\prime}}(u) > L_{I^{\prime}}(v)$. By neighborhood, $v$ and $u$ differ in exactly one coordinate. Thus, for any $i \not\in I^{\prime}$, we have $v_{i} = u_{i}$. So,
  \begin{equation*}
    \begin{split}
      L_{I}(u) &= L_{I^{\prime}}(u) - u_{l} >  L_{I^{\prime}}(v) - u_{l} = L_{I^{\prime}}(v) - v_{l} = L_{I}(v) \\
      L_{I}(u) &= L_{I^{\prime}}(u) - u_{l} \inb{  \ge  }  L_{I^{\prime}}(v) - u_{l} = L_{I^{\prime}}(v) - v_{l} = L_{I}(v) 
    \end{split}
  \end{equation*}
  In contradiction to $v$ be a  (strong) local maximum of $L_{I}$. 
\end{proof}

\begin{claim}
  A vertex $v$ can not be simultaneously both a local maximum of $L_{x}$ and a  strong  local maximum of $L_{\{x\}}$.
\end{claim}
\begin{proof}
  Assume towards contradiction that there are a vertex $v$ and a coordinate $x$ such $v$ is both a strong local maximum to $L_{\{x\}}$ and a local maximum to $L_{x}$. Thus for any $u\sim v$ we have both:
  \begin{equation*}
    \begin{split}
      u_{x} &> v_{x} \\
      u_{x} & \le v_{x}
...
\begin{claim}
  There is no a coordinates subset $I \subset [d]$, a coordinate $x \in I$, and a vertex $v$ such $v$ is both strong local maximum to $L_{\{x\}}$ and a local maximum to $L_{x}$. 
\end{claim}
\begin{proof}
  Direct result of \Cref{claim:contain}.
\end{proof}
...
\begin{claim}
  Let $z$ be a connected assignment on the $d^{\prime}$-facet at finite size, and let $\Gamma$ be the set of vertices which are adjacent to unsatisfied checks induced by $\Gamma$. Let $v$ be a vertex in $\mathcal{G}^{d}_{\infty}$ such that $v$ is a strong global maximum of $L_{S}$, at $\Gamma$. Then there is a rooted assignment $\tilde{z}$ at $v$ such that $z_{v} = \tilde{z}_{v}$. 
\end{claim}
\begin{proof}
  Let $\theta_{v_{1},S^{\prime}_{1}}, \theta_{v_{2},S^{\prime}_{2}},.. \theta_{v_{k},S^{\prime}_{k}}$ be the collection of $d$-facets forms $z$, namely:  
  \begin{equation*}
    \begin{split}
