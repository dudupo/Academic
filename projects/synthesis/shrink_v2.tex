...
\begin{equation*}
  \begin{split}
     ...
     \text{ partial correction } X^{e +\hat{e}} \mapsto & (-1)^{(e+\hat{e}) (f+w) } (-1)^{e \cdot w} Z^{f}Z^{w}X^{e}\ket { C_{z} } \\
     \text{apply } H^{\otimes n } \mapsto & (-1)^{(e+\hat{e}) (f+w) } (-1)^{e \cdot w} X^{f}Z^{\hat{e}}X^{w}\ket { C_{z} } \\
     = & (-1)^{(e+\hat{e}) (f+w) } (-1)^{e \cdot w} X^{f}Z^{\hat{e}}X^{w}\ket { w + C_{z}^{\perp}  } \\
  \end{split}
\end{equation*}
