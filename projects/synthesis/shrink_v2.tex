...
Let $v$ be a vertex in $z/ \Gamma_{z}$ such that $v$ is a local maximum of $L_{d+1}$ in $z$. By definition, since $v \notin \Gamma_{z}$, we have $(\partial z|_{v})|_{v} = 0$, and since $v$ is a local maximum of $L_{d+1}$ in $z$, it holds that $z|_{v+} = z|_{v}$. Combining the two, we get that $z|_{v}$ is a codeword of the small code at $v$, namely $z|_{v} \in \mathcal{C}_{v}$.

On one hand, for any stabilizer $w$ rooted at $v$, we have $(\partial w)|_{v} = 0$; particularly, $(\partial w)|_{v+} = 0$, and therefore $w|_{v} \in \mathcal{C}_{v}$. On the other hand, we claim that, $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$. Then, since $z|_{v} \in \mathcal{C}|_{v}$, it follows, that there is must to be a stabilizer $w \in \text{Stab}_{v}$ such $w$ and $z$ agree on the positives faces of $v$ namely $ w|_{v} = z|_{v}$. We prove that $\dim \mathcal{C}_{v} = \dim \text{Stab}_{v}$ below, for now assume it corrects.

 
Set $\tilde{z} \leftarrow z + w$. Since $w$ is a stabilizer, we have $\partial \tilde{z} = \partial (z + w) = \partial z$, and hence $\Gamma_{z} = \Gamma_{\tilde{z}}$. Yet $\tilde{z}|_{v} = (z + w)|_{v} = 0$. Thus, $v \notin \tilde{z}$. We say that the \textit{termination-condition} holds if: For any vertex $u \in \tilde{z}$ such that $u$ is a local maximum of $L_{d+1}$ in $\tilde{z}$ but not a local maximum at $\Gamma_{\tilde{z}}$ it holds that 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) \le \min_{v \in \Gamma_{z}} L_{d+1}(v)
  \end{split}
\end{equation*}
Observes that if the \textit{termination-condition} holds then any local maximum of $L_{d+1}$ in $\Gamma_{z}$ is also a local maximum of $L_{d+1}$ in $\tilde{z}$ and we finish. Consider the other case, namely there is at least one vertex $u \in \tilde{z}/\Gamma_{z}$ such that $u$ is a local maximum for $L_{d+1}$ and 
\begin{equation*}
  \begin{split}
    L_{d+1}(u) > \min_{v \in \Gamma_{z}} L_{d+1}(v)
  \end{split}
\end{equation*}
...
\begin{algorithm}[H] 
  $\gamma \leftarrow \min_{v \in \Gamma_{z}} L_{d+1}(v)$\\ 
...
We repeat the process while possible; namely, we stop when all the local maxima of $L_{d+1}$ are in $\Gamma$.
\end{proof}

\inb{ 
We continue by using the fact that if, for a local maximum $v$ in $\Gamma$, it holds that $z|_{v}$ is rooted at $v$, then the decoder will find bits rooted at $v$ such that their $\partial$ agrees with $\Gamma$.
}
