...
\begin{definition}
  Let $n$ be an integer, and let $C = \{u_{i} \}$ be a quantum circuit such that each of its registers $\mathcal{R} = R_{1},..,R_{k}$ is at length at most $n$. We define the \textit{$\infty$-extension of $C$}, denoted by $C^{\infty}$, as follows:
  \begin{enumerate}
    \item Each register $R_{i}$ of $C$ is mapped to a register $R^{\prime}_{i}$ with infinite length. If $R_{i}$ is an encoded register by code $\mathcal{C}$, then a check $c_{x} = (x_{1},..,x_{\Delta}) \subset \mathbb{Z}_{n}^{\Delta} $ of the code is mapped to the collection of checks: 
      \begin{equation*}
        \begin{split}
          c_{x} = (x_{1},..,x_{\Delta}) \mapsto \bigcup_{j = - \infty}^{\infty}\left\{ (x_{1} + jn,..,x_{\Delta} +jn)   \right\}
        \end{split}
      \end{equation*}
    \item Each gate $u_{i} \in C$ is mapped by:     
      \begin{equation*}
        \begin{split}
        ...
        \end{split}
      \end{equation*}
    \item \inb{Now I understand that this is not what I wanted. The mapping should be defined such that the basic gate $g_{i}$ is rooted in $x + jn$. So, we would like to have a notation of rooted gates. }
  \end{enumerate}
For a set of faults $\mathcal{E} = \{ f_{i} \}$, we denote by $C^{\infty}[\mathcal{E}^{\infty}]$ the \textit{$\infty$-extension of $C[\mathcal{E}]$}.
\end{definition}

\begin{definition}
For a quantum circuit $C$ and a set of faults $\mathcal{E}$, denote:
  \begin{equation*}
    \begin{split}
...
\end{claim}
\begin{proof}
  Let $u_{1},u_{2}$ be the gates which which act non-trivial on the qubits at $[w]$. \inb{We have to rewrite the $\infty$-extension such that the decoding is a unitary operation, the syndrome is written.}  It's enough to prove for  
 \begin{equation*}
   \begin{split}
