...
Consider a subset of $d$-facets $z \subset \mathcal{F}_{d}$ and a vertex $v \in \mathcal{G}_{n}^{d}$. We say that $z$ is a \textit{rooted assignment} at vertex $v$ if there are subsets $S_{1}, \ldots, S_{k} \subset S^{\times k}$ such that $z$ can be decomposed as:
  \begin{equation*}
    \begin{split}
      z = \sum_{i}^{k}\Theta_{v,S^{\prime}_{i}}
    \end{split}
  \end{equation*}
...
\begin{definition}[Sweep Rule]
  ...
  \begin{enumerate}
    \item First looks for the maximal size coordinate subset $I \subset [d] $ such that $L_{I}$ is a strong local maximum of $L_{I}$ in $\Gamma$. 
    \item Then, check if there is a rooted assignment $\tilde{z}$, such that $z + \tilde{z}$. 
  \end{enumerate}
