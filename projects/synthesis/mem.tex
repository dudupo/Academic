...
\section{Notations and Definitions.} Consider a code with a 2-colorized ($k$-colorized) Tanner graph, such that any two left bits of the same color share no stabilizer. For a subset of bits $S$, we denote by $S_{c_1}$ its restriction to color $c_1$. We use the integer $\Delta$ to denote half of the stabilizers connected to a single bit. (We assume fixed left and right degree in the graph). Our computation is subjected to $p$-depolarized noise. We denote by $m$ the block length of the code. The decoder works as follows:
\begin{enumerate}
...
\begin{equation*}
  \begin{split}
    ...
    & = \sum_{S^{\prime} \subset S \text{ and }  |S^{\prime}_{c_1}| \ge \frac{1}{4} |S^{\prime}|}{q^{\Delta |S^{\prime}_{c_1}|}p^{|S /S^{\prime}|}  } + \sum_{S^{\prime} \subset S \text{ and }  |S^{\prime}_{c_1}| < \frac{1}{4} |S^{\prime}|}{q^{\Delta |S^{\prime}_{c_1}|}p^{|S /S^{\prime}_{c_1}|}  }\\
