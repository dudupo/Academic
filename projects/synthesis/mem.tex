...
\begin{enumerate}
  ...
  \item Else, namely $v \in B$, denote by $\Delta_{l}, \Delta_{r}$ the degree of $v$ when ristricted to $\Gamma(A)/\Gamma(B)$ and  $\Gamma(B)/\Gamma(A)$,and define $\xi = \Delta_{l} |\Theta|    + \Delta_{r}|\Theta| \cdot \left(  \frac{|\Gamma(A)|}{ |\Gamma(B)/\Gamma(A)| } \right)^{-1} $ then: $ \Delta + \xi   \ge \Delta(v) \ge \Delta + \frac{1}{2}\xi$.  
\end{enumerate}

  
\section{Classical Case, Repetition Code.}
In each decoding iteration, we pick random triples and set the values of their bits to the majority. Now, assume that the probability of a subset $S$ to be an error is less than $q^{|S|}$, And denote by $X_{i}$ the random variabilr that counts the number of triples which $i$ of their bits belong to $S$. On one hand: 
\begin{equation*}
  \begin{split}
