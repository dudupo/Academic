...
\begin{enumerate}
  ...
  \item Denote by $S$ the support of an error occurring on $\rho$ (S is a random variable). Then, with high probability\footnote{I'm leaving specifying what it is to later.}, $|S_{c_{1}}| > \frac{1}{4} |S_{1}|$. 
\end{enumerate}
...
So, it remains to show that property (2) still holds with high probability. Let $S^{t} = \Su \left(E\right) $ at time $t$ and denote by $\Pt_{t}$ the probability that $|S^{t}_{c_{1}}| > \frac{1}{4}|S^{t}|$. Then:
\begin{equation*}
  \begin{split}
