...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Thus, the upper bound is summed up to: 
\begin{equation*}
  \begin{split}
...
\begin{claim}  
  \label{claim:ind}
  Let $Q$ be a subset of checks, and $S$ a subset of (qu)bits such that each check in $Q$ adjoins to at most a single qubit in $S$. The probability that all the (qu)bits in $S$ are faulty at end of a correction round is: 
...
\begin{proof}
  ...
  For a check $c$, denote by $X_{c}$ the random variable that counts how many faulty bits, not in $S$, adjoin to $c$. 
  \begin{equation*}
    \begin{split}
      \prb{ \text{  Any (qu)bit of }  S \text{ sees } \mu \text{-majority } } &  \le  \prb{ \text{  More than  } \left( \frac{1}{2} - \mu \right)   \text{ portion of  } \mu \text{ are satisfied } }  \\
      & \le \sum_{i = (\frac{1}{2} - \mu ) |Q|}{{|Q| \choose  i }  \prb{ i \text{ checks are satisfied } } } \\ 
...
The plot in \Cref{fig:qpow} illustrates the values of the exponent. 

\begin{figure}[h]
  \centering
\includesvg{exponent_q.svg}
  \caption{ So for values, $\mu = \frac{1}{10}$, $\Delta_{2}=4$ and $q< 0.05$ we have positive values.}
  \label{fig:qpow}
\end{figure}

\begin{claim}
  Suppose, that the Tanner graph $G = (L,R,E)$, is a $\varepsilon$-expander graph. Then, with high probability, the probability that $S \subset L$ is the error, is lower by the same bound presented in \Cref{claim:ind} but the following substitution in the exponent: 

  \begin{equation*}
    \begin{split}
      |Q| & \mapsto \Delta |S| \\
      \frac{1}{2} - \mu  & \mapsto  \left( \frac{1}{2} - \mu \right)\left( 1 - \varepsilon \right) - 2\varepsilon
    \end{split}
  \end{equation*}
\end{claim}

\begin{proof}
  First, notice that by the Markov inequality for $q$-local stochastic noise: \Cref{calim:markov}, we have that with with probability $|S| < \gamma n$. \ctt{ That argument should appear somewhere else}.

  Thus the probability, Let $|Q|$ be the checks adjoin to $S$, and denote by $Q_{u}$ the checks which connected to at most one (qu)bit in $S$. Observes that by the expansion property:  
  \begin{equation*}
    \begin{split}
|Q|  & \ge (1-\varepsilon) \Delta |S| \\
|Q_{u}| & > (1 - 2\varepsilon)\Delta |S|.
    \end{split}
  \end{equation*}
  Then, 
  \begin{equation*}
    \begin{split}
      \prb{ \text{  Any (qu)bit of }  S \text{ sees } \mu \text{-majority } } & \le  \prb{ \text{  More than  } \left( \frac{1}{2} - \mu \right)   \text{ portion of  } \mu \text{ are satisfied } }  \\
      &   \le \sum_{i = (\frac{1}{2} - \mu ) |Q|}{{|Q| \choose  i }  \prb{ i \text{ checks are satisfied } } } \\
      &   \le \sum_{i = (\frac{1}{2} - \mu ) |Q|}{{|Q| \choose  i }  \prb{ i - |Q/Q_{u}| \text{ of the uniq checks are satisfied } } }  \\
      &   = \sum_{i = (\frac{1}{2} - \mu ) |Q|}{{|Q| \choose  i }  \prb{ i - 2\varepsilon |Q| \text{ of the uniq checks are satisfied } } }  
    \end{split}
  \end{equation*}
...
\section{Concentration (Markov) in Local Stochastic Noise.} 
...
\ctt{REMOVE:}Denote by $\mu$ the expected flips, and by $\alpha,\beta < 1$ marginal parameters such the volume of picking $(\mu - \beta, \mu + \beta)$ is larger than picking $( \ge \mu + \alpha)$. 
\begin{equation*}
  \begin{split}
...
\ctt{ For most of the time we are in the conditioned case $X \not\in C_{z}^{\perp}$. }
...
\begin{enumerate}
  ...
  \item If $S \subset \Omega_{\varepsilon} / \Omega_{0}$ then   $\prb{S} \le \left(\prbm{S}{\mathbf{B}_{p}}\right)^{1-\delta t}$  %   C_{z}^{\perp}$ 
\end{enumerate}
