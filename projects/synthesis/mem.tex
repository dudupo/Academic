...
\begin{claim}
  Suppose, that the Tanner graph $G = (L,R,E)$, is a $\varepsilon$-expander graph. Then, with high probability, the probability that $S \subset L$ is the error, is lower by the same bound presented in \Cref{claim:ind} but the following substitution: 
