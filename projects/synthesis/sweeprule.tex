...
\end{enumerate}
  ...
  \begin{claim}
    Consider a time graph $(G,T)$, an embedding $f : G \rightarrow \mathbb{R}^{d}$, such that.. , Let $K$ be a slice in $G$, and let $\hat{K} = \braket{K, x_{1}, .. x_{d+1}}$ be a spanned slice such that $x_{1},..,x_{d+1}$ are the poles of $S$ and $K \neq \History{K}$. Then for some integer $k$ there exist spanned slices $\hat{K}_{0}, ..,\hat{K}_{k}$ and Forks $F_{1},..,F_{k}$ such that:
    \begin{enumerate}
      \item $ \pre{x_{i}} $ belongs to $K_{0},..,K_{k}$.  
