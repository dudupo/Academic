...
Define the bits-cluster graphs $G=(V,E)$, such that the vertices are connected clusters at different times $\{A_{i}^{t}\}$, and there is an edge between $A_{i}^{t}$ and $A_{j}^{t-1}$ only if $\Pi_{x} A_{i}^{t} \cap \Pi_{x}A_{j}^{t-1} \meq \emptyset $. 
