...
\begin{claim}  
Let $\xi$ be the event in which, at time $t$, the error contains a connected cluster with a span greater than $\frac{d}{4}$, and for $i \in [t]$, let $\sigma_{i}$ be the event in which, at time $i$, there is a connected cluster with a span of at least $\sqrt{d}$ that belongs to the error. Then:
\begin{equation*}
  \begin{split}
