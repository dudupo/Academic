      ...
      \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. Thus, by \cref{claim:prevslices}, we know that for some $k$ there exist spanned slices $\hat{K}_{0}, .., \hat{K}_{k}$ and forks $F_{1},..,F_{k}$ that satisfy \cref{claim:prevslices}. 
        ...
        \begin{enumerate}
          ...
          \item $\mathbf{R} = \{ \{v_{i}, \prei{v_{i}} \} \} \cup_{i=0}^{k}{ \mathbf{R}_{i} }$ with at most 
            \begin{equation*}
              \begin{split}
                & d+1 + \sum^{k}_{i=0}{\alpha (m_{i}-1)  - s_{i} } \\ 
                \le & d+1 + \alpha m - \alpha(k+1) - \left(s + \xi - k \cdot \Size{\text{fork}}  \right) \\
                = & \alpha \left(m - 1\right) - s + \left(\alpha + d+1 - (k+1) \alpha - \xi + k \cdot \Size{\text{fork}} \right)
              \end{split}
            \end{equation*}
            So its enough to show that for any $k \ge 1$: 
            \begin{equation*}
              \begin{split}
                & \alpha + d+ 1 - \xi - (k+1) \alpha  +  k \Size{\text{fork}} \le 0 \\ 
                \Rightarrow &  \alpha \ge \frac{ d+ 1 - \xi }{k} + \Size{\text{fork}}
              \end{split}
            \end{equation*}
        \end{enumerate}
