...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
First, let us bound from above the probability of the right intersection, In that case, at the \textbf{end} of time $t-1$, the erorr consists of clusters each at span less than $\sqrt{d}$, Thus, .  So denote by $\tau$ the number of bits that require to be flipped in order to merge the flipped connected component to a one with a span greater than $d/4$, So $\tau$ has to be at least:   


\begin{equation*}
  \begin{split}
    & \sum\Size{X_{i} } \le n \Rightarrow m \le \frac{n}{\sqrt{d}}\\
