...
\end{enumerate}
For a time $t$, we define the function $P_{t} : V \rightarrow 2^{V}$ that, for a vertex $u$, returns the set of vertices that add $u$.
\end{definition}
...
We will choose the $z_{j}^{i}$ by the following process. Pick a $z_{j}^{i}$ that has not been set yet. If $J_{j} = R_{i}$, then set $z_{j}^{i} \leftarrow \prei{x_{i}}$ - we call this a type-1 assignment. Otherwise, consider the path from $J_{j}$ to $R_{i}$ (by connectivity, it has to be such a one, and by minimality, there is exactly one). Let $J_{t}$ be the successor on the path, then pick $z_{j}^{i} \in J_{j} \cap J_{t}$ - similarly, we call this assignment a type-2 assignment. Since any type-2 assignment is associated with a non-trivial intersection, and any non-trivial intersection induces a type-2 assignment for all $i$'s (since for all $i$'s, one of the $J_{j}$ on its support is a successor of the rest on their path to the $i$th pole), we have that:
 ...
 For a tree $T = (V, E)$ and a vertex $v \in V$, we call the subtree obtained by taking the descendants of $v$ the subtree rooted at $v$.
  ...
  \begin{claim}
    Let $T = (V,E)$ be a tree with $m > 3$ nodes and $\tau$ edges, then it has a subtree with $m^{\prime} \in ( \frac{1}{3}m, \frac{2}{3}m)$ nodes and at most $ \frac{m^{\prime}}{m} \tau$ edges. 
  \end{claim}
  \begin{proof}
Order the vertices according to their heights. Denote by $v \in V$ the lowest vertex for which the subtree rooted at $v$ has at least $\frac{1}{3}m$ nodes. Now, denote by $T_{1}$ the subtree obtained through the following iterative process over $v$'s children:
\begin{enumerate}
  \item Initialize $T_{1}$ as the subtree rooted at $v$.
  \item For a child $u$ of $v$:
    \begin{enumerate}
      \item Check if erasing from $T_{1}$ the subtree rooted at $u$ doesn't decrease the number of nodes below $\frac{1}{3}m$. If so, then we update $T_{1}$ to be the result of the erasure.
    \end{enumerate}
\end{enumerate}
Clearly, the algorithm is defined such that $ T_{1} $ has to have at least $\frac{m}{3}$ nodes. Furthermore, since any child of $ v $ is a root of a subtree with fewer than $\frac{m}{3}$ nodes (otherwise we get a contradiction to the minimality of $ v $'s height), we have that if $ T_{1} $ would have more than $\frac{2}{3}m$ nodes, then the subtraction of any of its children would yield a tree with at least $\frac{2}{3}m - \frac{1}{3}m = \frac{1}{3}m$ nodes. Namely, $ v $ has in $ T_{1} $ a child $ u $ such that the subtraction of the subtree rooted at $ u $ from $ T_{1} $ results in a subtree with more than $\frac{1}{3}m$ nodes. But if indeed there were such a child, then the algorithm would erase it, so there is not. 


Now let $T_{2} = T/T_{1} \cup {v}$, namly the tree induced by the subtraction of $T_{1}$'s edges from $T$. Observes that since the summation of nodes in $T_{1}$ and $T_{2}$ is the number nodes in $T$ + 1 (since $v$ is counted twice), and $T_{1}$ has no more than $\frac{2}{3}m$ nodes, and no less than $\frac{1}{3}$ nodes, it holds that the number of nodes in $T_{2}$ is also in the range $(\frac{1}{3}m, \frac{2}{3}m)$. We claim that at least one of the  subtrees has fewer than $\frac{m^{\prime}}{m} \tau$ edges. 

To see this, denote by $m_{1},m_{2}$ and by $ \tau_{1}, \tau_{2} $ the nodes and the edges in $T_{1},T_{2}$ respectively. Now:
\begin{equation*}
  \begin{split}
    \frac{\tau}{m} &= \frac{\tau_{1} + \tau_{2} }{m_{1} + m_{2}-1} \ge  \frac{\tau_{1} + \tau_{2} }{m_{1} + m_{2}} =  \frac{m_{1} }{m_{1} + m_{2}} \frac{\tau_{1} }{m_{1}}  + \frac{m_{2} }{m_{1} + m_{2}} \frac{\tau_{2} }{m_{2}} 
  \end{split}
\end{equation*}
...
By repeating recursively on the above construction, we get the following corollary:
  \begin{corollary}
Let $T = (V,E)$ be a tree with $m$ nodes and $\tau$ edges. Then, for any $\alpha > 0$, it has a subtree with $m^{\prime} \in \left( \frac{1}{3}\alpha m, \frac{2}{3} \alpha m \right)$ nodes and at most $\frac{m^{\prime}}{m} \tau$ edges.
  \end{corollary}
...
\begin{claim}
  \label{claim:toZ2}
  Consider a running in which no non-trivial circle was closed, then there is $\psi : \mathbb{T}^{d}_{n} \rightarrow \mathbb{Z}_{2n}^{d}$ such:
  \begin{enumerate}
