...
\end{claim}
\begin{proof}
We will prove that $v \notin \Gamma^{t+1}$, and then by repeating on \cref{claim:toombound} where $\sum_{\alpha_{j}} \ge 1$, we get the desired result. Assume, toward contradiction, that $v \in \Gamma^{t+1}$, and denote by $w$ the vertex that adds $v$. If $w \neq v$, then there is at least one coordinate $i$ for which $w_{i} < v_{i}$, thus $L_{d+1}(w) > L_{d+1}(v)$, in contradiction to $L_{d+1}(v)$ being a maximum in $\Gamma^{t}$. In the other case, where $v$ adds itself, it follows that $\sigma_{v}$ can't be decomposed into pairs of unsatisfied checks; namely, $|\sigma_{v}|$ is odd.
\end{proof}
