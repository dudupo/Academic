...
\end{enumerate}
 ...
 Another example is referring to $x_{i}$ as a vertex, so when saying 'For $x_{i} \in V_{k}$' or 'For $\{x_{i},u\} \in F$', we refer to the associated vertex or the edge connecting it to $u$.
  ...
  \begin{proof}
    ...
    Now consider a non trivial intersrion $\bigcap_{j \in X} J_{j}$, we claim that for any $i$, there is $t \in X$ such that $J_{t}$ is the succsessor of $J_{j}$ on the path to $R_{i}$ for any $j \in X/\{t\}$. If not, namely there are $j_{1}, j_{2}$ such that $J_{t}$ is the succsessor to $J_{j_{1}}$ but not the successor of $J_{j_{2}}$ then one can close a cycle by concatinating $J_{t} \rightarrow R_{i} \rightarrow J_{j_{2}} \rightarrow J_{t}$ which is a contradiction to the minimality. Thus, $z_{X}$ will appers $|X|-1$ in $\{ z_{j}^{i} : j \in X \}$ for any $i$. 
  \end{proof}
