...
\begin{claim}
Consider a probability space, $(\Omega, \mathbf{Pr})$, for which the atomic events are subgraphs of $G = (V = \mathbb{T}_{n}^{d} \times \mathbb{Z}_{t}, E)$, where two vertices might be connected only if their difference in the time coordinate is at most $1$ and their Hamming distance in the toric coordinate is at most $2$. We say that a vertex of such a subgraph is a time-leaf if and only if it is not connected to vertices with a lower time coordinate. Suppose that the following two conditions hold:
\begin{enumerate}
  \item Probability function $\mathbf{Pr}$ induces a locally stochastic noise bound over the time-leaves. Namely, there is $p \in (0,1)$ such that for any $S \subset V$, the probability that $S$ are time-leaves of an outcome subgraph is less than:
    \begin{equation*}
      \begin{split}
        \prb{S \text{ are time-leaves } } \le p^{|S|}
      \end{split}
    \end{equation*}
...
\end{equation*}
\begin{proof}
  Let $\omega \in \Omega$ with more than $\beta n$ vertices, observes that a spanning tree of $\omega$ has a ratio of edges to time-leaves which is at most $\omega$'s ratio, and number of edges equals to $\omega$'s vertices - 1. Therefore, by \cref{claim:big_subtree}, $\omega$ has a subtree with less than $\frac{\beta}{2}n$ edges and at least $\frac{\beta}{4\gamma} n$ time-leaves.  that any by \cref{claim:counting_trees}  
\end{proof}
