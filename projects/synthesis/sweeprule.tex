...
\begin{claim}  
  Let $\xi$ be the event in which, at the \textbf{beginning} of time $t$, the error contains a connected cluster with a span greater than $\frac{d}{4}$, and for $i \in [t]$, let $\sigma_{i}$ be the event in which, at the \textbf{end} time $i$, there is a connected cluster with a span of at least $\sqrt{d}$ that belongs to the error. Then:
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
First, let us bound from above the probability of the right intersection, In that case, at time $t-1$, the erorr consists of clusters each at span less than $\sqrt{d}$, and by .  So denote by $\tau$ the number of bits that require to be flipped in order to merge the flipped connected component to a one with a span greater than $d/4$, So $\tau$ has to be at least:   
\begin{equation*}
  \begin{split}
