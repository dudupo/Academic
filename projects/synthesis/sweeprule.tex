...
\begin{claim}  
  Let $\xi$ be the event in which, at the time $t$, the error contains a connected cluster with span greater than $\frac{d}{4}$, and for $i\in [t]$, let $\sigma_{i}$ the event in which at time $i$ there is a connected cluster with span at least $\sqrt{d}$ which belongs to the error. Then: 
