...
\end{claim}
\begin{proof}
  Consider $u \in \Gamma^{* (t)}$. By \cref{claim:inprev} there is $v \in P_{t}(u)$ satisfies $v < u$, choose it to be $v_{d+1}$. Recall that since the vertices are the points of the d-dimensional lattice embedded in $\mathbb{R}^{d}$, the inequality $v <u$ implies that there exists at least single $i \in[d]$ such $ v+ \mathbf{1}_{i} \le u$. Therefore we get: 
  \begin{equation*}
    \begin{split}
      L_{d+1}(v_{d+1}) = -\braket{ \mathbf{1}, v_{d+1} } = -\sum_{j \in [d]}v_{j} =  -\sum_{j \in [d]/i}v_{j} - v_{i} \ge  -\braket{ \mathbf{1}, u } + 1 
    \end{split}
  \end{equation*}
  , Otherwise, positive edges getting out of $u$ adjacent, at time $t -1$, to bits $\not{\in} D^{(t-1)} $, thus if $u \not{\in} D^{(t-1)}$ then all those 
\end{proof}
