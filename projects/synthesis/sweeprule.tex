...
\end{enumerate will call to $T$ time-function, to $A$ arrows, and to $F$ forks. Observes that from the second requirement it follows that forks can connect only vertices that have the same time value (set by $T$). 
  \end{definition}
...
Reconsider the construction of the explanation. We could add at the top an imaginary layer in which all the vertices at time $ t $ are connected by arrows to all the vertices (at time $ t+1 $) which are adjacent to them in the Tanner graph. For the last layer we might gets that the span of a slice is expand, instead of shrinking, namely \cref{claim:prevslices} for the final slices would change to: 
