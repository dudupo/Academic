...
\end{enumerate}
 ...
 Another example is referring to $x_{i}$ as a vertex, so when saying 'For $x_{i} \in V_{k}$' or 'For $\{x_{i},u\} \in F$', we refer to the associated vertex or the edge connecting it to $u$.
  ...
  \begin{proof}
    ...
    Consider the graph $G^{\prime} = (V^{\prime}, E^{\prime})$, such that $V^{\prime} = \{J_{j}\}_{j} \cup {v}$. where to any $x\in X $ we have $ \{ J_{x}, v\} \in E^{\prime}$ and for any $x,y$ such $y\notin X$, and $J_{x}\cap J_{y} \neq \emptyset$, then  $\{ J_{x}, J_{y} \} \in E^{\prime}$. \ctt{Should split any junction}. 
    
    Since, the graph is tree, 
  \end{proof}
