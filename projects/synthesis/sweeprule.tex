...
\begin{claim}
For any $u \in \Gamma^{* (t)}$, there has to be a vertex $v$ such that $v < u$ and $v \in P_{t}(u)$.
\end{claim}
\begin{proof}
  Consider $u \in \Gamma^{* (t)}$, and recall that $u \in D^{(t)}/\mathcal{E}^{(t)}$, Thus . First observes that there is must to be $v < u$ in $P_{t}(u)$, Otherwise, positive edges getting out of $u$ adjacent, at time $t -1$, to bits $\not{\in} D^{(t-1)} $, thus if $u \not{\in} D^{(t-1)}$ then all those 
\end{proof}
