  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. Thus, by \cref{claim:prevslices}, we know that for some $k$ there exist spanned slices $\hat{K}_{0}, .., \hat{K}_{k}$ and forks $F_{1},..,F_{k}$ that satisfy \cref{claim:prevslices}. 
        ...
        Denote by $s_{i} = \Size{\hat{K}_{i}}$. By the inductive hypothesis, we know that $\hat{K}_{i}$ has an explanation formed by a set of leaves $\mathbf{W}_{i}$, a set of arrows $\mathbf{R}_{i}$ and a set of forks $\mathbf{F}_{i}$. Let $m_{i} =|\mathbf{W}_{i}|$, the sets $R_{i}$ and $F_{i}$ have at most $\alpha m_{i} - \beta s_{i}$ and $m_{i}-1$ leaves. We define:
        \begin{enumerate}
          ...
          \item $\mathbf{R} = \{ \{v_{i}, \prei{v_{i}} \} \} \cup_{i=0}^{k}{ \mathbf{R}_{i} }$ with at most 
            \begin{equation*}
              \begin{split}
                & d+1 + \sum^{k}_{i=0}{\alpha (m_{i}-1)  - \beta s_{i} } \le d+1 + \alpha m - \alpha(k+1) - \beta\left(s - 1 + k \cdot \Size{\text{fork}}  \right) \\
