...
Let $\sigma^{(t)}$ be a domain wall at time $t$, and let $\Gamma^{t} = \{ v : \exists \{v,u\} \in \sigma^{t} \}$, where we treat to the edges in a direct manner, means $\{u,v\} \neq \{v,u\}$. We denote by $\phi^{t}$ the subsets of bits which are flipped as a result of applying the Toom's rule at time $t$, and by $\phi_{v}$ the bits which are flipped by vertex $v$. We say that $u$ is added to $\Gamma^{t+1}$ by $v$ at time $t$, if $u\in \Gamma^{t+1} / \Gamma^{t}$ and $u \in V(\phi_{v})$.


\ctt{$V(\phi_{v})$ was not defined.} 
