...
\begin{definition}[$v$ adds $u$ at time $t$]
  Let $D^{(t)} \subset \mathcal{F}$ be the bits that deviate from the ideal running \ctt{Define it!}. Let $\Gamma^{(t)} \subset V$ be the vertices that support the plaquettes in $D^{(t)} / \mathcal{E}^{(t)}$. Namely, $\Gamma^{(t)} = \{ v : \exists f \in D^{(t)} / \mathcal{E}^{(t)} \text{ s.t. } v \in f\}$. In other words, $D^{(t)}$ are the vertices that, at time $t$, are adjacent to a deviated bit.
...
\end{enumerate}
For a time $t$, we define the function $P_{t} : V \rightarrow 2^{V}$ that, upon vertex $u$, returns the set of vertices that add $u$. 
\end{definition}

\begin{claim}[Shrinking Lemma]
For any $u \in \Gamma^{t}$, there are $v_{1}, \ldots, v_{d+1} \in P_{t}(u)$, not necessarily different, such that for any $i \le d$, it holds that $L_{i}(v_{i}) = L_{i}(u)$, and for $i = d+1$, it holds that $L_{d+1}(v_{d+1}) = L_{d+1}(u) + 1$.
\end{claim}
\begin{proof}
