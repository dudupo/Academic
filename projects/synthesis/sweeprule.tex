...
Let $\xi$ the event in which, at the time $t$, the error contains a connected component in the Tanner graph with a span $>\frac{d}{4}$, and let $\sigma_{i}$ the event that at time $i$ a connected with span at least $\sqrt{d}$ belongs to the error. Then: 
