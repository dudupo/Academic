...
\end{enumerate}
  ...
  \begin{definition}[Spanned Set]
    Consider an embedding $f : \star \hookrightarrow \mathbb{R}^{d}$, a subset of $f$'s domain $S \subset \star$, and $d+1$ points $x_{1}, \ldots, x_{d+1}$. We say that $\braket{S, x_{1}, \ldots, x_{d+1}}$ is a spanned set of $S$ if for any $i \in [d+1]$ and $s \in S$, it holds that $L(f(s)) \le L_{i}(x_{i})$. If all the inequalities are equalities, then $x_{1}, \ldots, x_{d+1}$ are called the poles of $S$. We extend the notation to a spanned slice when $S$ is a slice of some time-graph. Notice that saying $\braket{S, x_{1}, \ldots, x_{d+1}}$ is a spanned set means that $\Size{S} \le \Size{ \{x_{1}, \ldots, x_{d+1}\}}$.
  \end{definition}

  Observe that in the case where the subset $S$ is a slice and $x_{1}, \ldots, x_{d+1}$ are its poles. Each of these points can be associated with vertices of $G$. If there are multiple vertices whose images give equality, associate the point with one of them arbitrarily. We will use the notion of applying functions originally defined over vertices to the points, referring to the application over the vertices associated with them. For example $\pre{x_{i}} = \pre{v}$ for the vertex $v \in S$ which is associated with the pole $x_{i}$. 

  \begin{claim}
    Consider a time graph $(G,T)$, an embedding $f : G \rightarrow \mathbb{R}^{d}$, such that.. , Let $K$ be a slice in $G$, and let $\hat{K} = \braket{K, x_{1}, .. x_{d+1}}$ be a spanned slice of $K$ such that $x_{1},..,x_{d+1}$ are its poles and $K \neq \History{K}$. Then for some integer $k$ there exist spanned slices $\hat{K}_{0}, ..,\hat{K}_{k}$ and Forks $F_{1},..,F_{k}$ such that:
    \begin{enumerate}
      \item For $i \in [d+1]$ $ \prei{x_{i}} $ belongs to the poles $K_{0},..,K_{k}$.  
