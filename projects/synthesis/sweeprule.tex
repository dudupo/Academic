...
Later, we are going to take the points $\{z_{j}^{i}\}$ from the intersection of $J_{j}$'s. For convenience, let us assume that for the indices subset $X \subset [2k]\cup\{0\}$, the intersection $\bigcap_{j \in X}J_{j}$ returns a single point; if there is more than one, then decide on an arbitrary one.
    ...
    We will choose the $z_{j}^{i}$ by the following process. Pick a $z_{j}^{i}$ that has not been set yet. If $J_{j} = R_{i}$, then set $z_{j}^{i} \leftarrow \prei{x_{i}}$ - we call this type-1 assignment. Otherwise, consider the path from $J_{j}$ to $R_{i}$ (by connectivity it has to be such one, and by minimality there is exactly one). Let $J_{t}$ be the successor on the path, then pick $z_{j}^{i} \in J_{j} \cap J_{t}$ - similarly, we call this assignment, a type-2 assignment. 

    %according to some deterministic rule dependenting only on $J_{j}, J_{t}$. 

    \begin{equation*}
      \begin{split}
        \sum_{j,i}L_{i}(z_{j}^{i}) &= \sum_{j,i, z_{j}^{i} \in \text{type-1}}L_{i}(z_{j}^{i}) + \sum_{j,i, z_{j}^{i} \in \text{type-2}}L_{i}(z_{j}^{i}) \\
        &= \sum_{i}^{d+1}L_{i}\left( \prei{x_{i}} \right) + \sum_{X : \bigcap_{j\in X}J_{j} \neq \emptyset }\left(|X|-1\right) \sum_{i} L_{i}\left(\bigcap_{j\in X}J_{j} \right)
      \end{split}
    \end{equation*}
