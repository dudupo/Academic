...
\begin{enumerate}
  \item There exists $n \in \mathbb{N}$ such that the probability function $\mathbf{Pr}$ induces a locally stochastic noise bound over the time-leaves, which are different in their space coordination module $n$. Namely, there is $p \in (0,1)$ such that for any $S \subset V$, with a maximal subset $S^{\prime} \subset S$, such that for any $v = (v_{x},v_{t}), u = (u_{x},u_{t}) \in S^{\prime}$ it holds that $v_{x} \mod n \neq u_{x} \mod n$, the probability that $S$ are time-leaves of an outcome subgraph is less than:
    \begin{equation*}
      \begin{split}
        \prb{S \text{ are time-leaves } } \le p^{|S^{\prime}|}
      \end{split}
    \end{equation*}
  \item For any subgraph $\omega \in \Theta$ the ratio of edges to time-leaves is at most $\gamma$. 
\end{enumerate}
...
\begin{proof}
  ...
  Now, since any two connected vertices are at a space-distance of at most $2$, it follows that all the leaves in the subtree are at a space-distance of at most $\beta n$ from each other. Thus, if $\beta < 1$, no two of them have the same space-coordinate modulo $n$, and therefore the probability to have such subtree is at most $p^{\frac{\beta}{4\gamma}}n $.  

  That any by \cref{claim:counting_trees}  
\end{proof}
