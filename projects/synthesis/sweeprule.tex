...
\end{enumerate}
 ...
 Another example is referring to $x_{i}$ as a vertex, so when saying 'For $x_{i} \in V_{k}$' or 'For $\{x_{i},u\} \in F$', we refer to the associated vertex or the edge connecting it to $u$.
  ...
  \begin{proof}
    ...
    Let us denote by $J_{0},..,J_{2k}$ the enumarated union $K_{0},..K_{k},F_{1},..,F_{k}$. Observes that for any choise of $(d+1)(2k+1)$ elements in $\mathbb{R}^{d}$, $z_{0}^{1},z_{0}^{2}, z_{0}^{3},..,z_{0}^{d+1}..,z_{2k}^{d+1}$ such that $z_{j}^{i} \in J_{j}$ it holds that:
    \begin{equation*}
      \begin{split}
