...
\end{enumerate}
 ...
 Another example is referring to $x_{i}$ as a vertex, so when saying 'For $x_{i} \in V_{k}$' or 'For $\{x_{i},u\} \in F$', we refer to the associated vertex or the edge connecting it to $u$.
  ...
  \begin{proof}
    ...
    Now consider a non trivial intersrion $\bigcap_{j \in X} J_{j}$, we claim that for any $i$, there is $t \in X$ such that $J_{t}$ is the succsessor of $J_{j}$ on the path to $R_{i}$ for any $j \in X/\{t\}$.
