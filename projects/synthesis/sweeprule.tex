...
\begin{claim}
  \label{claim:inprev}
  For any $u \in \Gamma^{* (t)}$, there exists $f \in D^{(t-1)}$ such that for any vertex $v \in f$, it holds that $v < u$. Notice that its not necessarily that $u \in f$. 
\end{claim}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Else, there has to be a plaquette $f \in D^{(t)}/D^{(t-1)}$ such that $v \in f$. Thus $f$ was been flipped at time $t$. Since, by the definition of the Toom's rule, a plaquette is getting flipped only if two of its edges are unsatisfied \footnote{Here the argue works no matter what edges that been flipped are.}, we get that at least three vertices of $f$ adjacent to unsatisfied checks at time $t-1$, namely adjacent to plaquettes in $D^{(t-1)}$\footnote{Those plaquettes are adjacent to $f$.}. 
