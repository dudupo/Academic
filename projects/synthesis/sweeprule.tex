...
\end{claim}
\begin{proof}
  Consider $u \in \Gamma^{* (t)}$, and recall that $u \in D^{(t)}/\mathcal{E}^{(t)}$, Thus . First observes that there is must to be $v < u$ in $P_{t}(u)$, otherwise, all the positive edges getting out of $u$ adjacent, at time $t -1$, to bits $\in D $
\end{proof}
