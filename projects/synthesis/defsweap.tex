...
\end{proof}
  ...
  \begin{definition}[Base graph $G_{o}$]
    Let $C$ be a quantum circuit with registers $R_{1}, \ldots, R_{k}$, all of which are encoded by Toric codes with the same parameters (i.e., dimension, length, etc.). We define the base graph of $C$, denoted by $G_{o}$.  
  \end{definition}
