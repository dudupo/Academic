...
This bound is known as the Expander Mixining Lemma. We refer the reader to \cite{hoory2006expander} for more detailed servery. 
  ...
  \end{lemma}
  \begin{proof}
    Denote by $S^{(0)} \subset V^{+}$ and  $T^{(0)} \subset V^{-}$ the subsets of left and right vertices adjacent to the error at the beginning of the algorithm. Similarly denote by $S^{(1)}$ ($ T^{(1)}$) the vertices adjacent to an error after the first left (right) correction round. Consider a vertex $v \in V^{+}$. If $v \notin S^{(0)}$, namely the bits on $v$'s edges satisfying the local restrictions, then no correction is made by $v$. By the bipartition of the graph, any other vertex which share an endpoint with any of $v$'s edge has to be in $V^{-}$. Hence after the first left correction, left vertices which where not adjacent to an error are still not adjacent to an error. Therefore we have $S^{(1)} \subset S^{(0)}$.
