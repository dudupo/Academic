...
Taking the block length to be $|B| = \log ( (W \cdot D)^{c} )$ gives a linear\footnote{Assuming $W$ is polynomial in $D$} fault tolerance construction that success with probability $ 1- \frac{1}{\log^{c_{2}}( W\cdot D)} $. 
