...
\begin{lemma} Let $S = \braket{S_1,..,S_{m}}$ be a stabilizer code and $M\subset \Lambda$ be an arbtrary subset of qubits. Denote $J(M)$ a set of indexes $a$ such that the support of $S_{a}$ overlaps with $M$. Then one of the following is true: 
  ...
  \subsection{The CSS LDPC version for the Cleanining Lemma.}
  Consider, the partition of edges (over tanner graph or whatever) such that any vertex see less then $\delta \Delta$ edgs. Then np logical codeword is supported by that set. 
