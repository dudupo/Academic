...
This bound is known as the Expander Mixining Lemma. We refer the reader to \cite{hoory2006expander} for more detailed servery. 
  ...
  \end{lemma}
  \begin{proof}
  Denote by $S^{(0)} \subset V^{+}$ and  $T^{(0)} \subset V^{-}$ the subsets of left and right vertices adjacent to the error. And denote by $T^{(1)} \subset T^{(0)}$ the right vertices such any of them is connect by at least $\frac{1}{2}\delta_{0}\Delta$ edges to vertices at $S^{(0))}$. Note that that any vertex in $V^{-}/T^{(1)}$ has on his local view less than $\frac{1}{2}\delta_{0}\Delta$ faulty bits and therefore it corrects into his right local view in the first right correction round. We will show that \ctt{$\Gamma(\cdot)$} $T^{(1)}$ is a constant portion times smaller than $S^{(0)}$. 
  %adjacent to an error after a right correction round which . Consider a vertex $v \in V^{+}$. 

 %   If $v \notin S^{(0)}$, namely the bits on $v$'s edges satisfy the local restrictions, then no correction is made by $v$. By the bipartition of the graph, any other vertex which share an endpoint with any of $v$'s edge has to be in $V^{-}$, means that it doesn't suggest a correction at the left correction turn. Hence after the first left correction, left vertices which where not adjacent to an error previously to the decoding will remain still not adjacent to an error afterwords. Therefore we have $S^{(1)} \subset S^{(0)}$.
