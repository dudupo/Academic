...
This bound is known as the Expander Mixining Lemma. We refer the reader to \cite{hoory2006expander} for more detailed servery. 
  ...
  \end{lemma}
  \begin{proof}
    Denote by $S^{(0)} \subset V^{+}$ and  $T^{(0)} \subset V^{-}$ the subsets of left and right vertices adjacent to the error at the beginning of the algorithm. Similarly denote by $S^{(1)}$ ($ T^{(1)}$) the vertices adjacent to an error after the first left (right) correction round. Consider a vertex $v \in V^{+}$. 
