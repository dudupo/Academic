...
This bound is known as the Expander Mixining Lemma. We refer the reader to \cite{hoory2006expander} for more detailed servery. 
  ...
  \begin{lemma}
    If the error is less then $\beta n $ then a single round of the  majority reduce the error by at least constant fraction. 
  \end{lemma}
  \begin{proof}
Assume that the error $e$ is at size at most $\beta n$. Then: \begin{equation*}
  \begin{split}
    \frac{1}{2}\delta_{0}\Delta |T| & \le \Delta \frac{|T||S|\cdot \frac{1}{2}\delta_{0}}{n} + \lambda\sqrt{|T||S|} \\ 
  \left( \frac{1}{2} \delta_{0} \Delta - \beta \Delta \right) |T| & \le \lambda \sqrt{|T||S|} \\ 
|T| & \le \left( \frac{1}{2} \delta_{0} \Delta - \beta \Delta \right)^{-2}\lambda^{2} |S| 
  \end{split}
\end{equation*}
...
\subsubsection{Pippenger Construction.} Encode any bit of the original code by $n$ bits using the repitation code. Replace any of the AND, and the OR gate by their transversal gates. In each even turn applay a single round of the majoritiy decoder. Finally at the end applay the decoder.  
