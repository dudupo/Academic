...
\ctt{In that section the $\cdot$ operation describe the pair wise product (pair wise AND).}
...
Clearly we have that  $|z\cdot z^{\prime}| \le |z|,|z^{\prime}|$ therefore we have that $\prbm{z,z^\prime \in \Czdu}{ | z \cdot z^{\prime} | \ge t} \le \prbm{z\in \Czdu}{ | z | \ge t}$. Now assume that the tanner code by which the code defined is bipartite graph and denote by $z_{+},z_{-}$ the grouping of the $z$'s generators supported on the even and the odd vertices of the graph. By tingle inequality $|z| = |z_{+} + z_{-}| \le |z_{+}| + |z_{-}|$, So if $|z| > t$ then at least one of $|z_{-}|,|z_{+}|$ is greater than $t/2$. Hence: 
\begin{equation*}
  \begin{split}
    \prbm{z\in \Czdu}{ |z|  } \le \prbm{z\in \Czdu}{ \bigcup_{i \in \pm }{|z_{i}| \ge t/2}   } \le \sum_{i \in \pm }{\prbm{z\in \Czdu}{ |z_{i}| \ge t/2   }}
  \end{split}
\end{equation*}

Since any two positive (negative) generators are disjoint and intersection of bits can occurs only between positive and negative vertices we have    $z_{+}$ 
