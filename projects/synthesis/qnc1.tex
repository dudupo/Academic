...
\begin{proof}
 ...
 % \begin{widetext}
  ...
  consider the decoder under $\mathcal{N}$ action. $P_{1}D_{1}P_{2}D_{2},..,P_{i-1}D_{i}P_{i}$, by the fault-tolerance theorem a logical error happens at the $i$th stage occurs with probability $p^{2^{i}}$, therefore by the union bound the probability that in one of the first $i-1$ steps the circuit absorbs an error that is not corrected is less than $p^{2} + p^{4} + .. < 2p^{2}$.
