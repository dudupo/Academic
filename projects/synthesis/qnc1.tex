...
\abstract{In this work we study the overall depth overhead cost required for constructing fault tolerance circuits. We focus on shallow depth circuits classes, In particular, $QAC_{0}, QNC_{0,f}$ and $QNC_{1}$ and certain knowns problem candidates for demonstrating quantum advantage such as factoring and Instantaneous Quantum Polynomial-time \cite{Bremner_2017}, \cite{Paletta_2024}. We give a partial answerer, Yet, clues that might pave the way towards a full understanding the trade-off between compleixiiy versus fault tolerance. 
