...
\section{ Notations. }
$C_{g}$ - good qLDPC, $C_{ft}$ - concatenation code ($ft$ stands for fault tolerance). For a code $C_{y}$ we use $\Phi_{y}, E_{y}, D_{y}$ to denote the channel maps circuits into the circuits compute in the code space, the encoder, and the decoder. We use $\Phi_{U}$ to denote the 'Bell'-state storing the gate $U$. We say that a state $\ket{\psi}$ is at distance $d$ from a quantum code $C$ if there is an operator $U$  sending $\ket{\psi}$ into $C$ such that $U$ spanned on paulis with degree at most $d$. Sometimes, when the code been used is clear form the context, we will say that a block $B$ of qubits absorbed at most $d$ noise if the stated encoded on $B$ is at distance at most $d$ form that code.  
