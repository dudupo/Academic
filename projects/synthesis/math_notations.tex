...
We follow the standard notations commonly used in the literature. For an integer $n$, we denote by $[n]$ the set of integers $\{1, 2, \ldots, n\}$. To present a countable set, which might or might not be finite, where the number of elements does not matter, we use curly brackets with a subscript denoting the first index of the set. For example, $\{u_{i}\}_{i=1}$ represents $u_{1}, u_{2}, \ldots$; For finite sets, we might add a superscript to present the size of the set. For an integer $i$, we denote by $1_{i}$ the vector consisting of one at the $i$th coordinate and zero elsewhere.
