...
\begin{definition}
  ...
  \end{enumerate}
\end{definition}
For example, the empty code, that contains only the zero code word, i.e $C = \{ 0 \}$, is a \trig with respect to any code. In fact for proving \Cref{theorem:main} taking the empty code is sufficient. For other example, the Triorthogonals codes defined in \cite{bravyi2012magic} are \trig with respec to themself.  
\begin{definition}
  \label{def:thecode}
