...
For a fixed quantum circuit $C$, where all its quantum registers are encoded with the Toric code, and a set of faults $\mathcal{E}$, such that the depth of $C[\mathcal{E}]$ is $\mu$, we will say that the Paulis $P_{1}, \ldots, P_{\mu+1} \in \mathcal{P}$ are fault paths of $C[\mathcal{E}]$ if there are Paulis $Q_{1},..,Q_{\mu+1}$, such $P_{i} \sim Q_{i}$ for any $i \in [\mu +1]$ and in addition, $Q_{1}$ is the identity and for every $t\le \mu$ the Pauli $Q_{t+1}$ supports $C[\mathcal{E}]|_{t} \circ Q_{t}$. Namely:
  \begin{equation*}
    \begin{split}
      \Trr{} \big( C[\mathcal{E}]|_{t} Q_{t} (C[\mathcal{E}]|_{t})^{\dagger} Q_{t+1} \big) \neq 0
    \end{split}
  \end{equation*}
