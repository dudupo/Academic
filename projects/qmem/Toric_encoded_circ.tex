...
\section{Toric Encoded Circuits and Toom's Contours}
  ...
  \begin{definition}[Base graph $G_{o}$]
    Let $C$ be a Toric encoded circuit at depth $d$. Denote its quantum registers by $\{R_{i}^{j}\}$, where the subscript and the superscript identify the register number and the time, respectively. For example, $R_{0}^{0}$ is the first register at time zero. Notice that measurements take quantum registers into classical. Let's denote by $\{\mathcal{G}_{i}^{j}\}$ the Toric complexes that induce the codes. We define the base graph of $C$, denoted by $G_{o} = (V_{o}, E_{o})$, as follows:
    \begin{enumerate}
      \item The vertices $V_{o}$ are tuples, where the first coordinates correspond to a vertex in the disjoin union over the vertices in $\{\mathcal{G}_{i}^{0}\}$, and the second coordinate is associated with a time. Namely:
        \begin{equation*}
          \begin{split}
