...
\begin{definition}
Let $\{u_{i}\}_{i}$ be the gates of the subjected quantum circuit $C[\mathcal{E}]$, placed according to the steps' order. We define the ideal evolution superoperator, denoted by $\Phi_{i}(\rho)$, to be:
  \begin{equation*}
    \begin{split}
\Phi_{i+1}(\rho) = 
      \begin{cases}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Step. Assume the correctness of the claim for every step $\tau^{\prime} < \tau^{\prime}$. Then, let $E_{1},..,E_{m}$ be the karus operators of $g_{\tau}$, and let's write each of the $E_{i}$s by it's Pauli decomposition: 
      ...
      &= g_{\tau}\circ \bigl( \sum_{i,j}\alpha_{i,j} P_{i} \Phi_{\tau} (\rho) P_{j}  + c.c   \bigr) \\
      &= \sum_{l}  E_{l}  \bigl( \sum_{i,j}\alpha_{i,j} P_{i} \Phi_{\tau} (\rho) P_{j}  + c.c   \bigr) E_{l}^{\dagger} \\ 
      &= \sum_{l,k_{1},k_{2} i,j}  \gamma_{k_{1}}\gamma_{k_{2}}^{*}\alpha_{i,j} Q_{l}^{k_{1}}P_{i} \Phi_{\tau} (\rho) P_{j} Q_{l}^{k_{2}} + c.c \\
      &= \sum_{l,k_{1},k_{2} i,j} (-1)^{N_{i,j,l,k_{1},k_{2}}} \alpha_{i,j} P_{i}\gamma_{k_{1}}\gamma_{k_{2}}^{*}Q_{l}^{k_{1}}\Phi_{\tau} (\rho)  Q_{l}^{k_{2}} P_{j} + c.c \\
        \end{split}
      \end{equation*}
  \end{enumerate}
\end{proof}
