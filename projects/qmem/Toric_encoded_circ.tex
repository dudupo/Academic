...
Where we define a vertex $v$ to belong to location $l$ if there is a qubit-face $f$ in $l$ such that $v$ belongs to its associated face.
  ...
  \inb{\textbf{Do somthing.}} We close the section by presenting Pauli-paths, which can be thought of as the collapsing errors due to stabilizer measurements.

\begin{definition} 
  \label{def:paulipath}
For a fixed quantum circuit $C$, where all its quantum registers are encoded with the Toric code, and a set of faults $\mathcal{E}$, such that the depth of $C[\mathcal{E}]$ is $\mu$, we will say that the Paulis $P_{1}, \ldots, P_{\mu+1} \in \mathcal{P}$ are fault paths of $C[\mathcal{E}]$ if $P_{1}$ is the identity and for every $t\le \mu$ the Pauli $P_{t+1}$ supports $C[\mathcal{E}]|_{t} \circ P_{t}$. Namely:
  \begin{equation*}
    \begin{split}
      \Trr{} \big( C[\mathcal{E}]|_{t} P_{t} (C[\mathcal{E}]|_{t})^{\dagger} P_{t+1} \big) \neq 0
    \end{split}
  \end{equation*}
...
Finally, we define a reduced Pauli-path $\tilde{P_{1}}, \ldots, \tilde{P}_{\mu+1}$ to be the Paulis obtained by taking a Pauli-path, considering its view in the $\infty$-extension, and for any finite connected chain that corresponds to a subset of the $X$ component of $P_{t}$'s support, substitute it with its reduced form (\Cref{def:reduced}). Then do the same for the $Z$ component in the dual picture.
\end{definition} 
...
\inb{\textbf{Consider, seting that Toom's contour to be $X,Z$-Toom's contour.  }}
...
%        \end{equation*}
      ...
      \item The edges $E_{\mathcal{H}} \subset E_{o} $ is partitioned into two types $A_{\mathcal{H}}$ and $F_{\mathcal{H}}$, where for any logical gate $g$ which applied in time $t$ in $C[\mathcal{E}]^{\infty} $ we add edge to $A_{\mathcal{H}}$ as follow: 
        \begin{enumerate}
          \item If $g$ is either a logical Pauli, a measurement in either the computational or the phase basis, or an identity, then for any face $f$ in the register on which $g$ acts, we add:
            \begin{equation*}
              \begin{split}
                & \bigl\{  \{ \tilde{u} = (\cdot, u,t),  \tilde{v} = ( \cdot, v,t+1)\} : u,v \in f \text{ and } \tilde{v},\tilde{u} \in V_{\mathcal{H},X} \\ 
            ...
            \begin{equation*}
              \begin{split}
                & \bigl\{  \{ \tilde{u}  = (\cdot, u,t), \tilde{v} = ( \cdot, v,t+1)\} :  u \in f_{1}\cup f_{2}, v \in f_{1}\cup f_{2}  \text{ and } \tilde{v},\tilde{u} \in V_{\mathcal{H},X}  \\
...
\begin{definition}[$X,Z$-Toom's Contour]
  $X,Z$-Toom's Contour.
\end{definition}

 
\begin{definition}[Sequential Toom's Contour ]
  Sequential Toom's Contour.
\end{definition}
...
\begin{minipage}{0.75\textwidth}
  \textit{
            ...
            \begin{equation*}
              \begin{split}
                & \bigl\{  \{ \tilde{u} = (\cdot, u,t),  \tilde{v} = ( \cdot, v,t+1)\} : u \in f , v \in \phi(f) \text{ and } \tilde{u} \in V_{\mathcal{H},X}, \tilde{v} \in V_{\mathcal{H},Z}, \\ 
...
\begin{definition}[Analysis Toom's Contour at Step $\tau$]
  ...
  \begin{enumerate}
    \item We define $t^{\star}$ as the time of the gate at step $\tau$ plus one, and then add for the vertices in $V_{\mathcal{H},\{X\}}$ the duplication of the previous layer, namely the subset:
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t^{\star}}  & : v_{t^{\star}}=( \{X\}, v,t^{\star}) \text{ if there exists a face } f \in \text{ supp } X(P_{t^{\star}-1}) \\ 
            & \text{ such } v \in f \in X(P_{t^{\star} - 1}) \bigr\} 
          \end{split}
        \end{equation*}
        Similarly we add to $V_{\mathcal{H}, Z}$ the subset: 
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t^{\star}}  & : v_{t^{\star}}=( \{Z\}, v,t^{\star}) \text{ if there exists a face } f \in \text{ supp } Z(P_{t^{\star}-1}) \\
            & \text{ such } v \in \phi(f) \bigr\} 
          \end{split}
        \end{equation*}
      \item For the edges, we add the following arrows:
        \begin{equation*}
          \begin{split}
            \bigl\{ \bigl\{ \tilde{u} = (\cdot, u,t^{\star} -1 ), \tilde{v}= (\cdot, v,t^{\star})  \bigr\} & \text{ if there exists a  face } f \in  X(P_{t^{\star} - 1}) \\ 
            & \text{or  } \phi^{-1}(f) \in Z(P_{t^{\star} - 1}) \text{ such } v,u \in f   \bigr\}  
          \end{split}
        \end{equation*}
