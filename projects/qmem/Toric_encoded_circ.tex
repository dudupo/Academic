...
We are about to zoom out from a single register encoded in the Toric code to describe a fully fault-tolerant computation. For a given quantum circuit, assumed to consist of gates $\mathbf{CX}, \mathbf{H}, \mathbf{S}, \mathbf{T}$ and measurements, we define its Toric encoded circuit as its version where each qubit is encoded by the Toric code. Formally:
    \begin{definition}[Troic Encoded Circuit]
      ...
      \begin{enumerate}
        ...
        \end{enumerate} 
    \end{definition}
    As a concept, a Toomscontouer is a directed graph that records the deviation's evolution. The vertices of the graph are taken from the vertices of the Torices skeletons that induce the registers' encodings. In fact, they are the vertices whose local checks are unsatisfied. 
    For giving a formal definition, first let us introduce the base graph - the  
