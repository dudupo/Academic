...
\inb{\textbf{Redefine the Base graph such it will not be dependent on $\mathcal{E}$ and $\alpha$, yet it will match them, in terms of layers. (it can be dependent when it cones to edges), so the way to do so is to pads in identity, when control $S$ doesn't occur. }}
...
\begin{definition}
  \label{def:paulipath}
A Pauli-path is the $\infty$-lift of a propagation of $C[\mathcal{E},\alpha]$.
 

%\inb{\textbf{Consider removing.}} Consider a quantum circuit subjected to noise $C[\mathcal{E}]$, with width $w$ and depth $\mu$, whose registers are all encoded in the $(d,d^{\prime})$-Toric code with side length $n$ such that $C[\mathcal{E}]$ has an $\infty$-extension. We say that the set of Pauli $\{ (P_{i}) \}_{1}^{\mu+1}$, where for each $i$, $P_{i} \in \mathcal{P}^{[w]}$, is a Pauli-path for $C[\mathcal{E}]$. If $P_{1}$ is the identity and for any $t \in [\mu]$, the Pauli $P_{t+1}$ is defined such it satisfies the follow:  
%  \begin{equation*}
%    \begin{split}
...
\begin{definition}[Toom's Contour Graph of $C(\mathcal{E},\alpha)$, (Standard form)]
    \label{def:contour}
    For a Toric encoded quantum circuit $C$, set of faults $\mathcal{E}$, measurements outcomes $\alpha$, and a Pauli-path $P_{1},..$ of $C[\mathcal{E}, \alpha]$. We define the \textit{Toom's contour graph} $\mathcal{H} = ( V_{\mathcal{H}}, E_{\mathcal{H}})$ as follows: 
    \begin{enumerate}
...
%        \end{equation*}
      ...
      \item The edges $E_{\mathcal{H}}$ is partitioned into two types $A_{\mathcal{H}}$ and $F_{\mathcal{H}}$, where for any realization of logical gate $g$ which applied in time $t$ in $C[\mathcal{E}]^{\infty} $ we add edge to $A_{\mathcal{H}}$ as follow: 

        \inb{\textbf{$\mathcal{E}|\alpha$ was not defined.}}
        \begin{enumerate}
          \item if $g$ does not belong to the faults, namely $g \notin \mathcal{E}|\alpha$ then we add the arrows: 
            \begin{equation*}
              \begin{split}
                & \bigl\{  \{ \tilde{u} = (u,t, \cdot),  \tilde{v} = (v,t+1, \cdot)\} : \{\{u,t\},\{v,t+1\}\} \in E_{o} \text{ and } \tilde{v},\tilde{u} \in V_{\mathcal{H},X} \\ 
                &  \, \, \, \text { or }  (v,t+1, \cdot)\} : \{\{u,t\},\{v,t+1\}\} \in E_{o}  \phi(f) \text{ and } \tilde{v},\tilde{u} \in V_{\mathcal{H},Z} \bigr\} 
              \end{split}
            \end{equation*}
          \item Else, if $g \in \mathcal{E}|\alpha$ then, we add:   
            \begin{equation*}
              \begin{split}
                & \bigl\{  \{ \tilde{u} = (v,t, \cdot),  \tilde{v} = (v,t+1, \cdot)\} : \text{ such either } \tilde{u} \in V_{\mathcal{H},X} \text { or } \tilde{u} \in V_{\mathcal{H},Z} \bigr\} 
              \end{split}
            \end{equation*}
...
\inb{\textbf{ Keep the remark change the explantation.} }
\begin{remark}
...
\begin{definition}[Analysis Toom's Contour Up to Step $\tau$]
  For a Toric encoded quantum circuit $C$, a set of faults $\mathcal{E}$, outcomes $\alpha$, and a step $\tau$, let $C[\mathcal{E},\alpha]_{\tau}$ be the partial computation circuit of $C[\mathcal{E},\alpha]$ up to step $\tau$. We define the \textit{analysis Toom's contour at step $\tau$} $\mathcal{H}_{\tau} = (V_{\mathcal{H}}, E_{\mathcal{H}})$ to be the graph obtained by taking the Toom's contour of $C[\mathcal{E},\alpha]_{\tau}$ and adding edges as follows:
  \begin{enumerate}
    \item Let $t$ be the depth of $C[\mathcal{E},\alpha]_{\tau}$. We add the following additional arrows:
        \begin{equation*}
          \begin{split}
          ...
          \end{split}
        \end{equation*}
      \item Then, we add forks to preserve the property that any two vertices with the same time coordinate, which are connected to a third (with a $+1$ higher time coordinate) via arrows, are also connected by a fork.
  \end{enumerate}
\end{definition}
