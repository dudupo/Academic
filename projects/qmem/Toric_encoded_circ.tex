  ...
  \begin{enumerate}
    \item We define $t^{\star}$ as the time of the gate at step $\tau$ plus one, and then add for the vertices in $V_{\mathcal{H},\{X\}}$ the duplication of the previous layer, namely the subset:
      ...
      \item For the edges, we add the following arrows:
        \begin{equation*}
          \begin{split}
            \bigl\{ \bigl\{ \tilde{u} = (u,t^{\star} -1, \cdot ), \tilde{v}= (v,t^{\star},\cdot)  \bigr\} & \text{ if there exists a} d^{\prime}-1  \text{ dimensional face } f \\ 
            & \text{such either } \tilde{u},\tilde{v} \in V_{\mathcal{H},X} \text{ and } u,v \in f  \\
            & \text{or  } \tilde{u},\tilde{v} \in V_{\mathcal{H},Z} \text{ and } v,u \in f   \bigr\}  
          \end{split}
        \end{equation*}
