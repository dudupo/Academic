...
\inb{\textbf{Consider whether it's steel necessary and if not then delete it.}}
\begin{definition} 
...
For a time Pauli-path $(P_{1},..,P_{\mu+1}$, the Toom's contour is the restriction of the base graph to vertices that witness a deviation, namely, those that belong to qubits supporting $P_{1},P_{2},..,P_{\mu+1}$. 
...
\begin{definition}[Analysis Toom's Contour at Time $t$]
  For a Toric encoded quantum circuit $C$, a set of faults $\mathcal{E}$, and a time $t$, according to the standard order, let $C[\mathcal{E}]_{t}$ be the partial computation circuit of $C[\mathcal{E}]$ up to time $t$. We define the \textit{analysis Toom's contour at time $t$}, denoted by $\mathcal{H}_{t} = (V_{\mathcal{H}}, E_{\mathcal{H}})$, to be the graph obtained by taking the Toom's contour of $C[\mathcal{E}]_{t}$ and adding vertices and edges as follows:
  \begin{enumerate}
    \item We add for the vertices in $V_{\mathcal{H},\{X\}}$ the duplication of the previous layer, namely the subset:
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t+1}  & : v_{t+1}=( v,t+1,\{X\}  ) \text{ if there exists a face } f \in \text{ supp } X(P_{t}) \\ 
            & \text{ such } v \in f \in X(P_{t}) \bigr\} 
          \end{split}
        \end{equation*}
        Similarly we add to $V_{\mathcal{H}, Z}$ the subset: 
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t+1}  & : v_{t+1}=(  v,t+1,\{Z\} ) \text{ if there exists a face } f \in \text{ supp } Z(P_{t}) \\
