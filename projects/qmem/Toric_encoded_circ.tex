...
\begin{definition}
  Let $\{u_{i}\}_{i}$ be the gates of the subjected quantum circuit $C[\mathcal{E}]$, placed according to the steps' order, and let $\mathcal{J}$ be the gates which are not belong to either $\mathcal{E}$, or to a sweep-cycle layer. We define the ideal evolution superoperator, denoted by $\Phi_{i}(\rho)$, to be:
  \begin{equation*}
    \begin{split}
\Phi_{i+1}(\rho) = 
      \begin{cases}
  ...
  \begin{enumerate}
    \item If $g$ is a gate inside a sweep cycle, meaning that $g$ measures a local syndrome and then applies correction, then there are Paulis $P^{\prime}, Q^{\prime}$ such that:
      \begin{equation*}
        ...
        \end{split}
      \end{equation*}
     \item If $g$ is a projective measurement then   
      \begin{equation*}
        \begin{split}
          g \circ \bigl( P \Phi_{\tau}(\rho) Q \bigr) = P^{\prime} \bigl( g \circ \Phi_{\tau}(\rho) \bigr) Q^{\prime}
        \end{split}
