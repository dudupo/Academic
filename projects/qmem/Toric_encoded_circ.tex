...
\inb{\textbf{ Consider defining the Pauli-path recursively: $P_{i}$ is a reduced Pauli, then $Q_{i+1}$ supports $C|_{i} \circ P_{i}|_{[w]}$, and $P_{i+1}$ is the reduced form of the $\infty$-extension of $Q_{i+1}$. Another option is not returning back to the finite Toric.  }}

\begin{definition} 
  \label{def:paulipath}
%For two Pauli $P, Q \in \mathcal{P}$, we define the relation $P \sim Q$ to hold if one is a translation of the other by a stabilizer of the $(d, d^{\prime})$-Toric code. Notice that $P$ and $Q$ might span multiple registers, and in that case, the translation might be composed of translations in each of the registers. 
For a fixed quantum circuit $C$, where all its quantum registers are encoded with the Toric code, and a set of faults $\mathcal{E}$, such that the depth of $C[\mathcal{E}]$ is $\mu$, we will say that the Paulis $P_{1}, \ldots, P_{\mu+1} \in \mathcal{P}$ are fault paths of $C[\mathcal{E}]$ if there are Paulis $Q_{1},..,Q_{\mu+1}$, such: 

\begin{enumerate}
  \item $Q_{1}= P_{1}$ and they are the identity. 
  \item For every $t\le \mu$ the Pauli $Q_{t+1}$ supports $C[\mathcal{E}]|_{t} \circ P_{t}$. Namely:
  \begin{equation*}
    \begin{split}
      \Trr{} \big( C[\mathcal{E}]|_{t} P_{t} (C[\mathcal{E}]|_{t})^{\dagger} Q_{t+1} \big) \neq 0
    \end{split}
  \end{equation*}
  Denote by $Q^{\infty}_{t+1}$ the view of $Q_{t+1}$ in the $\infty$-extension. Then, $P_{t+1}$ is obtained from $Q_{t+1}$ as follows:  
  \begin{equation*}
    \begin{split}
      P_{t+1} = 
      \begin{cases}
        \Trr{}_{/[w]} ( \text{ reduced form of }  Q_{t+1}^{\infty} )  & \text{ if } Q_{t+1}^{\infty} \text{ is finite }  \\ 
        \Trr{}_{/[w]} ( Q_{t+1}^{\infty}) & \text{ else }
      \end{cases}<++>
    \end{split}
  \end{equation*}
\end{enumerate}
...
$P_{i} \sim Q_{i}$ for any $i \in [\mu +1]$ and in addition, $Q_{1}$ is the identity and
...
For a reduced Pauli-path $P_{1},..,P_{\mu+1}$, the Toom's contour is the restriction of the base graph to vertices that witness a deviation, namely, those that belong to qubits supporting $P_{1},P_{2},..,P_{\mu+1}$. 


\begin{definition}[Toom's Contour Graph of $C(\mathcal{E})$, (Standard form)]
    \label{def:contour}
    For a Toric encoded quantum circuit $C$, set of faults $\mathcal{E}$ and a reduced Pauli-path $P_{1},..$, let us denote the base graph of $C[\mathcal{E}]$ by $G_{o}$. We define the \textit{Toom's contour graph} $\mathcal{H} = ( V_{\mathcal{H}}, E_{\mathcal{H}})$ as follows: 
    \begin{enumerate}
...
\begin{definition}[Analysis Toom's Contour at Step $\tau$]
  ...
  \begin{enumerate}
    \item We define $t^{\star}$ as the time of the gate at step $\tau$ plus one, and then add for the vertices in $V_{\mathcal{H},\{X\}}$ the duplication of the previous layer, namely the subset:
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t^{\star}}  & : v_{t^{\star}}=( v,t^{\star},\{X\}  ) \text{ if there exists a face } f \in \text{ supp } X(P_{t^{\star}-1}) \\ 
      ...
      \begin{equation*}
          \begin{split}
            \bigl\{ v_{t^{\star}}  & : v_{t^{\star}}=(  v,t^{\star},\{Z\} ) \text{ if there exists a face } f \in \text{ supp } Z(P_{t^{\star}-1}) \\
      ...
      \item For the edges, we add the following arrows:
        \begin{equation*}
          \begin{split}
            \bigl\{ \bigl\{ \tilde{u} & = (u,t^{\star} -1, \cdot ),  \tilde{v}= (v,t^{\star},\cdot)  \bigr\}  \text{ if  either there exists }\\ 
           & \text{ a } d^{\prime}-1  \text{ dimensional face } f  \text{such } \tilde{u},\tilde{v} \in V_{\mathcal{H},X} \text{ and } u,v \in f  \\
          & \text{or there exists a } d - d^{\prime}-1  \text{ dimensional face } f \text{ such } \tilde{u},\tilde{v} \in V_{\mathcal{H},Z} \text{ and } v,u \in \phi^{-1}(f)   \bigr\}  
          \end{split}
        \end{equation*}
      \item Finally, we add forks for vertices at time $t^{\star}$ to preserve the property that any two vertices with the same time coordinate, which are connected to a third (with a $+1$ higher time coordinate) via arrows, are also connected by a fork.
  \end{enumerate}
