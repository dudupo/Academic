...
%        \end{equation*}
      ...
      \item The edges $E_{\mathcal{H}} \subset E_{o} $ is partitioned into two types $A_{\mathcal{H}}$ and $F_{\mathcal{H}}$, where for any logical gate $g$ which applied in time $t$ in $C[\mathcal{E}]^{\infty} $ we add edge to $A_{\mathcal{H}}$ as follow: 
        \begin{enumerate}
          \item If $g$ is either a logical Pauli, a measurement in the computational base, or an identity, then for any face $f$ in the register on which $g$ acts, we add:
            \begin{equation*}
              \begin{split}
              ...
              \end{split}
            \end{equation*}
          \item \inb{\textbf{In fact we don't need that, since we are going to uses the replacement argument-\Cref{claim:replace}.}}   If $g$ is a measurement in the phases base, then for any face $f$ in the register on which $g$ acts, we add:
            \begin{equation*}
              \begin{split}
