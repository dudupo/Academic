  ...
  \begin{definition}[Tooms Contour Graph of $C(\mathcal{E})$]
    ...
    \begin{enumerate}
      \item The vertices $V_{\mathcal{H}}$ are the vertices of faces supporting the reduced Pauli-path. Namely:
        \begin{equation*}
          \begin{split}
            V_{\mathcal{H}} &\colonequals \big\{ v_{t} : v_{t}=(v,t) \in V_{o} \text{ and there exists a face } f \in \text{ supp } P_{t} \text{ such } v \in f \big\} 
          \end{split}
        \end{equation*}
        ...
        \begin{enumerate}
          \item If $g$ is either a logical Pauli, a measurement (both in the computational and phase basis), or an identity, then for any face $f$ in the register on which $g$ acts, we add:
            ...
            \begin{equation*}
              ...
              \end{split}
            \end{equation*}
          \item If $g$ is a logical $\mathbf{H}$. 
        \end{enumerate}
        In addition for any fault $g$ of $\mathcal{E}^{\infty}$, that is applied at time $t$, and any face $f \in \text{Locc}(g)$, we add the edges:
            \begin{equation*}
              \begin{split}
  ...
  \begin{enumerate}
    \item For vertices, we define $t^{\star}$ as the time of the gate at step $\tau$ plus one, and then add duplication of the previous layer, namely the subset:
      ...
      \item For the edges, we add the following arrows:
        \begin{equation*}
          \begin{split}
            \bigl\{ \bigl\{ (u,t^{\star} -1 ), (v,t^{\star})  \bigr\} & \text{ if there exists a  face } f \in \partial P_{t^{\star} - 1} \\ 
