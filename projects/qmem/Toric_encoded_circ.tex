...
\begin{definition}
  ...
  With the same notations as in \Cref{def:paulipath}, let $\tau$ be a step in $C[\mathcal{E}]$. Recall that each gate in $C[\mathcal{E}]$ acts on at most $\Delta$ qubits. Let $\Delta^{\prime} \in [\Delta]$. In addition, denote by $g$ and $t$ the gate and its time which is applied at step $\tau$. We say that the set of Pauli $P_{1},\ldots,P_{t},P_{\star}$, is a Pauli-path up to step $(\tau,\Delta^{\prime})$, if the $P_{1},..,P_{t}$ is a Pauli-path for the circuit obtained form $C[\mathcal{E}]$ by taking only its first $t-1$ layers. And $P_{\star}$ satisfies the follow: let $g^{\prime}_{1},..,g_{l}^{\prime} = g$ be the gates applied at time $t$ at step at most $\tau$, placed by order. Let $P^{\prime}_{0},P^{\prime}_{\Delta} \in \mathcal{P}^{(\cdot)}$ be Paulis such:     
  \begin{equation*}
    \begin{split}
      P_{0}^{\prime} \in \bigl(g^{\prime}_{l-1}g^{\prime}_{l-2}..g_{1}\bigr) \circ P_{t} \qaud P^{\prime}_{\Delta} \in g \circ P_{0}^{\prime}
    \end{split}
  \end{equation*}
