...
\begin{definition}
  Consider a quantum circuit subjected to noise $C[\mathcal{E}]$, with width $w$ and total number of steps $\tau_{f}$. We say that the set of Pauli's tuples $\{ (P_{i},Q_{i})  \}_{1}^{\tau_{f}}$  where for each $i$  $ (P_{i},Q_{i}) \in  \mathcal{P}^{[w]} \times  \mathcal{P}^{[w]}$ is a Pauli-path for $C[\mathcal{E}]$. If:
  \begin{equation*}
    \begin{split}
    ...
    \end{split}
  \end{equation*}
  \inb{\textbf{Extend to Pauli-paths according times for inducting Toom's contour. }}
\end{definition}
...
\begin{definition} 
 ...
 \inb{\textbf{ Continue to here. }}
Consider a quantum circuit $C$, where all its quantum registers are encoded with the Toric code with a side of length $n$, and a set of faults $\mathcal{E}$, such that $C[\mathcal{E}]$ has depth $\mu$ and width $w$. We will say that the Paulis $P_{1}, \ldots, P_{\mu+1} \in \mathcal{P}^{[w]}$ are fault paths of $C[\mathcal{E}]$ if:
\begin{enumerate}
  \item $P_{1}$ is the identity. 
  \item For every $t\le \mu$ there is a Pauli $Q \in \mathcal{P}$, such $Q$ supports $C[\mathcal{E}]|_{t} \circ P_{t}$. Namely:
  \begin{equation*}
    \begin{split}
      \Trr{} \big( C[\mathcal{E}]|_{t} P_{t} (C[\mathcal{E}]|_{t})^{\dagger} P_{t+1} \big) \neq 0
    \end{split}
  \end{equation*}
And the view of $P_{t+1}$ in the $\infty$-extension is the reduced form of $Q$ in the $\infty$-extension.
\end{enumerate}
