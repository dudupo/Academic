...
Where we used that the term in the closure is invariant to sweeping between $g$ and $g^\prime$, and therefore any term in the sum appears twice. Similarly:
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  The conditions $ (\partial^{\pm})^{\circ 2} = 0 $ implais that   $ \mathbb{F}_{2}^{|\mathcal{F}_{i}|} \substack{ \rightarrow^{\partial^{-}} }{ \leftarrow^{\partial^{+}}  } \mathbb{F}_{2}^{|\mathcal{F}_{i-1}|}$ is a chain complex. The same cancellation argument applies dually to the coboundary operator. Equivalently, every $d^{\prime}-1$ face and every $d^{\prime}+1$ face overlap on an even number of $d^{\prime}$-faces, so the corresponding $X$- and $Z$-checks commute. This is exactly the CSS condition.
\end{proof}
