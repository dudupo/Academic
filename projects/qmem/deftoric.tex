...
\end{claim}
\begin{proof}
  We will show that $ \mathbb{F}_{2}^{|\mathcal{F}_{i}|} \xrightarrow{ \partial^{\pm} } \mathbb{F}_{2}^{|\mathcal{F}_{i \pm 1}|}$ \inb{are a}\footnote{Check if that is grammatical correct.} chain complex and its dual chain. First, we show that $\left( \partial^{+} \right)^\top = \partial^{-}$. Consider vertices $v, u \in V$ and two generator subsets $A,B \subset S$. Consider the basis elements defined by them: $\rotfv{v}{A}$ and $\rotfv{u}{B}$. Suppose that:
  \begin{equation*}
    \begin{split}
