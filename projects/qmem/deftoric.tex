...
The $X$-checks defined by $\partial$ and the $Z$-checks defined by $\partial^{*}$ commute, and therefore the resulting toric code is a CSS code.
\end{claim}
\begin{proof}
We will show that $ \mathbb{F}_{2}^{|\mathcal{F}_{i}|} \xrightarrow{ \partial^{\pm} } \mathbb{F}_{2}^{|\mathcal{F}_{i \pm 1}|}$ is a chain complex and its dual chain. First, we show that $\left( \partial^{+} \right)^\top = \partial^{-}$. Consider vertices $v, u \in V$ and two generator subsets $A,B \subset S$. Consider the basis elements defined by them: $\rotfv{v}{A}$ and $\rotfv{u}{B}$. Suppose that:
  \begin{equation*}
    \begin{split}
...
Means that there exists $g \notin A$ such that $A \cup \{g\} = B$ and either $v = u$ or $g^{-1}v = u$. But that is equivalent to saying that there is $g \in B$ such that $B \setminus \{g\} = A$ and either $v = u$ or $v = g^{-1}u$. In other words:
  \begin{equation*}
    \begin{split}
       & ( \partial^{+} \rotfv{v}{A} ) \rotfv{u}{B}   =  \Leftrightarrow \rotfv{v}{A} ( \partial^{-} \rotfv{u}{B} ) \\ 
       & \left( \partial^{+} \right)^\top = \partial^{-} 
    \end{split}
  \end{equation*}

  So it's left to show that the boundary of a boundary is zero. Cinsider a basis vector $\rotfvs{}$:
  \begin{equation*}
    \begin{split}
