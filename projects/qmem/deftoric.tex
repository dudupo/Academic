...
We denote the complex by $G = (V, E, F)$, where $V$, $E$, and $F$ are the $0$-, $1$-, and $2$-faces. Below, we formalize binary assignments on the toric complex and introduce boundary operators. These boundary operators are the same as standard boundary operators in the chain complex formalism.
...
\end{claim}
\begin{proof}
  It is enough to check the claim on basis vectors. For a basis vector $\rotfvs{}$, we have:
  \begin{equation*}
    \begin{split}
...
Where we used that the term in the closure is invariant to sweeping between $g$ and $g^\prime$, and therefore any term in the sum appears twice. Similarly:
  ...
  \begin{equation*}
    \begin{split}
      & ( \partial^{+} \rotfv{v}{A} ) ( \partial^{-} \rotfv{u}{B} )  \\
      & = \left(\sum_{g \in S/A} \rotfv{v}{A \cup g} + \rotfv{g^{-1}v}{ A \cup g }\right)  \left(\sum_{g \in B} \rotfv{u}{B / g} + \rotfv{gu}{ B / g }\right) \\
    \end{split}
  \end{equation*}
Split into cases based on whether $v$ equals $u$.
\begin{enumerate}
    \item If $v = u$, then for any $g, g^{\prime} \in S$, it holds that $gv \neq u$, $g^{\prime -1}u$, and $v \neq g^{\prime -1}u$. Thus, only terms of the form $\rotfv{v}{A \cup g}\rotfv{u}{B / g^{\prime}}$ can contribute to the product, yet for any $g, g^{\prime}$ for which this term is not zeroing out, it follows that $g \in B$ and $g^{\prime} \notin A$, and therefore the term $\rotfv{v}{A \cup g^{\prime}}\rotfv{u}{B / g}$. Therefore, the number of non-zero terms is even, namely the sum equals zero.

    \item If there is $g$ such that $gv = u$, then first notice that for any $g^{\prime}, g^{\prime \prime} \in S/g \cup \{e\}$, we have $g^{\prime}v \neq u$. So only terms of the form either $\rotfv{gv}{A \cup g}\rotfv{u}{B / g^{\prime}}$ or $\rotfv{v}{A \cup g^{\prime}}\rotfv{g^{-1}u}{B / g}$. Yet if $\rotfv{gv}{A \cup g}\rotfv{u}{B / g^{\prime}} \neq 0$, then $g \in B$ and $g^{\prime} \notin A$. So $\rotfv{v}{A \cup g^{\prime}}\rotfv{g^{-1}u}{B / g}$ is also not zeroing out, the number of non-zeros is even, and the sum equals zero.
\end{enumerate}
