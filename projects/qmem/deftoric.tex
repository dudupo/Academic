...
\begin{definition}[$d$-Dimensional Toric Complex] 
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  The $i$-faces of the complex are the collection of all the possible rooted spanned $i$-faces induced by $\text{Cay}(\mathcal{G}_{n}^{d})$. We will denote them by $\mathcal{F}_{i}$. Additionally, let us define the multiplication of the generators subset $S^{\prime} \subset S$ by a vertex $v \in V(\mathcal{G})$ as follows:
  \begin{equation*}
    \begin{split}
      S^{\prime} v = \left(\prod_{g \in S^{\prime}}g^{\prime} \right) v
    \end{split}
  \end{equation*}
   Finally, we name $n$ and $d$, the \textit{side length} and the \textit{dimension} of the complex.  
\end{definition}
...
The fact that $\partial^{\pm} :\mathbb{F}_{2}^{|\mathcal{F}_{i}|} \rightarrow \mathbb{F}_{2}^{|\mathcal{F}_{i \pm 1}|}$ induces a chain complex motivates referring to the collection of qubits/checks/vertices as chains of various dimensions. Next, we give a definition of the 'positive' vertices that a vertex $v$ sees, which lie on a chain $z$.
