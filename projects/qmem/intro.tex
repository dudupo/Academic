...
\paragraph{Toom's Contours.} A sufficient, simple, definition of Toom's contours in \Cref{Berman} would be a directed graph that describes a nosiy running with the addition marking of devrgenced bits. In quantum case, the vertices of the Toom's contour are nor bits neither checks, instead they are the vertices on the Torices that 'devrgenced'. And a proper conceptual definition would be: 
\begin{center}
\begin{minipage}{0.75\textwidth}
  \textit{Your paragraph text goes here. It will be padded on the left and right, constrained to 75}
\end{minipage}
\end{center}

Here,  derives a directed graph called Toom's contour. We adjust the Toom's contours as the follow graph $\mathcal{H} \colonequals (V_{\mathcal{H}}, E_{\mathcal{H}})$: 
\begin{enumerate}
