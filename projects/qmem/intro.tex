...
Similarly to other fault tolerance approaches, we use the concatenation construction in order to initialize zeros and resource states encoded in the 4D-Toric code, as referred to in \Cref{grospellier:tel-03364419}, \Cref{gottesman2014faulttolerant}. From there, we assume that all the quantum registers are encoded in the 4D-Toric code. Then, after the initialization step, the logical cycles are divided into error correction, which is a single sweep-cycle, and 'logical operation' cycle. For our universal logical gate set, we choose $\mathbf{CX}$ (controlled NOT), $\mathbf{H}$, $\mathbf{S}$, and $\mathbf{T}$ gates. Except for the 'Controlled NOT' gate, which is realized by transversal wire-wise physical 'Controlled NOT' gates, all the others are implemented via gate teleportation. We emphasize that even though the Hadamard gate has a fold-transversal realization and therefore also the $\mathbf{S}$ gate does, we have failed to provide proof that restricts the expansion of the bounded-triangle\footnote{The permutation used in the Hadamard realizations mirrors the errors}.

The raliztion by gate teleportation is done as follow, first we xoring the target state into the a resource state via $\mathbf{CX}$, 

Here we describe the construction, including the realization of the gates by  and discuss the initialization stage. 
