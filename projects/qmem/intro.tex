...
\begin{definition}[Local stochastic noise channel (Informal)]
...
In this work, we present a fault-tolerance construction that uses the 4D Toric code and has a constant-depth 'correction' cycle. In other words, we showed that non-hierarchical correction cycle is not a property belongs only to the good quantum LDPC or the Hyperproduct code.

\begin{theorem}[Informal]
  There exists a constant $p_{0} \in (0,1)$ such that for any local stochastic noise channel $\mathcal{N}_{\mathcal{E}}$ with rate $p < p_{0}$, a quantum circuit $C$ with depth $\mu$ and width $w$, and a, big enough integer $n$. There exists an 'logical equivalence' circuit $\tilde{C}$, with depth $ \Theta(\mu n) $ and width $ \Theta( (w +\mu) n)$  that simulates $C$ with probability: 
\begin{equation*}
  \begin{split}
