...
%\scalebox{0.8}{
  \begin{table}[H]
    ...
    {\footnotesize
      \begin{tabular}{l p{6cm} p{2cm} l l}
        \toprule
         Works Line  & Code   & Constant-depth correction cycle  & Depth and qubit overhead   \\
        \midrule
        {\sc Concatenation }   & Concatenation of fixed length code \cite{aharonov1999faulttolerant}  & \ding{55}  & $f(x)$  \\
        {\sc High-rate }       & \cite{grospellier:tel} \cite{Fawzi_2018}, \cite{nguyen2025quantumfaulttoleranceconstantspace} &  \ding{51}     & $f(x)$    \\
        {\sc Quantum Automata} & \cite{Balasubramanian_2026}, \cite{dünnweber2026quantummemoryautonomouscomputation}              & \ding{55}        & $f(x)$   \\
