...
In studying fault tolerance, the first challenge is defining when 'things go bad'. In von Neumann-style proofs \cite{ }, \cite{ }, it's common to track the number of flipped bits. In the work of Aharonov and Ben-Or \cite{aharonov1999faulttolerant}, a logical bit at the $n$ level admits a logical error if at least a constant fraction of the $n-1$ level logical bits that compose it admit an error. That can be thought as an adaptation  Gacs and Reif ideas \cite{GACS1988125}, Where the $n$-logical bit corresponds to colony, in their approach corrections done via the Toom's rule which is uniform for all the bits.  For our case, each of the approaches present a challenge. First, keeping track over the bits, since our correction code has sublinear distance and we excpect to at linear number of bits to be flipped .., In the concatination method, simultaing a one level below,  a 'correction' cycle at $O(1)$-depth is our goal, we have to give up a full corrections,  In  \cite{GACS1988125}, the 
