...
We modulate the noisy operation of a given circuit $C$ as follows: first, we sample space-time locations for the faults and denote the outcome by $\mathcal{E}$. Then, we impose layers of computation alternately, such that the even and odd layers are taken from $\mathcal{E}$ and $C$ according to their temporal order. The combination of the circuit $C$ with a subset of faults $\mathcal{E}$ defines a possible running and we denote it by $C[\mathcal{E}]$. 
