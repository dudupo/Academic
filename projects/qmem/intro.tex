...
The computation model we consider is quantum circuits subject to \textit{local-stochastic noise}, where only qubits are subjected to the noise, while classical bits are non-noisy. Classical computation is allowed, and the classical depth matters. Namely, at each time unit, only a constant depth of classical computation can be done. 

Local stochastic channel is defined to act such that the probability of any subset of locations $S$ (space-time coordinates), which is picked prior to the channel's action, being affected decays exponentially with the subset size. 
...
For circuits $\tilde{C}$ and $C$, we say that $\tilde{C}$ simulates $C$ with probability $p$ if, when one stops a noisy run of $\tilde{C}$ at a midpoint, they can, with probability $p$, recover by non-noisy operation an 'image' of a midpoint in the running of $C$, and if stopping at the last time point of $\tilde{C}$ enables recovery of the 'image' of $C$ at the end of its run.
...
Thus, when the qubit overhead is not a consideration, it means that the 4D Toric code is as good as the good quantum LDPC / Hyperproduct code. That translates to the two following applications: 
\begin{enumerate}
    ...
    \item Memory in the circuit model: Since protecting a 4D Toric encoded state for an exponentially long time is done by performing the same operation at each time unit. Then, combining with the simplicity of the presentation of the 4D Toric code, it suggests that the 4D Toric code is a promising candidate for memory in the circuit model.  
\end{enumerate}
