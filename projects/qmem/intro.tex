...
In this work, we present a fault-tolerance construction that uses the $(d,d^{\prime})$-Toric code and has a constant-depth 'correction' cycle, for any $d^{\prime},d-d^{\prime}>2$. In other words, the construction uses a relatively simple code - the 4D Toric code, yet doesn't incur the cost of a hierarchical correction cycle.
...
\begin{minipage}{0.75\textwidth}
  \textit{
A graph that describes a noisy run from the Torices' perspective. In particular, the vertices of the graph are the vertices of the Torices, equipped with a time coordinate, that are adjacent to unsatisfied checks.
  }
\end{minipage}
\end{center}
We adjust their construction as follows: Denote the graph by $\mathcal{H} \colonequals (V_{\mathcal{H}}, E_{\mathcal{H}})$ and
\begin{enumerate}
  \item If at time $t$, a vertex $v$, on the one the Toric complexes\footnote{Actually the infinite grid.} corresponding to one of the quantum registers, sees an unsatisfied check on it's local view, then add $(v,t)$ to the vertices. 
  \item If at time $t$, vertex $v$ sees an unsatisfied check, and at time $t+1$ its neighbor $u$ sees an unsatisfied check, then we think of the situation as '$v$ 'leads' to $u$' and connect $(v,t)$ and $(u,t+1)$ with an 'arrow'. The vertices $v$ and $u$ might belong to different registers (which matches the case that at time $t$ we apply an operator on both registers). For convenience, we don't require $u$ at time $t$ to see only satisfied checks.
...
One technical change that we made and haven't discussed yet is that in the gluing of the last layer, which was defined to capture a partial layer execution up to a turn, we connect by arrows any two vertices that were added to $ V_{\mathcal{H}} $ and share a check. By doing so, we ensure that connected components in the Tanner graph would also be connected by arrows in Toom's contours. We emphasize that such edges connection can be made only in the last stage, as it doesn't pass the induction step. However, the final slice that is formed passes the desired inequalities only by constants and is therefore sufficiently good to derive the probability bound.
