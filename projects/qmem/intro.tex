...
\begin{enumerate}
    ...
    \item 
      Memory in the circuit model: Since protecting a 4D Toric encoded state for an exponentially long time is done by performing the same operation at each time unit, it suggests that the 4D Toric code is a promising candidate for memory in the circuit model. Combining with the evidence the 4D-Toric code is also a memory in thermal noise models, and with it's relative simplicity, it seems a natural choice to a real-world memory applications. 
\end{enumerate}
