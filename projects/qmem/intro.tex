...
\end{definition}
See \Cref{def:sweeprule} for the formal version. To see the effectiveness of the sweep rule, consider an error that does not close a non-trivial loop on the Toric. Such errors have 'corners', and in particular, one can think of the most 'negative corner'. In the 2D case, it is the most 'bottom-left' vertex that supports the error. For simplicity, let's call that vertex the corner of the error. In \cite{Kubica_2019}, it was argued that, at the end of a sweep-rule excitation, the corner always suggests a non-trivial correction. Thereby advancing the boundary of the error toward shrinking.
That argument is easy to verify for low dimensions; for higher dimensions, the authors leave it as a conjecture. In this work, we prove that conjecture and, from the proof, derive an algorithm for finding the correction. 



A full error correction cycle, or a sweep-cycle, consists of applying the sweep-rule to each vertex and then doing the same in the dual space. \Cref{fig:sweep} provides an example of the result of the sweep rule given a syndrome.
