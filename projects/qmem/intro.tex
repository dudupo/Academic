...
%\scalebox{0.8}{
  \begin{table}[h]
    ...
    {
      \footnotesize
      \begin{tabular}{l l p{2cm} p{2cm} l l}
        \toprule
        { \sc Works Line }  &   Error correction code   &   Constant-depth \newline correction cycle  &    Depth and qubit overhead  &  Ref    \\
        \midrule
        {\sc Concatenation }   & Concatenation of fixed length code  & \ding{55}  & $f(x)$  & \cite{aharonov1999faulttolerant} \\
        {\sc High-rate }       & &  \ding{51}     & $f(x)$ &  \cite{grospellier:tel-03364419}, \cite{Fawzi_2018}, \cite{nguyen2025quantumfaulttoleranceconstantspace}  \\
        Quantum Automata & 2D-Toric code    & \ding{55}        & $f(x)$ &  \cite{Balasubramanian_2026}, \cite{dünnweber2026quantummemoryautonomouscomputation}  \\
