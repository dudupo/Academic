...
Then, we prove that if the bounded-fan circuit model (and also the errors are bounded-fan since they are single qubit Pauli matrices in the steps presentation), in each step a cluster of errors can increase by at most a constant factor (see \Cref{claim:deviation_clusters}). Equipped with that, we infer the intermediate theorem.
...
Here we describe the construction, including the realization of the gates by gate teleportation, and discuss the initialization stage. 
