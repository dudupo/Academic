...
\begin{claim}
Let $C$ be a Toric encoded circuit such each of its quantum register is ecnoded by $n$-length , $(d,d^{\prime})$-Toric code, and let $\mathcal{N}_{\mathcal{E}}$ be a local stochastic noise channel, with a noise rate $p$. Then, for any register $\mathcal{R}$ and a step $\tau$ in the standard order of $C[\mathcal{E}]$, the probability that $\tau$ is the first setp such there is a deivation cluster at size greather than $ \frac{1}{2} n^{\frac{d^{\prime}}{d}} $ is less than $ <++> $. 
\end{claim}

  \begin{proof}
    We denote by $A_{\tau}$ the event that $\tau$ is the first step to have a diveation cluster on register $\mathcal{R}$ at size greater than $\frac{1}{2}n^{\frac{d^{\prime}}{d}}$, and by $B_{\tau}$ the event that all the explantations of deviation clusters on register $\mathcal{R}$ at step $\tau$ have less than $n^{\frac{1}{d}}$ edges. Thus:  
    \begin{equation*}
      \begin{split}
