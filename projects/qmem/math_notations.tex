...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
For a quantum gate $U$, we denote by $\ket{U}$ the resource state that can be used to simulate $U$. Notice that, in general, the resource state is not unique. In this work, we address specific states that will be discussed later. The function $\prb{}$ is the probability function.  
