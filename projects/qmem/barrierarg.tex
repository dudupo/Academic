...
\end{claim}
\begin{proof}
  Let $x^{\prime}$ be the $\infty$-lift of $x$, we argue that the projection of any subset $y \subset x^{\prime}$, which doesn't contain a vertex $v$ and a translation of it $v + \mathbf{1}_{g} n$, can't closes a non-trivial loop. 
\end{proof}
...
We argue that each connected component of $\partial^{-}x^{\prime}_{i}$ has a size of at most $\Delta c n$ (\inb{\textbf{Prove that!}}). Now, define for any $i,j \in [1 + \Delta t]$ the quantity:
\begin{equation*}
  \begin{split}
