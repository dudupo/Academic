...
\begin{claim}
  Let $x_{1},x_{2} \in (\mathcal{F}^{n}_{d^{\prime}}; \mathbb{F}_{2})$ be $d^{\prime}$-dimensional chains, such the Hamming distance between $x_{1}$ and $x_{2}$ is at most $l$.  
\end{claim}
\begin{proof}
...
\begin{equation*}
  \begin{split}
    [x_{i+1} ] \sim_{\mathcal{C}_{Z^{\perp}}}  [x^{\prime}_{i\Delta+1} ] + [x^{\prime}_{(i+1)\Delta+1} ]  \sim _{\mathcal{C}_{Z^{\perp}}}  w_{i \Delta +1 ,(i+1)\Delta + 1}  
  \end{split}
\end{equation*}
And observes that: 
\begin{equation*}
...
\begin{equation*}
  \begin{split}
    \Size{ w_{i \Delta +1 ,(i+1)\Delta + 1} }  \le \sum_{j = i \Delta +1 }^{(i+1)\Delta} \Size{ w_{j ,j + 1} } \le  \Delta d + { d^{\prime} \choose 2 }\Delta c n  
  \end{split}
\end{equation*}
\end{proof}
