...
The $Z$-errors are 'handled' at the second phase of the sweep-cycle. In that stage, we move to the dual code, which is a $(d,d-d^{\prime})$-Toric code by the application of $\mathbf{H}_{\phi}$. Then, \Cref{claim:sweepshrink} guarantees that the error is shrunk relative to the dual image. What we would like to prove is that after returning to the original code, the error is shrunk. 
