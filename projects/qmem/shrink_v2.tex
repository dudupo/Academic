...
Consider an Pauli $P$, whose $X$- and $Z$-components are set on the reduced chains $x, z \in \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}}}$. Denote by $\tilde{P}$ the Pauli operator result by an application of the sweep-rule on $P$ and by $\tilde{x}, \tilde{z} \in \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}}}$ the chains corresponding to the $X$- and $Z$-components of $\tilde{P}$. If both $x, \xi(z)$ are finite connected chain, Then:
  \begin{enumerate}
    \item For any $i \in [d]$: 
      \begin{equation*}
        \begin{split}
