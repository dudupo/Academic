...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Let $v$ be a vertex that is a root of face in $\phi(z)$, we argue that $v$ is also in $z$. Consider a face $\rotfvs{}$ in $\phi(z)$ then $\phi^{-1}(\rotfv{v}{S^{\prime}}) = \rotfv{ (S/S^{\prime})^{-1} v}{S/S^{\prime}}$ is in $z$, and therefore $ (S/S^{\prime}) (S/S^{\prime})^{-1} v = v$ is in $z$. By the same arguments we have also that if $v$ is a root of face in $z$ then the vertex $S v$ is in $\phi(z)$. Now, fix a generator $g \in S$ and let $u$ and $w$ be the vertices in $z$ and $\phi(z)$ that achieve the maximum for $L_{g}$. Denote by $u_{r}$ and $w_{r}$ the vertices which are the roots of the faces contain $u$ and $w$ and observes that $L_{g}(w) \le L_{g}(w_{r})+1$ for any $g \in S$. We have that: 
    \begin{equation*}
      \begin{split}
