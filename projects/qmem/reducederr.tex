...
\inb{\textbf{Think if here is indeed the place.}} 

\begin{definition}
For a Pauli $P$, let $x$ and $z$ be the chain supports of its $X$ and $Z$ components, namely $P = X(x)Z(z)$. Then, we define the size of $P$ to be:
  \begin{equation*}
    \begin{split}
      \Size{P} = \max\{ \Size{x}, \Size{\phi(z)} \}
    \end{split}
  \end{equation*}
