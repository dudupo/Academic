...
\begin{definition}
  \label{def:finiteerr}
  Let $z$ be a $d^{\prime}$-dimensional chain. Consider the graph whose vertices are the faces of $z$, and any two vertices are connected by an edge if their intersection is a $d^{\prime}- 1$ dimensional face or if they are both contained in a $d^{\prime}+1$-dimensional face. Then $z$ is said to be connected if that graph is connected. In addition, we will say that a connected chain $z$ is finite if:
  \begin{equation*}
    \begin{split}
