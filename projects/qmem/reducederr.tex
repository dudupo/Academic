...
We prove the claim by induction over the dimensions $d, d^{\prime}$. For the base, we take all the cases where $d^{\prime} = 2$ and $d$ is any number greater than $d^{\prime}$. To prove the base, we will need the following claim:
\begin{claim}
...
Assume that $w|_{/gv} = 0$, which means that there are $d^{\prime}-2$-dimensional faces $A = \{\rotfv{v}{S^{\prime}}\}$ such that $w$ can be written as:
  \begin{equation*}
    \begin{split}
      w = \sum_{  \rotfvs{} \in A  } \rotfv{v}{S^{\prime} \cup g } + \rotfv{g^{-1}v}{S^{\prime} \cup g }
    \end{split}
  \end{equation*}
  and therefore:
  \begin{equation*}
...
\begin{proof}[ proof of \Cref{claim:agreement}]
