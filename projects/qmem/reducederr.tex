...
\begin{claim}
Let $x_{1}, \ldots, x_{t} \in (\mathcal{F}^{n}_{d^{\prime}}; \mathbb{F}_{2})$ be $d^{\prime}$-dimensional chains, such that $x_{1}$ is a trivial chain and for any $i \in [t-1]$, the Hamming distance between $x_{i}$ and $x_{i+1}$ is at most $\Delta$. There is a constant $c \in (0,1)$, depending only on $d, d^{\prime}$, and $\Delta$, such that if for any $x_{i} \in x_{1}, \ldots, x_{t}$, the size of each of the connected components of $\partial^{-} x_{i}$ is bounded by $c n$, then:
  \begin{equation*}
    \begin{split}
      [0]= [x_{1}] = [x_{2}] = ... = [x_{t}] 
    \end{split}
...
\end{claim}
\begin{proof}
  Assume through contradiction that there is $i$ such $0 = [x_{i}] \neq [x_{i+1}]$ denote by $ x_{i}^{1},x_{i}^{2},..$ and $x_{i+1}^{1},x_{i+2}^{2},..$ the reduced $d^{\prime}$-dimensional chains such $\partial^{-}x_{i}^{j}$ is a connected component of $\partial^{-}x_{i}$. Hence:  
  \begin{equation*}
    \begin{split}
      [x_{i+1}] = [x_{i+1}] + [x_{i}] = x_{i} + x_{i+1} + \sum_{j}x_{i}^{j} + \sum_{j}x_{i+1}^{j+1}
    \end{split}
  \end{equation*}
\end{proof}
