...
\begin{claim}
Let $x \in (\mathcal{F}^{n}_{d^{\prime}}; \mathbb{F}_{2})$ be a $d^{\prime}$-dimensional finite chain, such that the $d^{\prime}-1$-dimensional chain $\partial^{-} x$ is not connected. Suppose that $\Size{x} \le n$. Then there are $d^{\prime}$-dimensional chains $y_{1}, y_{2}, \ldots$ such that each $\partial^{-}y_{i}$ is a connected component of $\partial^{-}x$ and $\Size{y_{i}} \le n$.

\end{claim}

\begin{proof}
Consider a $d^{\prime}$-dimensional chain $x$ such that $\partial^{-} x$ has a decomposition $x = \tilde{x}_{1} + \tilde{x}_{2}$, where $\tilde{x}_{1}, \tilde{x}_{2}$ are $d^{\prime}-1$-dimensional chains, and $\tilde{x}_{1}$ is not connected to $\tilde{x}_{2}$. By disconnectivity, $\tilde{x}_{1}$ does not share a $d^{\prime}-2$ dimensional face with $\tilde{x}_{2}$. Thus, $\partial^{-} \tilde{x}_{1}$ and $\partial^{-} \tilde{x}_{2}$ are disjoint. On the other hand, since $\partial^{- \circ 2} x = 0$, then combining with disjointness, it follows that:
  \begin{equation*}
    \begin{split}
      \partial^{-}( \tilde{x}_{1} + \tilde{x}_{2} ) = 0 \Rightarrow \partial^{-} \tilde{x}_{1} = \partial^{-}\tilde{x}_{2} = 0 
    \end{split}
  \end{equation*}
  Namely, $\tilde{x}_{1},\tilde{x}_{2} \in \ker \partial^{-} : \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}-1}} \rightarrow \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}-2}}$. Suppose $\tilde{x}_{1} \notin \image \partial^{-} : \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}}} \rightarrow \mathbb{F}_{2}^{\mathcal{F}_{d^{\prime}-1}}$, then we have that $\tilde{x}_{1}$ is a non-trivial codeword of the $(d,d^{\prime}-1)$-Toric code. Since $d^{\prime} \ge 2$, the size of $\tilde{x}$ is at least $n$, and therefore the size of $x$ is at least $n$ as well, which is a contradiction. So we get that there are two $d^{\prime}$-dimensional chains $x_{1}$ and $x_{2}$ such that $\partial^{-}x_{1} = \tilde{x}_{1}, \partial^{-}x_{2} = \tilde{x}_{2}$. 


The reduced forms of $x_{1}, x_{2}$ have sizes lower than $n$. If both $\partial^{-}x_{1} $ and $\partial^{-}x_{2}$ are connected, then the reduced forms of $x_{1}, x_{2}$ satisfy the claim. If not, then repeat recursively on the reduced form for each whose $\partial^{-}$-boundary is not connected.
\end{proof}
