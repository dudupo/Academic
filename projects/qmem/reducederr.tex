...
Since $d^{\prime} \ge 2$, the size of $\tilde{x}$ is at least $n$, which is a contradiction. So we get that there is a $d^{\prime}$-dimensional chain $x_{1}$ such that $\partial^{-}x_{1} = \tilde{x}_{1}$. The reduced form of $x_{1}$ has size at most $\Size{\tilde{x}_{1}} \le \alpha n$, and satisfies the desired condition for the connected component $\tilde{x}_{1}$. To complete, we repeat recursively on the connected components of $\tilde{x}_{2}$.
\end{proof}
