...
The proofs of the claims are given in the appendix. We continue with the proof of the lemma.
  \begin{proof}
    ...
    \begin{claim}
      \label{claim:suff} 
Let $J$ be the set of steps in $\tilde{C}[\mathcal{E}]$. Denote by $\mathcal{J} \subset J \times \Delta$ the subset of tuples $(\tau, \Delta^{\prime})$ where $\tau \in J$, $\Delta^{\prime} \in [\Delta]$, and $\tau$ is a step of a gate in either a fault-layer or a sweep-cycle layer (namely, not a gate that is a part of logical gate realization). If, for any $(\tau,\Delta^{\prime}) \in \mathcal{J}$, no analysis of Toom's contour up to step $\mathcal{H}_{\tau,\Delta^{\prime}}$ has a slice of size greater than $cn$, then we will say that the 'no large syndrome component condition' is satisfied. With probability at least $q$, the condition is satisfied.
    \end{claim}
    \begin{proof}
      Proof.
    \end{proof}
    ...
    \begin{claim}
     Assume that the 'no large syndrome component condition' is satisfied. Then $\tilde{C}$ simulates $C$.
    \end{claim}
    \begin{proof}
    % Consider subset of faults $\mathcal{E}$ such 'no large syndrome component condition' is satisfied.
      Let $\rho_{1},\rho_{2},..,\rho_{l}$ the density matrices such $\rho_{i}$ is obtained by applying the gates of $\tilde{C}$ up to the $i$th logical-layer (excluding that layer). Similarly, let $\rho_{1}^{\prime},..,\rho_{l}^{\prime}$ be the density matrices that are obtained by taking prefixes of $\tilde{C}[\mathcal{E}] \sim \mathcal{N}_{\mathcal{E}}$ up to logical-layers. We prove by induction, over $i$, that for any register $R$: 
      \begin{equation*}
        \begin{split}
          \Trr{} \bigl( D \circ \rho^{\prime}_{i}|_{R} (  D \circ \rho_{i}|_{R} )^{\dagger}  \bigr) \ge q 
        \end{split}
      \end{equation*}
    \end{proof}
...
For a set of faults $\mathcal{E}$, let $\tau$ be the step in $\tilde{C}[\mathcal{E}]$ such that the following step $\tau+1$ begins a realization of $D$ in $\tilde{C}$. Denote by $\tilde{C}[\mathcal{E}]_{\rightarrow \tau }$ the circuit which is obtained from $\tilde{C}[\mathcal{E}]$ by taking gates up to step $\tau + 1$. Consider a Pauli-path $P_{1}, P_{2}, \ldots, P_{\star}$ for $C[\mathcal{E}]_{\rightarrow\tau}$. Decompose $P_{\star}$ into the product of Paulis $P_{\star} = P \otimes Q$ such that $P$ is supported on the register on which the realization of $D$ (that begins at step $\tau$) is applied, and $Q$ is supported on the rest.

If the Pauli $P$ is not a logical error with respect to $D$, then:
    \begin{equation*}
      \begin{split}
...
\begin{claim}
  Let $\tau$ be a step of $\tilde{C}[\mathcal{E}]$, which is either a part of a fault layer or a sweep-rule cycle (not a part of logical gate realization). Consider a Pauli-path up to step $(\tau,0)$, $P_{1},..,P_{\star}$. For a register $R$ denote by $P_{\star}|_{R}$ the restriction of $P_{\star}$ to the qubits in $R$. 
