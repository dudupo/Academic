...
\begin{claim} 
    ...
    Consider a time graph $(G,T)$, Let $K$ be a slice in $G$, and let $K$ be a slice of $K$ such that $K \neq \History{K}$. Denote by $x, y$ the vertices which achieve the maximal values of $L_{d+1}$ and $\max_{g}L_{g}$. Then for some integer $k$ there exist slices $K_{0} , ..,K_{k}$ and forks $F_{1},..,F_{k}$ such that:
    \begin{enumerate}
