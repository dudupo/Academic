...
Let $\ket{\psi}$ be a logical codeword of the $(d,d^{\prime})$-Toric code with side length $n$. Let $P \in \mathcal{P}^{(\cdot)}$. Assuming that during the application of ${ d \choose d^{\prime} } n^{d+1}$ ideal sweep-cycles over the $P\ket{\psi}$, the Toom's counter does not admit a slice of size greater than or equal to $n$, then at the end the error is erased, namely the result of the computation is $\ket{\psi}$.
\end{claim}
\begin{proof}
Since no new faults are introduced during the computation, we have that in each step the only option for a slice to grow is by merging. Observe that since slices consist of vertices connected by time, they cannot split. Specifically, a correction by the sweep rule that disconnects two error clusters that were connected by checks, and prior to correction belonged to the same slice, would still belong to the same slice after the correction since they are time connected (arrows connected).

Therefore, there can be at most ${ d \choose d^{\prime} } n^{d}-1$ merges. That is because the number of slices with support on a single register is bounded by the number of the register's qubits, and two slices can be merged together at most once. Hence there has to be a continuous sequence of $n$ cycles for which no merge occurs. Since it's given that the size of all the slices is lower than $n$, the sizes of all connected errors are lower than $n$, and a sweep-cycle decreases the size of a finite error by at least 1. So, we have that at the end of that sequence of $n$ cycles, all the slices shrink to size zero; namely, the connected errors are erased after those cycles.
\end{proof}
...
\inb{\textbf{Define the term infinite slice, which is a slice at size greater or equal to $n$. And then rewrite those both claims. }}
\begin{claim}
  \label{claim:replace}
  Consider a rapid application of the sweep-cycle for $l$ times. Denote by $A$ the event in which, after an additional ${ d \choose d^{\prime} } n^{d+1}$ \textbf{ideal} (non-noisy) iterations of the sweep-cycle, the error is not erased, and denote by $B$ the event that, during the application of an additional ${ d \choose d^{\prime} } n^{d+1}$ \textbf{noisy} iterations, the Toom's contour admits a slice with size greater than or equal to $n$. Let $f:(n,d,d^{\prime},l,p) \rightarrow \mathbb{R} $ be an upper bound on $\prb{B}$ that assumes only that the noise is distributed according to a local stochastic noise channel with noise rate $p$. Then, we have:
  \begin{equation*}
    \begin{split}
...
The proofs of the claims are given in the appendix. We continue with the proof of the lemma.
  \begin{proof}
    ...
    \Cref{claim:decodestopnoise} states that the $D$ successfully decodes assuming that no infinite slice occurs. And \Cref{claim:replace} states that it's sufficient to bound the probability that no infinite slice occurs by any upper bound that assumes only a local stochastic behavior on the probability that no infinite slice occurs in the running where the decoding procedures are subjected to noise\footnote{Observe that in this setting, the classical procedure that infers a logical measurement makes no sense}. Thus, it remains to prove that it happens with probability $q$  no infinite slice is admitted in the setting in which the decoding procedures are also subjected to noise. From now on, we consider that setting.  

    \inb{\textbf{Need to work on that.}} Let $\alpha = (\Delta d^{2})^{-1}$. \Cref{claim:intersyn} states that the probability of there being a time $t$ (or a step) that admits an infinite slice is bounded by the number of locations multiplied by the probability that step $\tau$ is the first to admit a finite connected error (chain) in the propagated error at size greater than $\alpha n$, maximized over $\tau$. Notice that if $\tau$ is the first step to admit an error at size $\alpha n$, then all the slices in analysis of Toom's contour at step $\tau$ have an explanation. 
