...
For the sake of proving correctness, we can assume that at the end we measure all registers, each in either the computational or the phase basis. Any proof that gives correctness without distinguishing in which basis we choose for the registers captures the referee test at the end of the running. Then, if there is no application in which the referee, $D$, fails - namely the corrections followed in the gate teleportation procedures are the right ones, then the logical gates that were applied simulate the gates in the original circuit. Thus, showing that $D$ does not fail, and that at the end of the running, the resulting state is 


To assure that $D$ succeeds in decoding the output states, it's enough to imagine that there are $n^{2d}$ additional ideal sweep-cycles at the end and require them to achieve a successful decoding.
