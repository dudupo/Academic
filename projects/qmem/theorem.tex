...
Consider an application of $D$ in $\tilde{C}$ and a set of faults $\mathcal{E}$. Let $\tau, \tau_{\mathcal{E}}$ be the step of the gate in application and its corresponding step in the subjected circuit $\tilde{C}[\mathcal{E}]$. Denote by $\tilde{C}_{\rightarrow \tau}, \tilde{C}[\mathcal{E}]_{\rightarrow \tau_{\mathcal{E}}}$ the circuits obtained by taking gates up to steps $\tau$ and $\tau_{\mathcal{E}}$. For coefficients $a_{1}, \ldots \in \mathbb{C}$ and Paulis $P_{1}, \ldots \in \mathcal{P}^{(\cdot)}$, let $\sum_{i}a_{i}P_{i}$ be the Pauli decomposition of the propagated error to step $\tau_{\mathcal{E}}$, namely:
    \begin{equation*}
      \begin{split}
        \tilde{C}[\mathcal{E}]_{\rightarrow \tau_{\mathcal{E}}} \colonequals  \bigl( \sum_{i} a_{i}P_{i} \bigr)  \tilde{C}_{\rightarrow \tau}
      \end{split}
    \end{equation*}
If none of the Paulis in the decomposition, $P_{i} \in P_{1},..,$ is a logical error with respect to $D$, then:
    \begin{equation*}
      \begin{split}
      D \tilde{C}[\mathcal{E}]_{\rightarrow \tau_{\mathcal{E}}}  = D   \bigl( \sum_{i} a_{i}P_{i} \bigr) \tilde{C}_{\rightarrow \tau} = D \tilde{C}_{\rightarrow \tau} 
      \end{split}
    \end{equation*}
