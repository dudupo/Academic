...
\begin{claim} 
    ...
    Consider a time graph $(G,T)$, Let $K$ be a slice in $G$, and let $K$ be a slice of $K$ such that $K \neq \History{K}$. Select vertices $x, y \in K$ that achieve the maximal values of $L_{d+1}$ and $\max_{g}L_{g}$. Then for some integer $k$ there exist slices $K_{0} , ..,K_{k}$ and forks $F_{1},..,F_{k}$ such that:
    \begin{enumerate}
...
\begin{definition}[Tooms Contour Graph of $C(\mathcal{E})$, (Standard form)]
    For a Toric encoded quantum circuit $C$, set of faults $\mathcal{E}$ and a reduced Pauli-path $P_{1},..$, let us denote the base graph of $C[\mathcal{E}]$ by $G_{o}$. We define the \textit{Tooms contour graph} $\mathcal{H} = ( V_{\mathcal{H}}, E_{\mathcal{H}})$ as follows: 
    \begin{enumerate}
      \item The vertices $V_{\mathcal{H}} = V_{\mathcal{H}, X} \cup V_{\mathcal{H}, Z}$ correspond to vertices of faces supporting the reduced Pauli-path. They consist of the triple $(v,t,\cdot)$ where $(v,t)$ is a vertex of the base graph, and the third is a symbolic coordinate that is either $\{X\}$ or $\{Z\}$ corresponding to the component of the Pauli that $v$ supports at time $t$.
        \begin{equation*}
          \begin{split}
            ...
            & \, \, \, \text{ and there exists a face } f \in \text{ supp } X(P_{t}) \text{ such } v \in f \big\} \\
             ...
             & \, \, \, \text{ and there exists a face } f \in \text{ supp } Z(P_{t}) \text{ such } v \in \phi(f) \big\} 
          \end{split}
        \end{equation*}
