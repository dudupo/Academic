...
\begin{definition}
  Denote the quantum registers by $R_{1},R_{2},.., R_{r}$. Let $K$ be a slice, let $K_{R_{1}},..,K_{R_{r}}$ be the restrictions of $K$ to the registers. So: 
  \begin{equation*}
    \begin{split}
      K =  \bigcup_{i=1}^{r} K_{R_{i}}
    \end{split}
  \end{equation*}
 For a $r$-length binary string $x \in \{0,1\}^{r}$, a $\psi_{x}$-configuration of $K$ is 
 \begin{equation*}
   \begin{split}
     \psi_{x}(K) \colonequals \bigcup_{x_{i}=0} K_{R_{i}} \bigcup_{x_{i}=1} \psi(K_{R_{i}})
   \end{split}
 \end{equation*}
 Where 
 \begin{equation*}
...
\begin{equation*}
  \begin{split}
    \Sizestar{A} \colonequals \max_{x \in \{0,1\}^{r}}\Size{ \psi_{x}(A) }
  \end{split}
\end{equation*}
\end{definition}
...
\begin{claim}
  Invariant of $\Sizestar{}$ under $\psi_{y}$. 
\end{claim}
\begin{proof}
\begin{equation*}
  \begin{split}
    \Sizestar{ \psi_{y}(A)} & \colonequals \max_{x \in \{0,1\}^{r}}\Size{ \psi_{x}(\psi_{y}(A)) } = \max_{x \in \{0,1\}^{r}}\Size{ \psi_{x + y}(A) }\\
  & = \max_{x \in \{0,1\}^{r}}\Size{ \psi_{x}(A) }
  \end{split}
\end{equation*}
\end{proof}
...
\end{claim}
\begin{proof}
  Imagine that any $\psi$-permutation is grouped together into a single step. Assume that at step $\tau$ there is a subslice at size-star greater than or equal to $n$. If at $\tau$ we permute by $\psi$, then by the invariant of $\Sizestar{}$ under $\psi$, we also have that $\tau -1$ has a subslice at size-star greater than or equal to $n$. Thus, we can assume that the gate applied at turn $\tau$ is not a part of $\psi$-permutation. Now assume that $\psi_{y}$ is the $\psi$-configuration that achieves the maximum.
  ...
  \begin{equation*}
    \begin{split}
      \Size{  \psi_{y}(J_{1}) \cup \psi_{y}(J_{2}) \cup ..   } \le \Size{ \psi_{y}(J_{1}) } + \Size { \psi_{y}(J_{2}) \cup .. }
    \end{split}
  \end{equation*}
  Thus:
\begin{equation*}
...
\inb{ \textbf{ The trick is to treat forks differently (like either apply $\psi$ over the two ends of a fork, or not apply at all).} }
