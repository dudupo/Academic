...
Consider an application of $D$ in $\tilde{C}$ and a set of faults $\mathcal{E}$. Let $\tau, \tau_{\mathcal{E}}$ be the step of the gate in application and its corresponding step in the subjected circuit $\tilde{C}[\mathcal{E}]$. Denote by $\tilde{C}_{\rightarrow \tau}, \tilde{C}[\mathcal{E}]_{\rightarrow \tau_{\mathcal{E}}}$ the circuits obtained by taking gates up to steps $\tau$ and $\tau_{\mathcal{E}}$. For coefficients $a_{1}, \ldots \in \mathbb{C}$ and Paulis $P_{1}, P_{2}, \ldots, Q_{1}, Q_{2}, \ldots \in \mathcal{P}^{(\cdot)}$, let $\sum_{i}a_{i}P_{i} \otimes Q_{i}$ be the Pauli decomposition of the propagated error to step $\tau_{\mathcal{E}}$, such that the $P$s support the qubits of the measured register while the $Q$s support the rest, namely:
    \begin{equation*}
      \begin{split}
