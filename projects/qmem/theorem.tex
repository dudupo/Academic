...
\begin{claim}
  ...
  \end{equation*}
\end{claim}
The diameter function fits well within the definition of space-time graphs. 

\newcommand{\Trans}[1]{ \text{Trans}(#1)}
\newcommand{\CM}[1]{ \text{C.M}(#1)}
However it doesn't translate invariant, therefore, we need the following definition of translated slice: 
\begin{definition}
  The center of mass of a subset $A$ is: 
  \begin{equation*}
    \begin{split}
      \CM{A} =  \frac{1}{|A|}\sum_{g \in S}\sum_{v \in A}L_{g}(v)\hat{g}
    \end{split}
  \end{equation*}
  Let $K$ be a slice, and let $u \in \text{pole}_{d+1}(K)$. The \textit{translation} of it with respect to $u$ is defined to be:
  \begin{equation*}
    \begin{split}
      \Trans{K} = \big\{  u^{-1} v : v \in K  \big\} 
    \end{split}
  \end{equation*}
\end{definition}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{split}
      d L_{g}(z) + L_{d+1}(x) & = d L_{g}(z) - d L_{g}(y) + dL_{g}(y) + L_{d+1}(x) \\
      & \le d L_{g}(z - w) - d L_{g}(y - w ) + dL_{g^{\prime}}(\nu ) + L_{d+1}(x) \\
      & \le d L_{g}(z - w) - d L_{g}(y - w ) + dL_{g^{\prime}}(\nu ) + L_{d+1}(x) \\
    \end{split}
  \end{equation*}
\end{proof}
...
The following is an adaptation of \Cref{claim:prevslices} to relate between the diameter of a slice and its predecessors.
\begin{claim} 
