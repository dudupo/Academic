

%\newcommand*{\ACM}{}%

\ifdefined\ACM

%\documentclass[sigplan,screen]{acmart}
\documentclass[manuscript,screen,review]{acmart}

\else
 \documentclass[12pt]{extarticle}
%\usepackage{libertine}
\usepackage{cuted}%\usepackage{widetext}
\input{./usepackage}
\usepackage{cancel}
\usepackage{tikz-3dplot}
\usepackage{subcaption}
\addbibresource{./sample.bib}

\fi
\usepackage{physics} 

\begin{document}
\input{newcommands}
\title{ Quantum Fault Tolerance Meets Toom's Contours }
\author{David Ponarovsky}
\maketitle

% Constant depth fault tolerance construction.   
\newcommand*{\xb}{\mathbf{x}}

\ifdefined\SWEEPHEA
\else 
  \newcommand*{\SWEEPHEA}{}
  %\newcommand{\History}[1]{\mathbf{History}(#1)}
\newcommand{\History}[1]{H_{#1}}
  %\newcommand{\Time}[1]{\mathbf{Time}(#1)}
  \newcommand{\Time}[1]{T(#1)}
  \newcommand{\Excuse}[1]{\mathbf{Excuse}(#1)}
  \newcommand{\Excusei}[1]{\mathbf{Excuse}_{i}(#1)}
  \newcommand{\Size}[1]{\mathbf{Size}(#1)}
  \newcommand{\Span}[1]{\mathbf{Span}(#1)}
  \newcommand{\pre}[1]{\text{pre}(#1)}
  \newcommand{\prei}[1]{\text{pre}_{i}(#1)}
  \newcommand{\rotf}[2]{\theta_{#1 ,#2}}
  \newcommand*{\rotfs}{\rotf{v}{S^{\prime}}}
  \newcommand{\rotfv}[2]{\mathbf{\rotf{#1}{#2} }}
  \newcommand*{\rotfvs}{\mathbf{\rotfs{} }}
\fi

\inb{\textbf{TODO:}}
\begin{enumerate}
  \item Move from assignments to chains.   
  \item Enter the computational model to the Notations (and preliminaries). 
  \item Enter that we care only about pauli errors somewhere. 
  \item Rewrite the all outline. 
  \item Advance the Sweeprule definition prior to the moving into the infinite grid.
  \item Define what is mean a finite chain. 
  \item Logical operators - change the decoding procedure. 
\end{enumerate}

\input{abs.tex}
\tableofcontents
\input{intro.tex}

\input{math_notations.tex}

\ifdefined\FULL

\input{shrink_v1.tex}
\fi


\input{time_unfolding.tex}
\input{deftoric.tex}
\input{logical_gates.tex}
\input{to_infinte.tex}
%\input{error_clusters.tex}
\input{defsweeprule.tex}
\input{reducederr.tex}
\input{shrink_v2.tex}

\input{intermidiate.tex}
\ifdefined\FULL

\input{shrink_v3.tex}



\input{investigation.tex}
\fi

%\newpage

\input{Toric_encoded_circ.tex}

\input{space_time_graphs.tex}
\input{minimal_explanation_form.tex}
%\input{plane_to_toric.tex}
\input{theorem.tex}
% \input{tanner_time_equ.tex}
%\input{decoder_phase.tex}
\printbibliography
\input{appendix.tex}
\input{Garbage.tex} 
\end{document}
