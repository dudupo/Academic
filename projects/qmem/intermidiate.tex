...
Assume by induction that this is true for any $t^{\prime} < t$. Then there are $Q_{1}, \ldots, Q_{l}$ such that $P_{t-1}C[\mathcal{E}]|_{t} = C[\mathcal{E}]|_{t} Q_{l}$, and on the other hand, $Q_{i}\tilde{C}[\mathcal{E}]|_{\tau^{\prime}+i} = \tilde{C}[\mathcal{E}]|_{\tau^{\prime}+i} Q_{i+1}$. 


Then we use \Cref{claim:prevslices}, yet only a finite number of qubits is involved.. 
\end{proof}
