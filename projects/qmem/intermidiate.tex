...
An essential ingredient of our proof is an 'intermediate argument', stating that prior to having a large error that closes a non-trivial loop on the Toric, there must be a step in which there is an error that on one hand doesn't close a non-trivial loop, yet it has a size that is comparable to the Toric side. Before the first step of that kind, we can assume that no error closes a non-trivial loop, or in other words, the errors are finite, and the sweep rule is, in some sense, effective (\Cref{claim:polesmove}). That motivates us to describe the evolution of a fault to a given step and define the notion of a step that admits infinite error.
