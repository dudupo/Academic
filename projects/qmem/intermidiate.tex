...
\begin{definition}[Step $\tau$ Admits Inifinte Error/Slice ]
  \label{def:localprop}
  Let $C[\mathcal{E}]$ be a Toric encoded circuit, and let $J=\tau_1, \tau_2, \ldots$ be an order agreeing with $\pi_{C[\mathcal{E}]}$. We say that a step $\tau \in J$ admits infinite error if there exists a reduced Pauli-path up to step $\tau$, $P_{1}, P_{2}, \ldots, P_{t^{\star}}$, that induces an analysis Toom's contour such that there is at least one Pauli in the path whose view in the $\infty$-extension is infinite (\Cref{def:finiteerr}). Similarly, we say that a step $\tau$ admits an infinite slice if there is a Pauli path such that $\mathcal{H}_{\tau}$ has an infinite slice (\Cref{def:infislice}). Moreover, we say that $\tau$ is the first step to admit an infinite error/slice if $\tau$ is the minimal step for which there is a Pauli path such that $\mathcal{H}_{\tau}$ admits an infinite error/slice.
\end{definition}
