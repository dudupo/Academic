...
Let $h = \Time{K} - \min_{u \in \History{K}} \Time{u}$. For slices in a standard Toom's contour, we prove by induction on $h$.
  ...
  \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. Combining this with the fact that no infinite slice is admitted, by \Cref{claim:prevslices}, we know that for some $k$ there exist slices $K_{0}, \ldots, K_{k}$ and forks $F_{1}, \ldots, F_{k}$ that satisfy \Cref{claim:prevslices} with respect to the pole selection. In particular, for $r < 3$ it holds that:
        \begin{equation*}
          \begin{split}
