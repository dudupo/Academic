...
For quantum circuits $C$ and $\tilde{C}$, a noise channel $\mathcal{E}$, and $p \in (0,1)$, we say that $\tilde{C}$ simulates $C$ with probability $1 - p$ if
  ...
  \begin{theorem}[Informal]
    Let $C$ be a quantum circuit, with depth $d$ and width $w$, then there exist a toric encoded circuit $\tilde{C}$ that simulate $C$ such the probability to have a logical error in $\tilde{C}$ is bounded by $wd \cdot p^{l}$. 
  \end{theorem} 
