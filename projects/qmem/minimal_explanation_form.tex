...
Given sets $W$ of points, $A$ of arrows, and $F$ of forks, we define $U$ as the set of vertices of the graph induced by the edges of $A \cup F$. The sets $W$, $A$, and $F$ form an explanation of a slice $K$, with poles selection $v_{1}, \ldots, v_{d+1}$, and order $\text{order}(K) = r$, if and only if:
    ...
    \end{enumerate}
Furthermore, for a time $t$ and a subslice $K$ in the analysis of Toom's contour at time $t$, with pole selection $v_{1},..,v_{d+1}$, we extend the definition of explanation as above, but require the following weaker inequality instead: For $m = |W|$ and $s = \Size{K}$, we have $|A| \le \alpha (m + 1) - \beta s$ and $|F| \le m - 1$.
  \end{definition}


  \begin{claim}
    \label{claim:edgesin}
Let $P_{1}, \ldots, P_{\mu+1}$ be the Pauli-path that induces Toom's contour. Let $t \le \mu+1$ such that $P_{1}, \ldots, P_{t}$ do not admit an infinite error. Every slice $K$, with time coordinate $\Time{K} \le t$, has an explanation. Moreover, if a time $t$ is the first step to admit an infinite error, then any subslice in the analysis of Toom's contour at time $t$ has an explanation.
  \end{claim}
...
Let $h = \Time{K} - \min_{u \in \History{K}} \Time{u}$. For slices in a standard Toom's contour, we prove by induction on $h$.
    \begin{enumerate}
      ...
      \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. \inb{\textbf{ If $K$ has an infinite predecessors we can take a subslice of it.}}  Combining this with the fact that no infinite error is admitted, by \Cref{claim:prevslices}, we know that for some $k$ there exist slices $K_{0}, \ldots, K_{k}$ and forks $F_{1}, \ldots, F_{k}$ that satisfy \Cref{claim:prevslices} with respect to the pole selection. In particular, for $r \neq 0$ it holds that:
        \begin{equation*}
          \begin{split}
...
The first three inequalities hold by the definitions of the $\mathcal{X}_{r}$, the fourth is satisfied since $\frac{\beta}{\alpha}\xi = 5 \frac{d+1}{\alpha}$, the last is obtained due to \Cref{claim:maxfork}, that states that the maximal size of a fork is at most $2d$, and that $\frac{\alpha}{\beta}\mathcal{X}_{r}$ is greater than $4d$.
    ...
    \end{enumerate}
It's left to prove for subslices in the top layer of the analysis of Toom's contours. Notice that the 'forming rules' of the analysis of Toom's contour are different from the standard Toom's contour only at the last layer. Therefore, the induction proof captures all the slices not in the last layer. For any subslice at the last layer, we repeat the induction step while using \Cref{claim:prevslicesB} instead of \Cref{claim:prevslices} to relate the subslice to its predecessor slices and use \Cref{claim:shrinkan} to bound the expansion in size.  So the bounding over $\mathbf{A}$'s size becomes:
    \begin{equation*}
      \begin{split}
