 \subsection{Explantation of a Spanned Slice}
 \label{subsection:explantion}
We are about to prove the main lemma stating that the number of faults causing an occurrence of slice is linear in both the slice's size and the minimal number of edges required to span a Tooms contour that induces it. 


First, we define the explanation tree of a slice to be the subtree of its history that connects its poles. The relation between the leaves of the subtree and its arrows, forks, and the size of its origin slice is embedded as a requirement in the definition. Then, we prove that any slice has an explanation tree.
  \begin{definition}
...
Furthermore, for a step $\tau$ and a spanned slice $\hat{K}$ in the analysis Toom's contour at step $\tau$, we extend the definition of explanation as above, but require the following weaker inequality instead: For $m = |W|$ and $s = \Size{K}$, we have $|A| \le \alpha (m + 1) - \beta s$ and $|F| < m - 1$.
  ...
  \begin{claim}
    \label{claim:edgesin}
Let $P_{1}, \ldots, P_{\mu+1}$ be the reduced Pauli-path that induces Toom's contour. And let $t \le \mu+1$ such that $P_{1}, \ldots, P_{t}$ do not admit an infinite error. Every spanned slice $K = \expval*{K, v_{1}, \ldots, v_{d+1}}$, with time coordinate $\Time{K} \le t$, has an explanation. Moreover, if a step $\tau$ is the first step to admit an infinite error, any slice in the analysis of Toom's contour has an explanation.
  \end{claim}
...
The first three inequalities hold by the definitions of the $\mathcal{X}_{r}$, the fourth is satisfied since $\frac{\beta}{\alpha}\xi = 5 \frac{d+1}{\alpha}$, the last is obtained due to \Cref{claim:maxfork}, that states that the maximal size of a fork equals $2d^{\prime}$, and that $\frac{\alpha}{\beta}\mathcal{X}_{r}$ is greater than $4d$.
    ...
    \end{enumerate}
    \inb{\textbf{Return here after deciding about the $\phi$-transpose.}} It's left to prove for spanned slices in the analysis Toom's contours. Notice, that $h$ has to be greater than zero, We prove the inequality by repeating on the induction bases, where all the spanned slices that are back in the history of $K$ are spanned slices in the standard Toom's contour. So the only different is in the bounding of $\mathbf{A}$'s size. We use \Cref{claim:shrinkan} (with the assumption that all the errors are finite) and get: 
    \begin{equation*}
      \begin{split}
...
Let $\hat{K}$ be a spanned slice with size $s$, and explanation with $|A|$ arrows, and $|F|$ forks. Then it is induced by at least the following number of faults:
    \begin{equation*}
      \begin{split}
        \frac{1}{2^{\max\{d^{\prime}, d - d^{\prime}\}}}  \frac{1}{2} \left( |F|+1 +  \alpha^{-1} ( |A| - 1) + \beta s  \right)
      \end{split}
    \end{equation*}
