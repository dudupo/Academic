...
Furthermore, for a step $\tau$ and a spanned slice $\hat{K}$ in the analysis Toom's contour at step $\tau$, we extend the definition of explanation as above, but require the following weaker inequality instead: For $m = |W|$ and $s = \Size{K}$, we have $|A| \le \alpha (m + 1) - \beta s$ and $|F| < m - 1$.
  ...
  \begin{claim}
    \label{claim:edgesin}
    Let $P_{1},..,P_{\mu+1}$ the reduced Pauli-path that induce the Toom's contour. And let $t \le \mu+1$ such $P_{1},..,P_{t}$ do not addmit an infinte error. Every spanned slice $K = \expval*{K,v_{1},..,v_{d+1}}$, with time cooridnate $\Time{K} \le t$ has an explanation.
  \end{claim}
