...
The distinction between slices resulting from different layers allows us to adjust \Cref{claim:prevslices} conditioning on the slice type. 
  ...
  \end{corollary}
  \begin{proof}
Since the space-time graph is a Toom's contour, we have that slices are vertices of a union of reduced connected chains that are connected by time. For the sweep-cycle part, we use \Cref{claim:polesmove}, which states that after each application of the sweep-cycle over a finite size assignment, the maximal $L_{d+1}$ value of the resulting reduced error decreases by 1, while the values of each $L_{g}$ do not increase. If $\hat{K}$ is the vertex domain of more than one (disjoint) connected chain, then it's sufficient to look over the one with the maximal $L_{d+1}$ value, and derive for it that the predecessor chain prior to the correction has to have a maximal $L_{d+1}$ value higher by 1.
