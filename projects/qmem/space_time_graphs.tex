...
\end{remark}
The history of a vertex consists of all the vertices that are connected to it by arrows. Here we allow arrows to pass in both directions, so it might be possible that the history of vertex $u$ contains vertices with the same time coordinate as $u$. 
  \begin{definition}[History, slice.]
For a time graph $(G,T)$ and a vertex $u \in V$, let the history of $u$, denoted by $H_{u}$, be the restriction of $G$ to vertices which both precede $u$ in time and connect to $u$ via arrows. A slice is an equivalence class of the relation $u \equiv v$ if and only if $\History{u} = \History{v}$. \inb{here}.


    Notice that the time function has to be constant on a slice, to see this consider vertices  $x,y$ with $T(x) > T(y)$, thus $x \notin G_{y}$ and therefore $x \in \History{x}/\History{y}$ So they are belong to different classes. Thus, we can extend the time function and the history to slices and write for a slice $K$, $\Time{K}$ and $\History{K}$.
  \end{definition}
