...
If $\hat{K}$ is a result of faults, then it cannot contain a vertex that supports a fault, because no arrow connection is made for a fault gate. Thus, if it contains a vertex that supports a fault, it will be the only vertex and $K = H_{K}$. So, we can imagine that all the vertices in $\hat{K}$ support the identity operator and handle that case as a result of a logical operator. Where $\hat{K}$ is a result of a  logical operator, we use the fact that since the operators are transversal, we have that the support of $\hat{K}$ is contained in the support of $\cup_{0}^{k} \hat{K}_{i}$, and therefore for any $i \in [d+1]$:
    \begin{equation*}
      \begin{split}
