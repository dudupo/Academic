...
Consider a spanned slice $\hat{K}$, at finite size, such that $K \neq H_{K}$. Since $K \neq H_{K}$, it follows that $\hat{K}$ has a predecessor decomposition, denoted by $\hat{K}_{0}, \ldots, \hat{K}_{k}, F_{1}, \ldots, F_{k}$. The inequality in \Cref{claim:prevslices} becomes:
  ...
  \end{corollary}
  \begin{proof}
For the sweep-cycle part, we use \Cref{claim:polesmove}, which states that after each application of the sweep-cycle over a finite size assignment, the maximal $L_{d+1}$ value of the resulting boundary decreases by $1$, while the values of each $L_{g}$ do not increase.
