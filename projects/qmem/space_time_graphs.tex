  ...
  Here, we present the results and proofs first given by Berman and Simon \cite{Berman} and later by Gacs \cite{gacs2021newversiontoomsproof}. Although the proofs we give are almost identical, we chose to present them rather than referring to the sources. The reason for this is that in the language appearing in \cite{Berman} and \cite{gacs2021newversiontoomsproof}, vertices are bits\footnote{It is also true that the gates are different, yet that difference alone does not constitute a conceptual change.}, so conceptually their Toom's contours track different creatures. Thus, jumping into their text while keeping our image in mind might cause confusion, and we want to avoid that.
