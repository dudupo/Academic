...
\inb{\textbf{ Change the structure to be yield form Pauli-path that does not admit a infinite slice.  }}
  ...
  \end{corollary}
  \begin{proof}
      For the sweep-cycle part, we use \Cref{claim:polesmove}, which states that after each application of the sweep-cycle over a finite-size assignment, the maximal $L_{d+1}$ value of the resulting reduced error decreases by at least one, while the values of each $L_{g}$ do not increase. Since no infinite slice is admitted, we have that the chain supported on the vertices $K_{0}, \ldots, K_{k}$ is finite, and therefore its size decreases.
    %\Cref{claim:prevslices} if $L_{d+1}(\predd{v_{d+1}}) = L_{d+1}(v_{d+1})$ then 
    %Since it's given that the Pauli-path doesn't admit an infinite error we have that $P$ is finite. Thereby, any connected chain in $a$    If $\hat{K}$ is the vertex domain of more than one (disjoint) connected chain, then it's sufficient to look over the one with the maximal $L_{d+1}$ value, and derive for it that the predecessor chain prior to the correction has to have a maximal $L_{d+1}$ value higher by at least one.

      If $\hat{K}$ is a result of faults, then it can not contain a vertex support a fault, that is because no arrows-connection is made for a fault gate. Thus, if it contains a vertex that is support fault, then that would be the 


      \footnote{Notice that $\hat{K}$ does not consist of faults since otherwise $K=H_{K}$, so the meaning here is that $\hat{K}$ contains vertices which weren't flipped in the fault layer.} or a logical operator, we use the fact that since the operators are transversal, we have that the support of $\hat{K}$ is contained in the support of $\cup_{0}^{k} \hat{K}_{i}$, and therefore for any $i \in [d+1]$:
    \begin{equation*}
      \begin{split}
