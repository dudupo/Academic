...
Let $(G,T)$ be a space-time, whose vertices are associated with points in $\mathbb{R}^{d}$. Let $A$ be either a slice, vertex, arrow, or a fork in $(G,T)$. We define the $i$th pole of $A$ to be:
    \begin{equation*}
      \begin{split}
        \poli{A} \colonequals \arg \max_{v\in A} L_{i}(v)
      \end{split}
    \end{equation*}
If there is more than one vertex that achieves the maximal value, then pick one arbitrarily. Similarly, we define the $i$th predecessor of a $A$, denoted by $\prei{A}$, to be the predecessor which achieves the maximal value of $L_{i}$, namely:
    \begin{equation*}
      \begin{split}
        \prei{A} \colonequals \arg \max_{v\in \pre{A}} L_{i}(v)
      \end{split}
    \end{equation*}
    Lastly, the size of $A$ is defined to be:
    \begin{equation*}
      \begin{split}
        \Size{A} \colonequals  \sum_{i}^{d+1}  \max_{v\in A} L_{i}(v) = \sum_{i}^{d+1}  L_{i}( \poli(A) )
      \end{split}
    \end{equation*}
  \end{definition}
 ...
 \paragraph{}  The following claim relates the size of a given slice to the sizes of its predecessor slices.
  \begin{claim}
    ...
    Consider a time graph $(G,T)$ whose vertices are associated with points in $\mathbb{R}^{d}$, Let $K$ be a slice in $G$, and let $v_{1}, .. v_{d+1}$ be it's poles, such that $K \neq \History{K}$. Then for some integer $k$ there exist $k+1$ slices $K_{0},K_{1} ..,K_{k}$ and $k$ forks $F_{1},..,F_{k}$ such that:
    \begin{enumerate}
      \item For $i \in [d+1]$ $ \prei{x_{i}} $ belongs to the poles of $K_{0},..,K_{k}$.  
  ...
  \begin{proof}
    ...
    For $i=1,..,d+1$ we set $\prei{v_{i}}$ to be the $i$th pole of $R_{i}$, This assures (1). The set of remaining poles for $K_0, ..,K_{k}$ will be equal to $\cup^{k}_{i=k}F_{i}$. This will assure (ii), We will also select poles for $F_{1}, ..,F_{k}$ in such way that:  
    \begin{equation*}
      \begin{split}
        \sum^{k}_{i=0}\Size{K_{i}} + \sum^{k}_{i=1}\Size{F_{i}} \ge \sum_{i=1}^{d+1}L_{i}\left( \prei{v_{i}} \right) 
      \end{split}
    \end{equation*}
    This will assure (3). 
...
We will choose the $z_{j}^{i}$ by the following process. Pick a $z_{j}^{i}$ that has not been set yet. If $J_{j} = R_{i}$, then set $z_{j}^{i} \leftarrow \prei{v_{i}}$ - we call this a type-1 assignment. Otherwise, consider the path from $J_{j}$ to $R_{i}$ (by connectivity, it has to be such a one, and by minimality, there is exactly one). Let $J_{t}$ be the successor on the path, then pick $z_{j}^{i} \in J_{j} \cap J_{t}$ - similarly, we call this assignment a type-2 assignment. Since any type-2 assignment is associated with a non-trivial intersection, and any non-trivial intersection induces a type-2 assignment for all $i$'s (since for all $i$'s, one of the $J_{j}$ on its support is a successor of the rest on their path to the $i$th pole), we have that:
  ...
  \begin{claim} 
    ...
    Let $K$ be a subslice of slice $K^{\star}$ such ,$K^{\star} \neq \History{K^{\star}}$. Then for some integer $k$ there exist subslices $K_{0}, ..,K_{k}$ and forks $F_{1},..,F_{k}$ such that:
      \begin{equation*}
        \begin{split}
...
While in the construction presented in \cite{Berman}, all the layers were the same (consisting of the same gates up to wiring), here different layers might differ both in realization and role. Thus, we would like to distinguish between them. We will define the \textit{predecessor decomposition} of a slice to be the collection of slices and forks which satisfy the condition in \Cref{claim:prevslices}. The order of a slice is a number in $\{0,1,2,3\}$ that matches the time modulo four of the layer that results in the slice. Therefore, it encodes the layer type.
  ...
  \begin{definition}
Let $K$ be a slice, let $K_{0}, \ldots, K_{k}$ be slices in $H_{K}$, and let $F_{1}, \ldots, F_{k}$ be forks, such that $K_{0}, \ldots, K_{k}, F_{1}, \ldots, F_{k}$ satisfy the conditions in \Cref{claim:prevslices}. We call $K_{0}, \ldots, K_{k}, F_{1}, \ldots, F_{k}$ the \textit{predecessor decomposition} of $K$.
  
     Furthermore, we say that $K$ is a result of a \textit{sweep-cycle}, \textit{faults}, or a \textit{logical operation} if the layer at time $T(K) - 1$ consists of sweep-cycle gates, fault gates (whose source is $\mathcal{E}$), or either measurement or logical Pauli. The \textit{order} of the slice is its time modulo $4$, namely $\text{order}(K) = T(K) \mod 4$. Observe that slices that are results of faults have an order that is either $0$ or $2$, whereas slices that are results of a logical operation or a sweep-cycle are of orders $1$ and $3$, respectively.
  \end{definition}
The distinction between slices resulting from different layers allows us to adjust \Cref{claim:prevslices} by conditioning on the slice type.
  ...
  \begin{corollary}
    ...
    For a Toom's contour, suppose that the reduced Pauli-path, that induces it, does not admit an infinite slice. Consider a slice $K$ in it, such that $K \neq H_{K}$. Since $K \neq H_{K}$, it follows that $K$ has a predecessor decomposition, denoted by $K_{0}, \ldots, K_{k}, F_{1}, \ldots, F_{k}$. The inequality in \Cref{claim:prevslices} becomes:
    \begin{enumerate}
...
First, since the arrows and forks connection rules do not allow vertices of $V_{\mathcal{H},X}$ to connect to vertices of $V_{\mathcal{H},Z}$, we can assume that $K \subset V_{\mathcal{H},X}$. Let us split upon what $K$ results from:
    \begin{enumerate}
        ...
        \item If $K$ is a result of faults, then it cannot contain a vertex that supports an $X$-fault. That is because no arrow connection is made in that case, and hence, if it were to contain a vertex that supports a $X$-fault, that vertex would be the only one in $K$, namely $K = H_{K}$. So, we can imagine that all the vertices in $K$ support the identity operator (or $Z$-faults, which are treated as identity for vertices in $V_{\mathcal{H},X}$) and handle that case as a result of a logical operator.
          \item Where $K$ is the result of a logical operator, we use that the operators are transversal, so we have that the support of $K$ is contained in the support of $\cup_{0}^{k} K_{i}$, and therefore for any $i \in [d+1]$:
        \begin{equation*}
          \begin{split}
  ...
  \begin{claim}
    ...
    For a step $\tau$, let $\mathcal{H}_{\tau}$ be analysis Toom's contour at step $\tau$, and let $t^{\star}$ be the maximal time at that contour. Assume that that the reduced Pauli-path that induces $\mathcal{H}_{\tau}$ does not admit an infinite error then for a slice $K$, at time $T(K) = t^{\star}$, where $K \neq H_{K}$, the inequality in \Cref{claim:prevslices} becomes:
        \begin{equation*}
          \begin{split}
            \sum_{i=0}^{k}\Size{K_{i}} +  \sum^{k}_{i=1}\Size{F_{i}} \ge \Size{K} - d - d^{\prime} - 1 
          \end{split}
        \end{equation*}
  \end{claim}
  \begin{proof}
    We allow vertices at the final time layer to be connected by an arrow if their counterpart on the skeleton shares a check. Therefore, for any $i \in [d]$, the value of $L_{i}$ for $v_{i}$ might be greater by at most 1 than the value of $\prei{v_{i}}$, and the $L_{d+1}$ for $v_{d+1}$ might be greater by at most $d^{\prime} + 1$ than the value of the $d+1$ pole of $K$. Hence, overall, we get that:
    \begin{equation*}
      \begin{split}
