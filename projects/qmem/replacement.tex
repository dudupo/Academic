...
Let $\ket{\psi}$ be a logical codeword of the $(d,d^{\prime})$-Toric code with side length $n$. Let $P \in \mathcal{P}^{(\cdot)}$. Assuming that during the application of $n^{d+1}$ ideal sweep-cycles over $P\ket{\psi}$, the Toom's counter does not admit an infinite slice, then at the end the error is erased, namely, the result of the computation is $\ket{\psi}$.
\end{claim}
\begin{proof}
