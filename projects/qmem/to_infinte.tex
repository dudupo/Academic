...
\begin{definition}
  \label{def:extension}
  Let $n$ be an integer, and let $C = \{u_{i}\}_{i=1}$ be a quantum circuit with quantum registers $\mathcal{R} = R_{1}, \ldots, R_{k}$ such that each register $R_{i}$ encodes $n$-length Toric code. In addition let us denote the decoding gates of $C$ by $\mathcal{U}$. We define the \textit{$\infty$-extension of $C$}, denoted by $C^{\infty}$, as follows:
  \begin{enumerate}
    ...
    \item Each gate $u_{i} \in C / \mathcal{U}$ is mapped it the infinite collection obtained by a  via the standard translation by a vertex:     
      \begin{equation*}
        \begin{split}
