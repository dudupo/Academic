...
On a location encoded in the register coordinate system, we can act via shifting, which translates the given location to a shifted location on the suitable infinite Toric.
    \begin{definition}[Shifting in Registers System]
      Consider the register coordinate system induced by a quantum circuit of width $w$, with $R$ registers, each of length at most $n$. For a tuple of integers  $r = (r_{1},..,r_{d}) \in \mathbb{Z}^{d}$, we define the $r$-shifting function $\text{Sh}_{r} \colon (\mathbb{Z}_{R} \times \mathbb{Z}_{n})^{\times \Delta} \rightarrow (\mathbb{Z}_{R} \times \mathbb{Z})^{\times \Delta}$ to act on space-location coordinates as follows:
  \begin{equation*}
    \begin{split}
      & \text{Sh}_{r}( l_{1}, .., l_{k} ) = ( \text{Sh}^{1}_{r}(l_{1}), .., \text{Sh}^{d}_{r}(l_{k}) ) \\ 
      & \text{ Where } \text{Sh}^{i}_{r}(l_{i}) = 
      \begin{cases}
        l_{i} + r_{i} & \text{ if } l_{i} \text{ is an inner coordinate } \\ 
        l_{i}& \text{ else }
      \end{cases}
    \end{split}
  \end{equation*}
...
Now we are ready to give an explicit description of the mapping. Given a quantum circuit $C$, we name its presentation over the infinite Torices the \textit{$\infty$-extension of $C$}.
\begin{definition}
  \label{def:extension}
  Let $n$ be an integer, and let $C = \{u_{i}\}_{i=1}$ be a quantum circuit with registers $\mathcal{R} = R_{1}, \ldots, R_{k}$ such that each register $R_{i}$ is encoded in the Toric code induced by a Toric complex with a side length $n$. In addition let us denote the decoding gates of $C$ by $\mathcal{U}$. We define the \textit{$\infty$-extension of $C$}, denoted by $C^{\infty}$, as follows:
  \begin{enumerate}
    ...
    \item Each gate $u_{i} \in C / \mathcal{U}$ is mapped to:     
      \begin{equation*}
        ...
        \end{split}
      \end{equation*}
    \item Each gate $ u \in \mathcal{U} $ is mapped to the collection of gates $\{ v_{j} \}_{j \in \mathbb{Z}^{d}}$ such that each of them is a translation of $u$.  has the same time coordinate as $ u $, is in the register that corresponds to the image of $ u $'s register, and is rooted at $\text{root}(v) + jn$. 
  \end{enumerate}
...
\label{claim:pauli_back}
  Let $u=(g,l)$ be a decoding gadget, and let $v_{0},v_{\pm 1}$ be its translates in the $\infty$-extension rooted at $\mathrm{root}(u)$ and $\mathrm{root}(u)\pm n$. Then
  \begin{equation*}
    \begin{split}
      v_{0}v_{\pm 1}|_{[w]} = u.
    \end{split}
