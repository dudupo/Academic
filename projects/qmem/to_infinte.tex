...
\end{claim}
\begin{proof}
  Since $u$ is a decoding gadget, then the gates $v_{0}$ and $v_{\pm i}$ are Pauli products, therefore we can decompose $v_{-i}$ to $ v_{-i} = \prod_{j}^{\Delta} v_{-i,j}$. Let $a$ be the vertex which is the root of $u$ and let  $(I_{1}, J_{1}), (I_{2}, J_{2}), \ldots, (I_{l}, J_{l})$ be relative paths that decode the faces on which $u$ act, In particular $v_{0,j}$ acts on the face $\rotf{(\prod_{g \in I_{j}} g) a}{J_{j}}$. Now, if $v_{-i,j}$ is in $[w]$ then $v_{i, 0} \notin [w]$, So in general we get all the terms in $u$.   
\end{proof}
