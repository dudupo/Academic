...
\begin{definition}
  \label{def:extension}
  Let $n$ be an integer, and let $C = \{u_{i}\}_{i=1}$ be a quantum circuit with quantum registers $\mathcal{R} = R_{1}, \ldots, R_{k}$ such that each register $R_{i}$ encodes a $(d,d^{\prime})=$-Toric code with side at length $n$. We define the \textit{$\infty$-extension of $C$}, denoted by $C^{\infty}$, as follows:
  \begin{enumerate}
...
\end{claim}
\begin{proof}
  Since $u$ is a decoding gadget, then the gates $v_{0}$ and $v_{\pm i}$ are Pauli products, therefore we can decompose $v_{-i}$ to $ v_{-i} = \prod_{j}^{\Delta} v_{-i,j}$. Now, if $v_{-i,j}$ is in $[w]$ then $v_{i, 0} \notin [w]$, So in general we get all the terms in $u$.   
\end{proof}
