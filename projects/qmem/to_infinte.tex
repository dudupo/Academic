...
\begin{claim}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  is a function that respects inclusion (\Cref{def:respinc}). 
\end{claim}
\begin{proof}
...
\item Consider two chains $x,y$ that has an intersection. Therefore their vertex domains intersect. For any $z$ that it's addition to $x+y$ decreasing the size it must to have that $z$ intersects $x+y$. Thus, for a fixed $z \in \mathcal{C}_{Z}^{\perp}$ that decreasing the size of $x+y$, we have: 
  \begin{equation*}
    \begin{split}
      \Size{x + y + z} \le \Size{x + z} + \Size{ y + z} \le 
    \end{split}
  \end{equation*}
