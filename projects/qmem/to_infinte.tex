...
We will need another method to translate a gate, 'rooted translation', here we assume that the locations of the gate are given in a relative to root fashion, means that there is root vertex, sand all the locations are given by paths from the root. 
  ...
  \begin{enumerate}
    ...
    \item Each gate $u_{i} \in C / \mathcal{U}$ is mapped to:     
      \begin{equation*}
        ...
        \end{split}
      \end{equation*}
    \item Each gate $ u \in \mathcal{U} $ is mapped to the collection of gates $\{ v_{j} \}_{j = -\infty}^{\infty}$ such that each of them has the same time coordinate as $ u $, is in the register that corresponds to the image of $ u $'s register, and is rooted at $\text{root}(v) + jn$. 
  \end{enumerate}
