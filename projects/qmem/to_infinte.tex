...
Let $u = (g, l)$ be a decoding gate. The restriction of the image of $u$ in $C^{\infty}$ to $[w]$ is $u$ (when applied to the infinite domain). Namely:
  \begin{equation*}
    \begin{split}
      \bigl(\prod_{a \in \mathbb{Z}^{d} } a * u \bigr)|_{[w]} = u
    \end{split}
  \end{equation*}
\end{claim}
\begin{proof}
  Denote by $\hat{a}^{j} \in \mathbb{F}_{2}^{d}$ the vector which assigns in its $i$th coordinate $1$ if the root that supports the face of $u_{j}$ has a $g_{i}$ power greater than $n$. We argue that the $j$th face of $-\hat{a}^{j} * u$ is in $[w]$ and that the $j$th face of any other root translation of $u$ is not in $[w]$. That implies that the restriction
  \begin{equation*}
    \begin{split}
      \bigl(\prod_{a \in \mathbb{Z}^{d} } a * u \bigr)|_{[w]}
    \end{split}
  \end{equation*}
...
By the definition of root translation, we get that $(\hat{a}^{j} * u)_{j}$ is in $[w]$. Let $a^{\prime} \in \mathbb{Z}^{d}$ be such that $a^{\prime} \neq \hat{a}^{j}$, and denote by $i$ the coordinate on which they differ. Then, we get that the $g_{i}$ power of the root of $(a^{\prime} * u)_{j}$ is at a distance $n$ from the $g_{i}$ power of the root of $(\hat{a}^{j} * u)_{j}$; therefore, their roots can't both be in the range $[0,n-1]$, which implies that $(a^{\prime} * u)_{j} \notin [w]$.
