...
\item Consider two chains $x,y$ that has an intersection. By the construction of the stabilizer $z$ which minimizes $\Size{x + y + z}$, we get that $z$ is either zero or has an intersection with $x+y$. Observes that if $z\cap x = z \cap y$ then $z$ introduce new bits.
  have an intersection with $x+y$.  Therefore $z\cap x \neq z \cap y$ and  their vertex domains intersect. Moreover, for any $z$ that it's addition to $x+y$ decreasing the size it must to have that $z$ intersects $x+y$. Thus, for a fixed $z \in \mathcal{C}_{Z}^{\perp}$ that decreasing the size of $x+y$, we have: 
  \begin{equation*}
    \begin{split}
