...
\begin{frame}
  \title{McDiarmid's Inequality for Local Stochastic Noise} 
...
\begin{block}{Today:}
\begin{itemize}
  \item McDiarmid's Inequality for Local Stochastic Noise.
  \item Dependence. 
...
Suppose that $\prb{S}$ is not bounded from beneath by $C_{1}p^{|S|}$. Then one can define the distribution $\mathcal{D}^{\prime}$ such that:
  \begin{align*}
    ...
    \end{cases}
  \end{align*}
  Normalize by:\footnote{ if $C_{1} < 1$ } 

  \begin{align*}
    \xi & =  \sum_{ \prb{\mathcal{D}(S) < C_{1}p^{|S|}}} C_{1}p^{|S|} + \prb{\mathcal{D}(S) > C_{1}p^{|S|}} \\
    & \ge \begin{cases}
      \sum_{S} p^{|S|} \ge 1  & \text{ if } C_{1} < 1 \\
      C_{1} \sum_{S} p^{|S|} \ge C_{1} & \text{else} 
        \end{cases}  \\ 
        & \ge C_{1}
  \end{align*}
\end{frame}
...
\begin{frame}
  \frametitle{Trick.}
  So for $C_{1} > 1$ we have: 
  \begin{align*}
    \exppm{ f }{\sim \mathcal{D}^{\prime} }  = & \frac{1}{\xi} \sum_{ \prb{\mathcal{D}(S) < C_{1}p^{|S|}}} f(S)C_{1}p^{|S|} + \sum_{ \prb{\mathcal{D}(S) \ge C_{1}p^{|S|}}}f(S)D(S) \\
    \le & \frac{1}{\xi}\exppm{ f }{\sim \mathcal{D} } \\
    \le & \exppm{ f }{\sim \mathcal{D} }
  \end{align*}
...
\begin{frame}
 ...
 Then: 
\begin{align*}
  & \prbm{f - \exppm{f}{ \sim\mathcal{D} } > t }{\sim\mathcal{D}} \\
  &\le  \prbm{f - \exppm{f}{ \sim\mathcal{D} } > t }{\sim\mathcal{D}^{\prime} \text{not normalized} } \\ 
  &\le  \frac{1}{C_{1}}\prbm{f - \exppm{f}{ \sim\mathcal{D} } > t }{\sim\mathcal{D}^{\prime} } \\ 
  &\le  \prbm{f - \exppm{f}{ \sim\mathcal{D} } > t }{\sim\mathcal{D}^{\prime} } \\
  &\le  \prbm{f - \exppm{f}{ \sim\mathcal{D}^{\prime} } > t }{\sim\mathcal{D}^{\prime} } 
\end{align*}
