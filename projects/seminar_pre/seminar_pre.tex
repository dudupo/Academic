...
\begin{tikzpicture}
  ...
  \node (bit) [draw, rectangle, below of=start, yshift=-1cm] {Bit observes a flip with probability $p$};
  \node (expectation) [draw, rectangle, below of=bit, yshift=-1cm] {Expected flips in block of length $n$: $pn$};
  \node (inequality) [draw, rectangle, below of=expectation, yshift=-1cm] {Inequality: $\eta (\beta + p) n \le \beta n$};
  \node (solution) [draw, rectangle, below of=inequality, yshift=-1cm] {Solution: $\beta \ge \frac{p}{1 - \eta}$};
  \node (end) [draw, rectangle, below of=solution, yshift=-1cm] {End};
