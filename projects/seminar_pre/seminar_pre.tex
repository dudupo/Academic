...
\begin{tikzpicture}[
  ...
  every node/.style={draw,minimum size=6mm,inner sep=1pt},
  code/.style={fill=blue!10},
  gate/.style={fill=red!10},
  scale=1.1, transform shape
]

% Level 0 - Logical gate
\node[gate] (G0) {Gate}
  child {node[code] (B1) {Block}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}}
  child {node[code] (B2) {Block}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
    child {node[code] {Qubit}}
...
% Labels
\node[above=0.2cm of G0] {Level 0 (Logical)};
\node[above=0.2cm of B1] {Level 1};
\node[below=0.2cm of B1-3] {Level 2};
