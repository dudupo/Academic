...
\begin{center}
\begin{tikzpicture}[
  gate/.style={draw, rectangle, minimum width=10mm, minimum height=6mm, fill=red!10},
  bit/.style={draw, circle, minimum size=3mm, fill=blue!10},
  voter/.style={draw, ellipse, minimum height=6mm, fill=green!20},
  wire/.style={-Latex},
  node distance=6mm and 5mm,
  every label/.style={font=\scriptsize}
  ]

% Input bits (logical A, B, C)
\node (A1) [bit, label=left:$A_1$] {};
\node (A2) [bit, below=of A1] {};
\node (A3) [bit, below=of A2] {};

\node (B1) [bit, right=20mm of A1, label=left:$B_1$] {};
\node (B2) [bit, below=of B1] {};
\node (B3) [bit, below=of B2] {};

\node (C1) [bit, right=20mm of B1, label=left:$C_1$] {};
\node (C2) [bit, below=of C1] {};
\node (C3) [bit, below=of C2] {};

% XOR gates
\node (X1) [gate, right=8mm of C1] {\scriptsize XOR};
\node (X2) [gate, right=8mm of C2] {\scriptsize XOR};
\node (X3) [gate, right=8mm of C3] {\scriptsize XOR};

% Majority gate
\node (Maj) [voter, right=14mm of X2] {\scriptsize MAJ};

% Output
\node (Out) [right=of Maj] {$L$};

% Wires from A to XOR
\draw[wire] (A1) -- ++(5mm,0) |- (X1.west);
\draw[wire] (A2) -- ++(5mm,0) |- (X2.west);
\draw[wire] (A3) -- ++(5mm,0) |- (X3.west);

% Wires from B to XOR
\draw[wire] (B1) -- ++(5mm,0) |- (X1.west);
\draw[wire] (B2) -- ++(5mm,0) |- (X2.west);
\draw[wire] (B3) -- ++(5mm,0) |- (X3.west);

% Wires from C to XOR
\draw[wire] (C1) -- (X1.west);
\draw[wire] (C2) -- (X2.west);
\draw[wire] (C3) -- (X3.west);

% XOR to Majority gate
\draw[wire] (X1.east) -- ++(4mm,0) |- (Maj.north);
\draw[wire] (X2.east) -- ++(4mm,0) -- (Maj.west);
\draw[wire] (X3.east) -- ++(4mm,0) |- (Maj.south);

% Majority to output
\draw[wire] (Maj.east) -- (Out);
