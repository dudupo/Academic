...
\begin{equation*}
  \begin{split}
    ...
    \le & \left( (1-p)2^{\kappa - 1} \cdot p^{2} + p 2^{\kappa-1} p \right)^{2} \\
...
Consider a subset of qubits $S$. Suppose that for no two of them, all the qubits in the intersection of their light cones were flipped.

Then the probability that $D$ fails to 'correct' all the qubits in $S$ is less than $p^{\frac{100}{99} \cdot |S|}$. \inb{( $C p^{\frac{100}{99} \cdot |S|}$. ) }
\end{block}
...
If there are two qubits that share a check, then the decoder doesn't fail to 'correct' them, and therefore the probability that the decoder fails to 'correct' all the qubits in $S$ is $0$.

So, assume that the light cones of any two of them share only (q)bits. Thus, the probability is less than
 
 \begin{equation*}
   \begin{split}
     & \inb{C} \left( (1-p)2^{\kappa^\prime - 1} \cdot p^{2} + p 2^{\kappa^\prime-1} p \right)^{|S|} \\
     \le & \inb{C} \left( p^{\frac{100}{99}} \right)^{|S|}
   \end{split}
 \end{equation*}
...
Consider the qubit at the bottom left corner. The decoder will check all three checks and will flip the qubit if the bottom left check is the only one that is unsatisfied.
\end{block}
\end{frame}
