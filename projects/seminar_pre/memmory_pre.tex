...
\newcommand*{\Axes}{
    ...
    \draw[->]  (0,0)coordinate(O) -- (11,0) node[anchor=north] {$t$};
...
\DECSLIDE{test_test_correction_by_None_2_8-20_30x4_plus_accu.svg}{Without Decoding}
...
\DECSLIDE{test_test_correction_accu_all_major_2_8-20_30x4.svg}{ Majority }
\DECSLIDE{test_test_correction_all_major_change_diff-2_2_8-20_30x4.svg}{$\mu$-Majority}


\DECSLIDERAW{test_test_correction_all_major_change_diff-2_lower_noise_accu_2_8-20_30x4.svg}{test_test_correction_all_major_change_diff-2_accu_2_8-20_30x4.svg}{test_test_correction_all_major_change_diff-2_high_noise_accu_2_8-20_30x4.svg}{$\mu$-Majority}
...
\begin{block}{Alg.}
  At the initialization, We divide the bits into subsets (colors). Bits of the same color don't share checks. Then, at $i$th correction round only bits associated with the $i$th color \footnote{$i \mod \textbf{COLORS}$} apply a correction. 
\end{block}

\begin{block}{Justification.}
  \begin{enumerate}
    \item No collisions, means that bits don't wait until they can read the syndrome.  
    \item The independence might help when doing math. 
  \end{enumerate} 
\end{block}
...
\DECSLIDE{test_test_correction_accu_by_colors_2_8-20_30x4.svg}{Colors}
\begin{frame}
...
\begin{block}{Alg.}
  We define a north vector $\mathbf{1} = (1,1,1..1)$, Then each \textbf{vertex} looks only on the \textbf{cells} on its neighbourhood such that they are positive relative to $\mathbf{1}$.
...
\DECSLIDE{test_test_correction_by_SWIFT_2_8-20_30x4_plus_accu_t_100.svg} {SWIFT }
\DECSLIDE{test_test_correction_by_SWIFT_2_8-20_30x4_plus_accu_fine} {SWIFT }
\DECSLIDE{test_test_correction_by_SWIFT_2_8-20_30x4_plus_accu} {SWIFT }
...
\begin{enumerate}
  \item Storing time is, likily, not $\Theta(L)$, might be. $\Theta(L^{\alpha})$ for $\alpha \in (0,1)$.   
  \item $ \Rightarrow $  implies that the structure of the theorem/key-lemma is diffrernet than known refrigerator.  
\end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Setting}
    We allow a non-nosy, storng as we wish, classical computation aside. Yet we would like to limit the communication to a constant number of bits per logical qbit, in each round.
\end{block}

\begin{block}{Justification}
  \begin{enumerate}
    \item<+-> If we don't have a constant overhead in depth, in that model, $\Rightarrow$ we also don't have it in the standard model. 
    \item<+-> In that model, gate teleportation for a constant dimension is easy. Namel, for the Topological codes the task reduced only to find a decoding with constant hint.  
  \end{enumerate}
\end{block}
...
\begin{block}{Justification.}
  \begin{enumerate}
    ...
    \item From mathematical point of view, if the number of colors is $c$, then we handle a fraction of at least $1/c$ of the dirty bits.
  \end{enumerate}
\end{block}
...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    \node (math) at (0,0) {\includegraphics[width=0.8\textwidth]{../synthesis/math.png}};
    \node (math2) at (1,-4) { \includegraphics[width=0.5\textwidth]{../synthesis/math-2.png}};
