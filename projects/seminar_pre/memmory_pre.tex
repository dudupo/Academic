...
\draw[dashed] (0,4) -- (9,4)  node[anchor=west] { $ \hat{q}$ };
  ...
  \draw[domain=0:5,smooth,variable=\x,blue,name path=c1] plot ({\x},{ 2*tanh(\x + 0.5 ) }) node[above]{$f_1(n)$};
  \draw[domain=5:8,smooth,variable=\x,blue,name path=c2] plot ({\x},{ 1.99 + 2*tanh((\x - 5) ) }) node[above]{$f_1(n)$};
