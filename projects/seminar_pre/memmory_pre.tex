...
\draw[dashed] (0,4) -- (9,4)  node[anchor=west] { $ \hat{q}$ };
  \draw[dashed] (0,2) -- (9,2)  node[anchor=west] { $q$ };
    \draw[domain=0:6,smooth,variable=\x,blue,name path=c1] plot ({\x},{ 4*tanh(\x) }) node[above]{$f_1(n)$};
    \draw[domain=0:6,smooth,variable=\x,blue,name path=c1] plot ({\x},{ 2*tanh(\x) }) node[right]{$f_1(n)$};
