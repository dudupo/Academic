...
\draw[dashed] (0,4) -- (9,4)  node[anchor=west] { $ \hat{q}$ };
  ...
  \draw[domain=0:8,smooth,variable=\x,blue,name path=c3] plot ({\x},{ 4*tanh(\x + 0.25) }) node[above]{$I(t)$};
  \draw[domain=0:8,smooth,variable=\x,blue,name path=c1] plot ({\x},{ 2*tanh(\x + 0.5 ) }) node[above]{$D(t)$};
\end{uncoverenv}
  %\draw[domain=5:8,smooth,variable=\x,blue,name path=c2] plot ({\x},{ 1.9 + 2.1*tanh((\x - 5) ) }) node[above]{$f_1(n)$};
  \draw[domain=0:11,smooth,variable=\x,red,name path=c4] plot ({\x},{exp(-\x) })node[above]{$D(t)$};
