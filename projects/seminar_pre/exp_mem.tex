...
\begin{frame}
  ...
  \begin{block}{Claim}
    Consider the application of a $2p(1-p)$-depolarizing channel. Let $S$ be a subset of qubits chosen uniformly random \footnote{each qubit is taken with probability $\frac{1}{2}$}, then the probability that there are more than $\frac{1}{100}$ depended qubits in $S$ is less than $2e^{-c\sqrt{n}}$.
  \end{block}
  ...
  \begin{block}{Proof}
    ...
    \begin{align*}
      & \exppm{  \text{Type II depended qubits in} S  }{ \sim 2p(1-p) } \\ 
    ...
    \begin{align*}
      & \exppm{  \text{Type I depended qubits in} S  }{ \sim 2p(1-p) } \\ 
...
\begin{frame}
  \frametitle{Dependence}
  Thus for small enough  $p \in (0,1)$, with probability greater than $1 - 2e^{-\frac{2\alpha^{\frac{2}{3}}}{\Delta^{l}}\sqrt{n}}$, the number of depended bits is less than:
  \begin{align*}
