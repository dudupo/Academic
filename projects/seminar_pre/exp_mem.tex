...
\draw node(u2) at (3,3) [draw,shape=rectangle]{$u_2$};
\draw node (x) at (0,6) [draw,shape=circle]{$x$};
\draw node (z) at (0,3) [draw,shape=circle]{$z$};
\draw node (y) at (0,1) [draw,shape=circle]{$y$};
\draw (u1) -- (x)
  (u1) -- (z)
  (u2) -- (y)
  (u2) -- (z);
\end{tikzpicture}
  ...
  \begin{align*}
    \prb{ (x,y) \in e | z \text{is not flipped} } &= \prb{ x \in e | z \text{is not flipped} }\prb{ y \in e | z \text{is not flipped} } \\ 
