...
\begin{frame}{Locality.}
  Denote by $U$ the uinion of the qubits and check bits. For any set $K\subset U$ with $\Gamma_{Q}(K)\cap \Gamma_{Q}(U / K) = \emptyset$ there is a valid executation of the Decoder, on the input $(E \cap K, D\cap \Gamma_{X}(K))$ whose output is $\hat{E}\cap K$ and whose support is $U \cap K$.
  \begin{block}
