...
\begin{frame}
  ...
  \begin{block}{Demonstration on the classical case.}
    Consider the classical expander code, defined via $(\gamma,\varepsilon)$- expander Tanner graph. Nemly, each subset on left at size at most $\gamma n$ sees $(1-2\varepsilon)\Delta$ uniq checks (checks that connected by single edge to the subset).
    \end{block}
