...
Suppose we have the following Lemma: 
  \begin{block}{Nice To Have Lemma.} There is a function $f : \mathbb{N} \rightarrow \mathbb{R}$, such that for any (encoded) $\rho$ which is subjected to $q$-local stochastic noise and a subset of qubits $S$ we have:  
    \begin{enumerate}
      \item With high probability ($1 - e^{-\sqrt{n}}$), after the decoding stage, the probability that the error is (completely) supported on $S$ is bounded by $q^{f(|S|)}$
...
\begin{equation*}
  \begin{align*}
    \prb{S} &  \le \sum_{S^{\prime} \subset S }{q^{f(S^{\prime}) }p^{|S /S^{\prime}|}  } \le \left( q^{\alpha} + p \right)^{|S|} \le q^{|S|}
  \end{align*}
\end{equation*}
\end{frame}
