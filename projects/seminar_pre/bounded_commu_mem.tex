...
\begin{frame}
  \begin{figure}[h]
    ...
    \begin{tikzpicture}%[scale=2.5]
      ...
      \draw node[draw,shape=rectangle, fill=yellow] (v0) at (3,1) {$u_1$};
\draw node[draw,shape=rectangle](v6) at (3,3) {$u_2$};
\draw node[draw,shape=circle] foreach \x in {1,2,3,4,5} (v\x) at (0,\x) {$v_\x$};
