...
\begin{multicols*}{2}
  ...
  \begin{equation*}
    \begin{split}
      \expp{B_{i}^{2} } &= \expp{ \left( \sum_{j}{X_{ij}} \right)^{2}  }= \expp{ \sum_{j,j^{\prime}}{ X_{ij}X_{ij^{\prime}}}  } \\
     % &= \sum_\left\{ j,j^{\prime} \right\}{ \expp{X_{ij}}\expp{X_{ij^{\prime}}}   } =  \sum_\left\{ j \neq j^{\prime} \right\}{ \expp{X_{ij}}\expp{X_{ij^{\prime}}}   }  + \sum_{j}{ \expp{X_{ij}}} = \\
     % &= \binom{n}{2} + 1 = O\left( 1 \right)     
    \end{split}
  \end{equation*}
