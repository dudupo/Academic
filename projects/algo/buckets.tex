...
\begin{multicols*}{2}
  ...
  \begin{equation*}
    \begin{split}
      \exp{B_{i}^{2} } &= \exp{ \left( \sum_{j}{X_{ij}} \right)^{2}  }= \exp{ \sum_{j,j^{\prime}}{ X_{ij}X_{ij^{\prime}}}  } \\
      &= \sum_\left\{ j,j^{\prime} \right\}{ \exp{X_{ij}}\exp{X_{ij^{\prime}}}   } =  \sum_\left\{ j \neq j^{\prime} \right\}{ \exp{X_{ij}}\exp{X_{ij^{\prime}}}   }  + \sum_{j}{ \exp{X_{ij}}} = \\ &= \binom{n}{2} + 1 = O\left( 1 \right)     
    \end{split}
  \end{equation*}
