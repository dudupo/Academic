...
\end{proof}
\begin{claim} If there exists an odd number $n > 1$ such that there is a valid protocol to solve the Dining Philosophers problem for $n$ philosophers, then there exists an even number $n' = 2n$ such that the problem can be solved by a protocol.
\end{claim}
\begin{proof}
  Let $P_1, P_2, ..., P_{2n}$ denote the $2n$ philosophers sitting around the table, and let $F_1, F_2, ..., F_{2n}$ denote the forks placed between them. Consider the running of the $n$-philosophers protocol over $2n$-philosophers. That it, each philosopher execute exactly the same code as it would execute in the $n$-philosophers protocol. 
...
\begin{proof}
    Let $\mathcal{A}$ be a protocol, and consider the protocol $\mathcal{A}^{\prime}$ in which each time that one of the philosopher is 
  \end{proof}
