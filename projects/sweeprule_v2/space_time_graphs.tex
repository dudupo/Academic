...
\begin{claim}
  The maximal size of a fork is $2d$. 
\end{claim}
\begin{proof}
Consider the rules of forks connecting in \Cref{def:contour}. For forks obtained by rules (a), (b), (d), or as a result of a fault, it's clear that their vertices at their ends are on intersecting faces (when projecting the vertices on $\mathcal{G}$). The maximal fork, in those cases, has the form of:
  \begin{equation*}
    \begin{split}
    \bigl\{ v , \bigl( \prod_{g \in S^{\prime}_{1}} \bigr) \bigl( g \prod_{g \in S^{\prime}_{2}} g \bigr) v    \bigr\}
    \end{split}
  \end{equation*}
Where $S^{\prime}_{1}, S^{\prime}_{2}$ are subsets of $S$ of size $d^{\prime}$, and their total size is $2d^{\prime}$.
 

For rule number (c), consider a fork connecting two vertices $u_{1}, u_{2}$ that are both connected to a third vertex $v$ by arrow according to (c)-rule. By definition of the rule there are two faces $f_{1}, f_{2}$ such that $u_{i} \in f_{i}$ and $v \in \phi(f_{i})$.  
\end{proof}
  \begin{definition}
