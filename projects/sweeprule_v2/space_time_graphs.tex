  ...
  \begin{definition}
    Consider a graph $G = (V, A, F)$, where $V$ is a set of vertices, $A$ is a set of directed edges, and $F$ is a set of undirected edges. For $v \in V$, denote by $\pre{v} = \{ u \in V \colon \{u,v\} \in A \}$, we extend the notion for subsets of vertices $X \subset V$ by $\pre{X} = \bigcup_{v \in X} \pre{v}$. Let $ T \colon V \rightarrow \mathbb{N}$, We say that $(G,T)$ is a time graph if and only if:
\begin{enumerate}
...
\end{remark}
  ...
  \ctt{ Is not correct when $A$ is directed. }
  \end{claim}
  \begin{proof}
