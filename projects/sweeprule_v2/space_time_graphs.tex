...
\begin{claim}
  \label{claim:maxfork}
  The maximal size of a fork is $8d^{\prime} = 4d$.
\end{claim}
\begin{proof}
Consider the rules of forks connecting in \Cref{def:contour}. For forks obtained by rules (a), (b), (d), or as a result of a fault, it's clear that their vertices at their ends are on intersecting faces (when projecting the vertices on $\mathcal{G}$). The maximal fork, in those cases, has the form of:
  \begin{equation*}
    \begin{split}
   F =  \bigl\{ v , \bigl( \prod_{g \in S^{\prime}_{1}} g\bigr) \bigl(  \prod_{g \in S^{\prime}_{2}} g \bigr) v    \bigr\}
    \end{split}
  \end{equation*}
  Where $S^{\prime}_{1}, S^{\prime}_{2}$ are subsets of $S$ of size $d^{\prime}$, and therefore the total size of such forks is $\Size{F} = 2d^{\prime}$.
 

  For rule number (c), consider a fork connecting two vertices $u_{1}, u_{2}$ that are both connected to a third vertex $v$ by an arrow according to the (c)-rule. By definition of the rule, there are two faces $f_{1}, f_{2}$ such that $u_{i} \in f_{i}$ and $v \in \phi(f_{i})$. Thus, $\phi^{-1}(v) \in f_{1} \cap f_{2}$, or in other words, $u_{1}, u_{2}$ belong to two intersecting faces, and we return to the previous cases. \ctt{Incorrect.}


For rule number (e), we use the fact that in realizing logical gate $\mathbf{S}$, we concatenate three layers (logical $\mathbf{CX}$, $\phi \circ \mathbf{H}$, and $\mathbf{CX}$). Thus, by a light cone argument, the ends of the forks are far away from each other by at most $2^{3}$ faces, so the sizes of forks are at most $8d^{\prime}$.
\end{proof}
  \begin{definition}
...
Furthermore, we say that $\hat{K}$ is a result of a \textit{sweep-cycle}, \textit{faults}, or a \textit{logical operation} if the layer at time $T(\hat{K}) - 1$ consists of sweep-cycle gates, fault gates (whose source is $\mathcal{E}$), or logical gates. The \textit{order} of the slice is its time modulo $4$, namely $\text{order}(\hat{K}) = T(\hat{K}) \mod 4$. Observe that slices that are results of faults have an order that is either $0$ or $2$, whereas slices that are results of a sweep-cycle or logical operation are of orders $1$ and $3$, respectively.
  \end{definition}

  \begin{corollary}
    Consider a spanned slice $\hat{K}$, and let $\hat{K}_{0}, \ldots, \hat{K}_{k}, F_{1}, \ldots, F_{k}$ be the predecessor decomposition of $\hat{K}$. 
    \begin{enumerate}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      \item For $g = \varphi$ and $g = \phi \circ \mathbf{H}$, the gates apply. The size is the same size for each of $\hat{K}_{i}$, the size of the .., this is not good, since we want to say something on the poles. At least for $\mathbf{H}$ it seems like we can say that $k=1$. For the $\mathbf{CX}$, the $\prei{x}$ belongs to one of the $\hat{K}_{j}$, since $ (v,t+1) \in V_{\mathcal{H}}$, only if there is $u \sim_{\mathcal{G}} v$ such that $(u,t-1) \in V_{\mathcal{H}}$. 
    \end{enumerate}
  \end{proof}

  \begin{corollary}
    For any $i \in [k]$, denote by $\hat{K}_{0}^{i},..,\hat{K}_{k_{i}}^{i},F_{1}^{i},..,F_{k_{i}}^{i}$ the be the predecessor decomposition of $\hat{K}_{i}$. Then if $\hat{K}$ is a result of..
  \end{corollary}
  \ctt{ 
