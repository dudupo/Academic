...
Furthermore, we say that $\hat{K}$ is a result of a \textit{sweep-cycle}, \textit{faults}, or a \textit{logical operation} if the layer at time $T(\hat{K}) - 1$ consists of sweep-cycle gates, fault gates (whose source is $\mathcal{E}$), or logical gates.
  ...
  \begin{corollary}
    ...
    \begin{enumerate}
      \item If , namely at the end of 
          \item If $t$ is the 
      \end{enumerate}

    at the end of even layer, namely at the end of the sweep-cycle. Let $g$ be the logical gate applied preceding to the sweep-cycle. Denote by $\hat{K_{0}},..,\hat{K_{l}}$ the decomposition, according to .., and for and for any $i \in 0,..,l$, let $\hat{K_{0}}^{i},..\hat{K_{l_{i}}}^{i}$ be the decomposition of $\hat{K_{i}}$. Then: 
    \begin{equation*}
      \begin{split}
