 \section{Explantation of a Spanned Slice}
  ...
  \begin{definition}
    Consider a Tooms contour graph induced by Toric encoded circuit, with sweep-cycle parameter $\xi = 32$, and  let $\alpha, \beta$ be constants and $\mathcal{X}_{0}, \mathcal{X}_{1}, \mathcal{X}_{2}, \mathcal{X}_{3} \in \mathbb{R}$ be quadruple, defining such:
    \begin{equation*}
      \begin{split}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. Thus, by \cref{claim:prevslices}, we know that for some $k$ there exist spanned slices $\hat{K}_{0}, .., \hat{K}_{k}$ and forks $F_{1},..,F_{k}$ that satisfy \cref{claim:prevslices}. In particular, for $r < 3$ it holds that: 
        ...
        \begin{equation*}
          \begin{split}
            \sum_{i=0}^{k}\Size{\hat{K}_{i}} +  \sum^{k}_{i=1}\Size{F_{i}} \ge \Size{\hat{K}} + \xi = \Size{\hat{K}} + 32
          \end{split}
        \end{equation*}
