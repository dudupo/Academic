 \section{Explantation of a Spanned Slice}
  ...
  \begin{definition}
    Consider a Tooms contour graph induced by Toric encoded circuit, with sweep-cycle parameter $\xi = 4\sqrt{d+1}$, and  let $\alpha, \beta$ be constants and $\mathcal{X}_{0}, \mathcal{X}_{1}, \mathcal{X}_{2}, \mathcal{X}_{3} \in \mathbb{R}$ be quadruple, defining such:
    \begin{equation*}
      \begin{split}
        \alpha &\colonequals 10(d+1), \ \ \  \beta \colonequals 5 \sqrt{d+1} \\
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item Induction Step: $h \neq 0$ implies $K \neq \History{K}$. Thus, by \cref{claim:prevslices}, we know that for some $k$ there exist spanned slices $\hat{K}_{0}, .., \hat{K}_{k}$ and forks $F_{1},..,F_{k}$ that satisfy \cref{claim:prevslices}. In particular, for $r < 3$ it holds that: 
        ...
        \begin{equation*}
          \begin{split}
            \sum_{i=0}^{k}\Size{\hat{K}_{i}} +  \sum^{k}_{i=1}\Size{F_{i}} \ge \Size{\hat{K}} + \xi = \Size{\hat{K}} + 4\sqrt{d + 1}
          \end{split}
        \end{equation*}
...
&  \mathcal{X}_{0} \le  \mathcal{X}_{0} - 4\frac{d+1}{\alpha}  + \frac{\beta}{\alpha}\xi  \\
 & \Size{\text{fork}} \le \frac{\alpha}{\beta} \mathcal{X}_{r} 
  \end{split}
\end{equation*}
And those inequalities hold for $\beta = \frac{d+1}{\xi}$ and $\alpha = \beta \Size{\text{fork}}$.
        \end{enumerate}
