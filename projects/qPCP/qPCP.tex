...
\begin{claim} \textbf{qPCP} $\subset $ \CLASS. \ctt{ For the claim to be true we have to pick $w$ from a subset of qubits such any of them supports at most $O(1)$ local terms. That requirement is problematic since the clock qubits are involved in all terms. We will have to overcome that issue}. 
