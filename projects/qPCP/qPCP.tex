...
\begin{definition}
  Given $k$-local Hamiltonian $H$ overt $n$ qubits it is promised that either for any quantum states $\braket{\psi|H|\psi} \ge b$ or:\begin{enumerate}
  ...
  \end{enumerate}
  Notice that if $b-a \in 1/poly(n)$ then the deciding which of the case holds is a problem in $QMA$.  
\end{definition}
