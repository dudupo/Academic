...
\begin{definition}
  Given $k$-local Hamiltonian $H$ either for any quantum states $\braket{\psi|H|\psi} \ge b$ or:\begin{enumerate}
    \item There exists $\ket{\psi}$ such that $\braket{\psi|H|\psi} \le a$
    \item For any $w\subset [n]$ the state $\ket{\phi} = \ket{\psi}|_{w} = Tr_{w}\left( \ket{\psi} \right)$ has energy at most $\braket{\phi|H|\phi|} \le a + \frac{b-a}{2}$
  \end{enumerate}<++>
