...
\begin{definition}
  Let $C = U_{T}U_{T-1}..U_{0}$ be a quantum circuit. Denote by $Z(C)$ the (random varible) circuit that obtained by the following random process: $Z(C)$ is the chainging of $X_{S}X_{S-1}..X_{0}$ such that $X_{0}=U_{0}$, $X_{S}=U_{T}$ and $X_{i}$ defined reqursevily, if $X_{i-1} = U_{j}$ for $j>0$ then $X_{i}$ is chosen uniformly from  $\{ U_{j+1}, U_{j}^{\dagger} \}$, We will call to any circuit that can be a results of such process a $C$-Zigzag. 
\end{definition}


\begin{definition}
  Let $C = U_{T}U_{T-1}..U_{0}$ be a quantum circuit. We will name any Hamiltonian that can be obtained by the following random process as a $C$-hashed. First, we chaing $C$ by himself $\Theta(T)$ as follow:  
  \begin{equation*}
    \begin{split}
      \rightarrow CC^{\dagger}CC^{\dagger}CC^{\dagger}C .. C^{\dagger}CC^{\dagger}CC^{\dagger}C  
    \end{split}
  \end{equation*}
  Now local term in the propgation Hamiltonian will be 
  \begin{equation*}
    \begin{split}
      I - \frac{1}{\Delta}  \left(  U_{i}\ket{t + 1 + 2T\cdot m^{\prime}}\bra{ t + 2T\cdot m} + U^{\dagger}_{t+1}\ket{t + 1 + 2T\cdot m}\bra{ t +1 + 2T\cdot m^{\prime}} \right)
    \end{split}
  \end{equation*}
  When $\Delta$ is the degree of vertex assosicated with $\ket{ t + 2T \cdot m }$ in the adjececity graph (where each time cordinated is associated with a vertex and two vertices are connected only if there is a check that checks their consist with each other).   
Let $C = U_{T}U_{T-1}..U_{0}$ be a quantum circuit. We will refer to any Hamiltonian that can be obtained by the following random process as a $C$-hashed. First, we chain $C$ with itself $\Theta(T)$ times as follows:  
  \begin{equation*}
    \begin{split}
      \rightarrow CC^{\dagger}CC^{\dagger}CC^{\dagger}C .. C^{\dagger}CC^{\dagger}CC^{\dagger}C  
    \end{split}
  \end{equation*}
  Now, the local terms in the propagation Hamiltonian will be 
  \begin{equation*}
    \begin{split}
      I - \frac{1}{\Delta}  \left(  U_{i}\ket{t + 1 + 2T\cdot m^{\prime}}\bra{ t + 2T\cdot m} + U^{\dagger}_{t+1}\ket{t + 1 + 2T\cdot m}\bra{ t +1 + 2T\cdot m^{\prime}} \right)
    \end{split}
  \end{equation*}
  where $\Delta$ is the degree of the vertex associated with $\ket{ t + 2T \cdot m }$ in the adjacency graph (where each time coordinate is associated with a vertex and two vertices are connected only if there is a check that verifies their consistency with each other).
\end{definition}
