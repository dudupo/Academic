...
\begin{claim}
  \label{claim:conj} There exists a constant $\alpha \in (0,1)$ such for sufficiently large $n$ there is a function $f: \left[ n^{2} \right] \rightarrow \left[ n \right]$ and a partition (not disjoin) $A_{1}, A_{2}, A_{3} .., A_{2n}$ such that:
  \begin{enumerate}
...
\begin{remark}
  ...
  \braket{n_{0}^{\frac{\log (1-\alpha)}{\log ( 1 - \alpha_{0})}}, \alpha}
    \end{split}
  \end{equation*}
  \ctt{The number of the groups is $4n^2$ where what's needed is $2n^{2}$ }I don't know how to prove the claim and don't sure weather is it even correct.
\end{remark}
