...
\begin{definition}
  ...
  \begin{equation*}
    \braket{\psi|M[H,q,\zeta]|\psi} = \begin{cases}
      \begin{split}
        1 & \prbm{ \braket{\psi|H_{i}|\psi}}{H_{i} \sim H(q)} \ge \zeta \\
        0 & \prbm{ \braket{\psi|H_{i}|\psi}}{H_{i} \sim H(q)} < \zeta 
    \end{split}
  \end{cases}
  \end{equation*}
\end{definition}
...
\begin{definition} \label{definition}  
  Let $H$ be a $k$-local, Denote by $M[H, \zeta]$ the $\zeta$-majority Hamiltonian relative by $H$ to be: 
  \begin{equation*}
    \begin{split}
