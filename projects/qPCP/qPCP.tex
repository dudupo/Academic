...
\begin{definition}
  The $C$-hashed-Zigzag Hamiltonian will be the mixed of the two technics above. Given circuit $C$ we first generate a $C$-Zigzag circuit from it, denote it by $C^{\prime}$ with length $T^{\prime}$ and then we applay the hash reduction, but this time we also allow connection at the form $U_{i}\ket{t + 1 + 2T^{\prime}\cdot m^{\prime}}\bra{ t^{\prime} + 2T^{\prime}\cdot m}$ where the $U_{t}..U_{0} = U_{t^\prime}..U_{0}$ in $C^{\prime}$ (basically, in time $t^{\prime}$ we have returned to the state at time $t$). 
\end{definition}
