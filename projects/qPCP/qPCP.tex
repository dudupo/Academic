...
\begin{definition}
  ...
  \begin{equation*}
    ...
    \end{split}
  \end{equation*}
  When $\Delta$ is the degree of vertex assosicated with $\ket{ t + 2T \cdot m }$ in the adjececity graph (where each time cordinated is associated with a vertex and tow vertices are connected only if there is a check that checks their consist with each other).   
\end{definition}
