...
What about defining $D = \bra{t} \mathbf{1}_{t\sim l} \ket{l} $ and $\bra{l} M_{\Delta} \ket{t}  =  \mathbf{1}_{t\sim l} 1/\Delta^{2}$ ? In that case $D$ is not unitary. 

\section{Ideas.}
\begin{enumerate}
  \item $M_{\Delta}$ doesn't exists. 
  \item $\Lambda_{t,l}$ is not a unitary matrix? 
