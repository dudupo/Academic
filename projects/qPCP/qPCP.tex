...
\begin{claim}
  \label{claim:conj} There exists a constant $\alpha \in (0,1)$ such for sufficiently large $n$ there is a function $f: \left[ n^{2} \right] \rightarrow \left[ n \right]$ and a partition (not disjoin) $A_{1}, A_{2}, A_{3} .., A_{n}$ such that:
  \begin{enumerate}
...
\begin{remark}
  ...
  Then one can easily concludes that a solution for $\braket{n_{0},\alpha_{0}}$ limitedly implies a solution for\begin{equation*}
    \begin{split}
  \braket{n_{0}^{\frac{\log (1-\alpha)}{\log ( 1 - \alpha_{0})}}, \alpha}
    \end{split}
  \end{equation*}
  I don't know how to prove the claim and don't sure weather it even correct.
\end{remark}
...
\end{claim}
\begin{proof}
  Divide $N$ qubits into $k$ group such that the intersection of any of them is exactly $\alpha n$,      We will show a reduction from \CLASS instance into $k$-local Hamiltonian with constant gap. Consider the complete graph $K_{n}$. Assign for each qubit for each edge of the graph. And for each vertex $v$    
\end{proof}
