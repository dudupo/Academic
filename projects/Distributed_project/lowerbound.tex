...
\paragraph{Conclusion And Open Problems.} The reduction above demonstrate how a known results can give us almost immedetitly insights about quantum computibility. We hope that beside of been an no-go-to proof this work will also use as an hint for direaction to other quantum advantags in the disturbuited computing setting. 

It's worth say that the $r$-rounds communiaction bound on disjointness does not hold in many cases, For simple example condsider that each of the set $x,y \in \{0,1\}^{n}$ drwan uniformly, Then it's clear that Alice and Bob could just answer ``Yes'' and they will be correct with heigh probability. The family of states which one can do project over them by only partly projection (defussion operators) are corresponed to the distributions over pairs of Alice and Bob sets which they can compute with their disjointness with less communication.        
\printbibliography 
\end{multicols*}
\end{document}
