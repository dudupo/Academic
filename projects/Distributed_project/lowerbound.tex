...
General speaking this method transform a known state $\ket{\Psi}$  with probability $a$ to measure a $\ket{i}$ to a state in which the desired measurement obtained with probability grater than $\frac{1}{2}$ at the cost of less than $\sqrt{a}$ sort of Grover iterations. Using this process, One can initialize a quantum states which spread over $n$ elements and amplify the probability to measure a desired state at $\sqrt{n}$ time. To understand the scale of power gained by this method, we mention max extraction as use-case. While any classical algorithm which runs at square time results an output deadened only on square fraction of the bits and therefore can't yield a constant probability to sample the maximum element, Quntemly this limitation doesn't hold. And the gap amplification is indeed enable a square root time maximum extraction algorithm. 
  ...
  One question that might rise is whether the above amplification process can be done assuming nothing but having a single entity of the initial state. Both positive and negetive answers to that question will shead light about the fundumentels beahind the concept of transfering probability weight. We gave a partly answer for that question by proving the follow theorem: 
