...
General speaking this method transform a known state $\ket{\psi}$  with probability $a$ to measure a $\ket{i}$ to a state in which the desired measurement obtained with probability grater than $\frac{1}{2}$ at the cost of less than $\sqrt{a}$ sort of Grover iterations. Using this process, One can initialize a quantum states which spread over $n$ elements and amplify the probability to measure a desired state at $\sqrt{n}$ time. To understand the power scale gained by this method, we mention max extraction as use-case. While any classical algorithm which runs at square time results an output deadened only on square fraction of the bits and therefore can't yield a constant probability to sample the maximum element, Quntemly this limitation doesn't hold. And the gap amplification is indeed enable a square root time maximum extraction algorithm. 
  ...
  One question that might rise is whether the above amplification process can be done assuming nothing but having a single entity of the initial state. Both positive and negetive answers will shead light about the fundumentels beahind the concept of transfering probability weight. We gave a partly answer for that question by proving that the given copy alone is not sufficent to simulate the difussion step. We formulate the above by the follow theorem: 

  \paragraph{Theorem 1} \textit{ There is no operator $D$ that for given two arbirary states $\ket{\psi},\ket{\phi}$ compute the transformation:} 
\begin{equation*}
...
The next claim argue that $D,U$ are sufficient to Alice to simulate a single iteration of the amplitude amplification. Since the techncial ditels of the amplification procdeure is not the focus of this paper, we only show equivlance without define the operators and the notation used by \cite{Brassard_2002}.   
