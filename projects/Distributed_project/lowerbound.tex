...
\title{No-Existance Of Genrilize Defussion.}
\author{David Ponarovsky}
% \date{July 2021}
\maketitle
\begin{multicols*}{2}

  \paragraph{Preamble} One of the most promised applications of quantum computation is the Amplitude Amplification algortihm, In which, one can transform a known state with probablility $a$ to measure a $\ket{i}$ to a state in which the desired measurement obtaind with probability grater than $\frac{1}{2}$ at the cost of less than $\sqrt{a}$ sort of Grover iterations. One quesion that might rise is wheter the above can be done given an single entity of the state. We show that there is no operator that given two state $\ket{\psi},\ket{\phi}$ comupte the transformation: 
...
We name the above gate the \textit{Generlize Defussion} gate, As if such gate were exists it could be used instand of the projection operator to simulate the amplitude amplification procdure. The countriduction of the existance follows by showing that using $U$ two players can compute the disjoints of their sets in single round and $O\left( \sqrt{n} \right)$ communication complexitiy.    
...
Combining the Braverman's communication  lower bound We obtain the following no-go corollary:  % \printbibliography 
\end{multicols*}
\end{document}
